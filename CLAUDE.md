...
- **2026-09-02 — GAP CLOSURE ARC A–C MERGED (#1239); ZERO-INFLATED FAMILIES APPROVED (#1240).**
  ...
  `spatial_*()` warns on a decorative `| token`, runaway-loading advice names `loading_ridge = 0.25`
  with VA's precondition, the front page/banner/DESCRIPTION are in plain words, `unit` defaults to
  `NULL` (implicit `site` still used with a once-per-session deprecation; hard requirement in 0.8).
  **The R twin now has a machine-readable ledger:** `docs/design/capability-status.md` (generated by
  `dev/gapclose/build-capability-status.R --check` from the validation-debt register; row names match
  GLLVM.jl byte-for-byte, `unit/unit_obs/cluster/cluster2` are rows) and `tools/parity_ledger.R`
  (matched / R-only / Julia-only with dispositions, CLOSURE line). **Zero-inflated `zi_poisson()`,
  `zi_nbinom2()`, `zi_binomial()`** (Laplace, per-trait intercept-only zero part, FAM-21..23 `partial`)
  approved by the maintainer on four points (vault D-207) and merged as #1240. Decision map:
  `docs/dev-log/2026-09-02-true-parity-decision-map-gllvmtmb.md`. Owed: Totoro/DRAC multi-seed
  recovery for the zi families before any row leaves `partial`; the ~999 bare aborts behind the ratchet
  test; the B3 issues are filed. **START HERE:** `docs/dev-log/after-task/2026-09-02-gapclose-arcs-abc.md`
  and `docs/dev-log/after-task/2026-09-02-gapclose-arcD-zero-inflated.md`.
