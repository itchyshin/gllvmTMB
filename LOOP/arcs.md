...
| id | arc | gate | status |
|---|---|---|---|
| O0 | Merge PR #1240 (approved D-207) once local suite+check (`scratchpad/g-all-arcD`, or rerun) and CI are green, by merge commit; then `git merge origin/main` into this lane; teardown the merged `gllvmTMB-gapclose-20260902` worktree (`git worktree remove`) and delete its remote branch is NOT allowed (leave branches) | merge is pre-approved by Shinichi | DONE 2026-09-02 (5855e2ad9) |
