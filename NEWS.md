...
* Temporal is an experimental addition beside the 5 × 3 stable-unit grid.
  `temporal_indep()`, `temporal_dep()`, and rank-one `temporal_latent()` accept
  Gaussian long and `traits(...)` wide calls. AR1 retains integer occasion
  gaps; OU uses user-supplied elapsed numeric time. With
  ...
  temporal slopes, higher rank, other families, replicated-panel lifecycle
  helpers, or temporal combinations with phylogenetic, animal, spatial, or
  kernel sources. Those combinations are retained as low-priority developer
  fixtures, not exported workflows.
