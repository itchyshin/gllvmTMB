...
* `extract_correlations()` now returns an `interval_status` column marking the
  ...
  mixed-family fits whose coverage is not yet established, and `"none"` for point-only `engine = "julia"` bridge fits.
...
* **Scope boundary.** IN: the `unique =` opt-out on the augmented ordinary
  ...
  calibration for augmented targets remains gated.
...
* Added the first fixed multi-kernel engine wave for `kernel_latent()`: two or
  ...
  component separately. IN: fixed dense PSD kernels, same grouping factor and
  ...
  non-Gaussian and cross-family coevolution models. PARTIAL: this
  ...
  claim boundary rather than a promotion. PARTIAL: broader/harder
...
* Added an `engine` argument to `gllvmTMB()` (`engine = c("tmb", "julia")`,
  ...
  IN: the bridge maps a single reduced-rank latent block
  ...
  per-trait cutpoint matrices plus per-trait category counts. PARTIAL:
...
* Added `suggest_lambda_constraints()`, a plural companion to
 ...
 . PARTIAL: the helper ranks the highest-evidence
...
* Added the public `fit-diagnostics` article, "Can I trust this fit?", as the
  ...
  `gllvmTMBcontrol(se = FALSE)` status rows, exact
  randomized-quantile residuals and fitted-model predictive displays for a
  scoped Poisson example, and report-ready metadata through
  `diagnostic_table()`, with both long and `traits(...)` wide
...
* **`latent(1 + x | unit, d = K)`** and the long-form
  **`latent(0 + trait + (0 + trait):x | unit, d = K)`** now fit the ordinary
  ...
  augmented diagonal, or their sum. Evidence:
  ...
  outside this slope-covariance lane.
  ...
  sum. Evidence: `test-ordinary-latent-random-regression.R` now covers
...
* `gllvmTMB(REML = TRUE)` adds a narrow Gaussian-only restricted maximum-likelihood pilot. IN: ordinary Gaussian random-intercept fits and Gaussian covariance fits through default `latent()` (with explicit `latent() + unique()` retained as compatibility syntax) match `glmmTMB(..., REML = TRUE)` log-likelihoods and AIC degrees of freedom in `test-gaussian-reml.R`. PARTIAL: fixed-effect profile CIs are not available for REML fits; use Wald CIs or refit with `REML = FALSE` for ML profiling. PLANNED: non-Gaussian REML, observation weights, `miss_control(response = "include")`, and `mi()` predictor models remain guarded / deferred.
...
* The new internal `cross-lineage-coevolution` article turns the Design 65 C2
  ...
  null covariance blocks. PARTIAL: the article
...
* **`bootstrap_Sigma()`** gains `link_residual = c("auto", "none")`, matching `extract_Sigma()` so bootstrap point estimates and refit summaries can either include family/link implicit residuals (`"auto"`, the existing default) or report the fitted model covariance without link-residual additions (`"none"`). IN: mixed-family bootstrap refits still preserve per-row family dispatch and the default link-residual formulas remain covered. PARTIAL: non-Gaussian bootstrap inference remains experimental under EXT-13 / CI-08 / CI-10 until the M3 target-explicit grid is rerun with the corrected `Sigma_unit_diag` convention.
* **`bootstrap_Sigma()`** gains `link_residual = c("auto", "none")`, matching `extract_Sigma()` so bootstrap point estimates and refit summaries can either include family/link implicit residuals (`"auto"`, the existing default) or report the fitted model-implied covariance only (`"none"`). IN: mixed-family bootstrap refits still preserve per-row family dispatch and the default link-residual formulas remain covered. PARTIAL: non-Gaussian bootstrap inference remains experimental under EXT-13 / CI-08 / CI-10 until the M3 target-explicit grid is rerun with the corrected `Sigma_unit_diag` convention.
