...
## `latent(lv = ...)` reserves predictor-informed latent-score means (2026-06-24)

* `latent(..., lv = ~ x)` now has an explicit reserved parser/API preflight for the Design 73 predictor-informed latent-score lane. Current releases accept the ordinary unit-tier syntax far enough to validate a one-sided `lv` formula, build the future unit-level `X_lv_B` design, and then stop before TMB construction with a clear "not implemented yet" error. IN for this slice: fail-loud parser validation for ordinary Gaussian unit-tier `latent()`, including long and `traits(...)` wide surfaces, intercept-dropping (`lv = ~ x` equals `lv = ~ 0 + x`), factor expansion, within-unit constancy, rank checks, fixed-effect overlap checks, and typed rejections for random terms, offsets, `mi()`, smooth terms, response/trait columns, `REML = TRUE`, non-Gaussian families, unsupported tiers, augmented random-regression combinations, ordinary diagonal aliases, source-specific forms, and kernel forms (`FG-18`, `LV-01`, `LV-04`, `LV-05`, `LV-06`, `LV-07`; `test-lv-parser-guard.R`). PLANNED: no likelihood, `alpha_lv_B`, ADREPORT, extractor, recovery evidence, non-Gaussian support, or Julia bridge parity is implemented yet.
