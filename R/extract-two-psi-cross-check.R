## Likelihood-based cross-check diagnostics for the two-psi PGLLVM
## decomposition. The two-psi joint REML fit decomposes the
...
##
## The split into Lambda Lambda^T and Psi is only weakly identified
...
## three-levels-of-success framing in `dev/two-psi-rewrite-plan.md`:
##
##   * `compare_dep_vs_two_psi()` (canonical, gold standard).
...
##     benchmark for the implied total covariance from the joint two-psi
##     fit. Tractable for T <= ~30.
##
##   * `compare_indep_vs_two_psi()` (cheap diagonal fallback).
...
##
## Both share the same engine, family, and link as the joint two-psi
...
#' Build the data frame, family, and grouping spec to refit a two-psi dataset
#' under a different covstruct intent.
#'
#' Pulls everything the diagnostic needs from `fit_two_psi` so the call to
...
#' Extract joint two-psi Sigma_phy and Sigma_non implied by Lambda Lambda^T + Psi
#'
...
#' the non-phylo unique on the species column (legacy two-psi layout),
#' otherwise from the B (between-unit) tier.
#'
#' @keywords internal
#' @noRd
.joint_two_psi_sigmas <- function(fit_two_psi) {
  T_n <- fit_two_psi$n_traits
...
#'
#' Replaces the user's two-psi formula with one of:
...
#' preserved verbatim from `fit_two_psi$formula`.
#'
#' @keywords internal
#' @noRd
...
#' Refit `fit_two_psi`'s data under the alt covstruct
#'
#' Wraps [gllvmTMB()] with the same family, units, and phylogeny as the
#' original two-psi fit. Errors are captured and returned as `NULL` so
...
#' (legacy two-psi layout). All four use cases reduce to the same
...
## ---- compare_dep_vs_two_psi (canonical) -------------------------------
...
#' two-psi fit. Compares the joint two-psi fit's implied
...
#' two-psi identifiability failures.
#'
...
#' benchmark for the implied total covariance. When the joint two-psi fit
...
#'   matrix or `ape::phylo` tree, only needed if `fit_two_psi` was
#'   produced by an older package version that did not store the
#'   phylogeny on the fit. Default is to recover them from the fit
#'   (`fit_two_psi$phylo_vcv` / `fit_two_psi$phylo_tree`).
#'
#' @return A list with components:
#' \describe{
#'   \item{`joint`}{The two-psi fit's implied `Sigma_phy` and `Sigma_non`
#'     (T x T matrices).}
...
#'
#' @seealso [compare_indep_vs_two_psi()] for the cheap diagonal fallback
...
## ---- compare_indep_vs_two_psi (cheap fallback) ------------------------
...
#' family, link, and grouping as the supplied two-psi fit. Compares the
#' per-trait diagonals of the joint two-psi fit's implied
...
#' unstructured fit needed by [compare_dep_vs_two_psi()] is intractable.
#' Otherwise prefer [compare_dep_vs_two_psi()] (gold standard).
#'
#' Cross-check intent: the per-trait diagonals are the simplest summary
#' the joint two-psi fit and the marginal baseline both target. If the
#' diagonals already disagree beyond the threshold, no inspection of
#' the cross-trait off-diagonals is warranted.
#'
#' @inheritParams compare_dep_vs_two_psi
#'
#' @return A list with components:
#' \describe{
#'   \item{`joint`}{Joint two-psi fit's implied `diag(Sigma_phy)` and
...
#' @examples
#' \dontrun{
#' diag <- compare_indep_vs_two_psi(fit)
#' diag$flag
#' diag$agreement
#' }
