...
#' intervals remains validation-gated. For Gaussian and binomial logit/probit/
#' cloglog `engine = "julia"` bridge fits, `ci_method = "none"` exposes point
#' estimates only (`std.error` is `NA` and `uncertainty_status` is
#' `"julia_bridge_point_estimate_only_no_ci_validation"`), while
#' `ci_method = "wald"` surfaces finite `std.error`, `lower`, and `upper`
#' (`uncertainty_status` is `"julia_bridge_wald_delta_method"`).
#'
#' @param fit A fit returned by [gllvmTMB()].
