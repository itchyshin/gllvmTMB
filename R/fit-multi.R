...
gllvmTMB_multi_fit <- function(parsed, data, trait, site, species,
  ...
  ## Source-specific auto-Psi companion (Stage A): `phylo_latent(residual=TRUE)`
  ## auto-emits a `phylo_rr(.phylo_unique, .auto_residual)` companion (the
  ...
  ## same grouping supersedes the `phylo_latent(residual=TRUE)` default, so
  ## `phylo_latent(residual=TRUE) + phylo_unique()` is byte-identical to the
  ...
  if (any(is_auto_phylo_psi)) {
    explicit_phylo_group <- vapply(seq_along(parsed$covstructs), function(i) {
      ...
      if (identical(cs$kind, "phylo_rr") && isTRUE(cs$extra$.phylo_unique) &&
          !isTRUE(cs$extra$.auto_residual)) deparse(cs$group) else NA_character_
    }, character(1L))
  ...
  ## auto-emitted Psi entirely (plain `diag` and phylo companions alike).
