...
gllvmTMB_multi_fit <- function(parsed, data, trait, site, species,
  ...
  ##   * phylo_unique (.phylo_unique=TRUE) -> populates phylo_diag (U_phy diag)
  ...
  ## diag(U_phy). This is the manuscript-aligned two-U PGLLVM decomposition
  ...
  tmb_data <- list(
    ...
    ## Two-U PGLLVM: per-trait phylogenetic random intercepts (U_phy diag)
  ...
  tmb_params <- list(
    ...
    ## Two-U PGLLVM: per-trait phylogenetic random intercept (U_phy diag).
