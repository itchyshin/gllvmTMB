...
# bridge payload carries a p x n trials matrix N (GLLVModels.jl
...
# (GLLVModels.jl _BRIDGE_NO_CI_FAMILIES): must never be advertised as CI-capable.
...
.gllvm_julia_gate_registry <- function() {
  data.frame(
    ...
    reason = c(
      ...
      "Profile/bootstrap confidence intervals for predictor-informed latent-score bridge rows are not admitted; Wald X_lv CI payloads are routed as reader plumbing only.",
...
.gllvmTMB_julia_dispatch <- function(
  ...
  ## GLLVModels.jl's structured-sources payload (identity covariance over the group
