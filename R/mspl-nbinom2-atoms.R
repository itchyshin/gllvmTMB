## Internal nbinom2 LA-MSPL atoms (Pure-R admit-packet science).
##
...
##
## Jeffreys-on-phi: DROPPED. Mean-atom GLM-outer W = mu*phi/(phi+mu)
## is unchanged. Helpers are not exported. Registry stays planned.
## These pins do not change the live tape (still unpinned c=1).
...
.gllvmTMB_mspl_nbinom2_trait_moments <- function(y, trait_id) {
  ...
  if (length(y) != length(trait_id)) {
    stop(
      "nbinom2 MSPL moments require y and trait_id of equal length.",
      call. = FALSE
    )
  }
