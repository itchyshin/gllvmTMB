...
profile_temporal <- function(object, level = 0.95, ...) {
  ...
  if (!identical(object$temporal$mode, "indep") ||
    ...
    .temporal_abort("{.fn profile_temporal} currently supports unreplicated Gaussian {.fn temporal_indep} fits only.")
  }
  active <- .gllvmTMB_predict_unhandled_re_tiers(object, handled = "temporal")
  if (length(active)) .temporal_abort("{.fn profile_temporal} currently requires the temporal source by itself.")
