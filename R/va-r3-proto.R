...
.va_r3_validate_data <- function(y, n_trials, X, unit_id, trait_id, q,
  ...
  if (family == "binomial") {
    ...
    ## n_trials >= 1 admits Bernoulli.  The n_trials >= 2 bound this replaces
    ## was Design 85's scope freeze, not a mathematical requirement: n_trials
    ## enters the objective only as the multiplier of the softplus expectation
    ## and inside log C(n, y), and log C(1, y) = 0 exactly for y in {0, 1}.
