...
| **M3.4** | Robust modeling: starts, `pdHess`, fit health, and boundary diagnostics | Curie + Fisher + Gauss | 🟢 Partial. `init_strategy = "single_trait_warmup"`, phi clamp, residual starts, simpler-model starts, restart history, protected/skipped `sdreport()` status, `gllvmTMBcontrol(se = FALSE)`, `fit_health`, and `check_gllvmTMB()` are implemented or in the current robust-modeling branch. The convergence/start-values article is drafted in the docs branch and teaches the no-SE plus bootstrap/profile workflow for hard fits. Covered/partial rows: MIS-16..MIS-20 and DIA-08..DIA-10. Remaining work is target-explicit empirical evidence, default-policy decisions, and family-specific stress lanes. |
