...
| **M2** | **Binary completeness** | 🟢 In progress | `█████░░░` 5/7 | Weeks 3–5. M2.1 design note + M2.2-A binary family recovery + M2.2-B CIs/extractors/glmmTMB cross-check + M2.3 `lambda_constraint` binary IRT + mirt + galamm cross-checks + M2.4 `suggest_lambda_constraint()` reliability shipped 2026-05-17. M2.8 (animal_* keyword family — adds 4th row to keyword grid) dispatched parallel slice. |
