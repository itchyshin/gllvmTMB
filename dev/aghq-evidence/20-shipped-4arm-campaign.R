...
LIB <- Sys.getenv("AGHQ_LIB", ""); if (nzchar(LIB)) .libPaths(c(LIB, .libPaths())); suppressMessages(library(gllvmTMB))
suppressWarnings(suppressMessages(library(parallel)))

OUT <- Sys.getenv("SHIP_OUT", "20-shipped4-inc.csv")
if (file.exists(OUT)) file.remove(OUT)
CORES <- as.integer(Sys.getenv("SHIP_CORES", "120"))
...
## FOUR ARMS, and the 4th is the point. `Laplace + ridge` was NOT reachable when the 3-arm
## version of this script was written -- the ridge was applied only on the AGHQ path, so the
## fair control could not be run and every gain was confounded with the penalty. Unbundled
## at 4dc351ed (opt-in: naming `aghq_ridge` is what arms it on the Laplace path; the default
## stays inert so no existing fit moves). The 2x2 is now complete:
##          k=1 (Laplace)          k=9 (AGHQ)
##   tau=Inf  laplace                aghq
##   tau=2    laplace_ridge          aghq_ridge  <- the SHIPPED configuration
## Attribution is only identifiable across the full 2x2: quadrature is (aghq - laplace),
## penalty is (laplace_ridge - laplace), and their interaction is what the reference
## implementation's table could not separate.
P <- 6L; Q <- 2L; LAM <- 1.0
jobs <- expand.grid(n = c(100L, 1600L), seed = 2001:2015,
                    arm = c("laplace", "laplace_ridge", "aghq", "aghq_ridge"),
...
for (n in sort(unique(res$n))) for (a in c("laplace", "laplace_ridge", "aghq", "aghq_ridge")) {
