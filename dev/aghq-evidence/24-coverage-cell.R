...
OUT   <- Sys.getenv("CV_OUT", "24-coverage-inc.csv")
...
P <- 6L; Q <- 2L; LAM <- 1.0
jobs <- expand.grid(n = c(100L, 200L, 400L, 1600L), arm = names(ARMS),
...
invisible(mclapply(seq_len(nrow(jobs)), function(i) {
  ...
  base <- data.frame(n=jb$n, arm=jb$arm, seed=jb$seed, stringsAsFactors=FALSE)
