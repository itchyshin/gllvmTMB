...
## the branch carries an uncommitted #946 offset fix the installed 0.6.0
...
suppressPackageStartupMessages({
  if (requireNamespace("devtools", quietly = TRUE) && !"gllvmTMB" %in% loadedNamespaces()) {
    devtools::load_all(".", quiet = TRUE)
  } else if (!"gllvmTMB" %in% loadedNamespaces()) {
    library(gllvmTMB)
  }
})
...
species_truth <- function(T_sp) {
  ...
  beta0 <- setNames(BETA0_D7[idx], sp_names)
...
#' @param phi spatial range of the shared kernel C(h) = exp(-h/phi), used for
#'   BOTH x and the bias fields g/h (same kernel, per D4); phi <= 1e-8 is i.i.d.
...
#' @param truth output of species_truth(T_sp); pass explicitly to avoid
#'   recomputing per call in a tight loop
#' @return long-format data.frame per the D9 schema, with design attributes
sim_phase_c <- function(seed, n = 400, T_sp = 8, d_true = 2, phi = 0.15,
                         kappa = 1, rho = 0.6, omega = 0.5, k = 3,
                         truth = species_truth(T_sp)) {
  ...
  set.seed(seed)
  x <- .gp_field(n, phi)                          # x field (first draw)
  ...
  for (j in seq_len(T_sp)) H[, j] <- .gp_field(n, phi)
  ...
  attr(df, "phi") <- phi; attr(df, "kappa") <- kappa
...
fit_arm <- function(df, arm = c("A1", "A2", "A3", "A4", "A5", "A6"),
  ...
  rhs <- switch(arm,
    ...
    A6 = "0 + trait + trait:x + trait:is_po + trait:bstar + offset(off)"
  )
...
#' @param arm "A1".."A6"
#' @param cfg one-row data.frame/list with kappa, rho, omega, phi, n, T_sp,
...
score_phase_c <- function(fit, arm, cfg, truth, elapsed_sec = NA_real_,
  ...
  base <- data.frame(
    block = cfg$block, kappa = cfg$kappa, rho = cfg$rho, omega = cfg$omega,
    phi = cfg$phi, n = cfg$n, T_sp = cfg$T_sp, d_fit = cfg$d_fit, k = cfg$k,
...
#' @param cfg one row of a configuration data.frame: kappa, rho, omega, phi,
#'   n, T_sp, d_fit, k, seed, block
#' @return rbind of 6 score_phase_c() rows (one per arm), never throws
run_dataset_c <- function(cfg, control = NULL) {
  truth <- species_truth(cfg$T_sp)
  df <- tryCatch(
    sim_phase_c(seed = cfg$seed, n = cfg$n, T_sp = cfg$T_sp, d_true = 2,
                phi = cfg$phi, kappa = cfg$kappa, rho = cfg$rho,
                omega = cfg$omega, k = cfg$k, truth = truth),
    error = function(e) e
  )
...
verify_bias_control <- function(seed = 1, n = 400, T_sp = 8, phi = 0.15, kappa = 1, k = 3) {
  ...
  for (rho in c(0, 0.6)) {
    for (omega in c(0, 0.5, 1)) {
      df <- sim_phase_c(seed = seed, n = n, T_sp = T_sp, phi = phi, kappa = kappa,
