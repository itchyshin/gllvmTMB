...
coordinate_ids <- function(par) {
  labels <- names(par)
  if (is.null(labels) || length(labels) != length(par) || anyNA(labels) ||
      any(!nzchar(labels))) {
    stop(
      "selected outer vector must have one non-empty block label per coordinate",
      call. = FALSE
    )
  }
...
if (identical(mode, "preflight")) {
  ...
  receipt <- list(schema = "G3_P1_SMOKE_PREFLIGHT_V1", packet = basename(packet_file),
    ...
    dll = dll_md5), n_rows = nrow(z$fixture$rows), n_mesh = ncol(z$mesh$A_st),
...
current_source_md5 <- c(
  ...
  dll = if (!is.null(loaded) && file.exists(loaded[["path"]])) hash_file(loaded[["path"]]) else NA_character_
)
if (!identical(receipt$source_md5, current_source_md5)) stop("estimator source or DLL identity drift", call. = FALSE)
...
if (inherits(fit, "error")) { ledger$status <- "FIT_ERROR"; ledger$error <- conditionMessage(fit) } else {
  saveRDS(fit, file.path(root, "fit.rds")); raw_par <- fit$opt$par
  ids <- coordinate_ids(raw_par); par <- stats::setNames(as.numeric(raw_par), ids)
  health <- fit$fit_health %||% list(); gradient <- stats::setNames(as.numeric(fit$tmb_obj$gr(raw_par)), ids)
  raw_objective <- fit$tmb_obj$fn(raw_par)
  tolerance <- 64 * .Machine$double.eps * max(1, abs(fit$objective$likelihood_nll))
  if (!is.finite(raw_objective) || abs(raw_objective - fit$objective$likelihood_nll) > tolerance) stop("selected likelihood_nll does not match the G3 objective", call. = FALSE)
  signature$objective <- hash_object(list(value = raw_objective, dll = current_source_md5[["dll"]]))
  signature$gradient <- hash_object(gradient)
  signature$parameter_order <- hash_object(list(block_labels = names(raw_par), coordinate_ids = ids)); signature$map <- hash_object(fit$tmb_map); signature$data <- hash_object(fit$tmb_data)
  ledger$signature <- signature; ledger$raw <- list(objective = raw_objective, gradient = gradient, parameter_vector = par,
  ...
  ledger$curvature_provenance <- list(metric_source = "sdreport_cov_fixed",
    positional_order_md5 = hash_object(list(block_labels = names(raw_par), coordinate_ids = ids)),
    raw_covariance_md5 = hash_object(ledger$g3$raw$covariance), requested_candidate_indices = requested,
    requested_candidate_covariance_md5 = stats::setNames(vapply(requested, function(i) hash_object(ledger$g3$trials[[i]]$covariance), character(1L)), as.character(requested)))
  ledger$status <- ledger$g3$status; ledger$selected <- ledger$g3$selected
}
ledger$terminal <- TRUE
}, error = function(e) {
  ledger$status <<- if (grepl("receipt drift|fixture/mesh rebuild|likelihood_nll|identity drift|clean committed", conditionMessage(e))) "INVALID_PROVENANCE" else "RUNNER_ERROR"
  ledger$error <<- conditionMessage(e); ledger$terminal <<- TRUE
})
cat(ledger$status, "\n")
}
main()
