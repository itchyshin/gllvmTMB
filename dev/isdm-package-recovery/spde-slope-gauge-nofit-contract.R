...
.spde_slope_gauge_nofit_state_ok <- function(state, schema) {
  ...
  is.list(state) &&
    ...
    identical(state$schema, schema) &&
    is.double(state$objective) &&
    length(state$objective) == 1L &&
    is.finite(state$objective) &&
    is.double(state$theta) &&
    identical(names(state$theta), raw_order) &&
    length(state$theta) == length(raw_order) &&
    all(is.finite(state$theta)) &&
    is.double(state$gradient) &&
    identical(names(state$gradient), raw_order) &&
    length(state$gradient) == length(raw_order) &&
    all(is.finite(state$gradient)) &&
    is.integer(state$convergence) &&
    length(state$convergence) == 1L &&
    is.list(state$covariance) &&
    is.list(state$start_provenance) &&
    is.data.frame(state$restart_history) &&
    is.list(state$warm_restart_provenance) &&
    is.list(state$isdm_polish_provenance) &&
    is.list(state$parameters) &&
    is.list(state$map) &&
    is.list(state$data) &&
    is.character(state$random) &&
    is.character(state$block_labels) &&
    length(state$block_labels) == length(raw_order) &&
...
spde_slope_gauge_nofit_validate_predecessor_bytes <- function(
  ...
  expected_root <- tryCatch(
    normalizePath(locked$root, mustWork = TRUE),
    error = function(e) NA_character_
  )
  if (
    !is.character(normal_root) ||
      length(normal_root) != 1L ||
      is.na(normal_root) ||
      is.na(expected_root) ||
  ...
  bytes_ok <- identical(
    sort(inventory),
    sort(c(expected_files, expected_dirs))
  ) &&
    all(vapply(paths, .spde_slope_gauge_nofit_regular_file, logical(1L))) &&
    all(vapply(
      directory_paths,
      function(path) {
        isTRUE(file.info(path)$isdir[[1L]]) &&
          identical(Sys.readlink(path), "") &&
          identical(
  ...
  state <- tryCatch(
    readRDS(file.path(normal_root, "v2-materialized-state.rds")),
    error = function(e) NULL
  )
  receipt_ok <- is.list(receipt) &&
    identical(
      names(receipt),
      c(
        "schema",
    ...
    identical(receipt$root, normal_root) &&
    identical(receipt$commit, locked$commit) &&
    ...
    all(vapply(
      receipt[c("runner_md5", "contract_md5", "design_md5")],
      .spde_slope_gauge_nofit_md5,
      logical(1L)
    ))
  state_ok <- .spde_slope_gauge_nofit_state_ok(state, locked$state_schema)
  .spde_slope_gauge_nofit_verdict(
    receipt_ok && state_ok,
    if (receipt_ok && state_ok) {
      "predecessor_bytes_valid"
    } else {
...
spde_slope_gauge_nofit_v2_validate_v1_forensic <- function(
  ...
  receipt_root <- if (
    is.list(receipt) && .spde_slope_gauge_nofit_scalar_character(receipt$root)
  ) {
    tryCatch(normalizePath(receipt$root, mustWork = TRUE), error = function(e) {
      NA_character_
    })
  } else {
    NA_character_
  ...
  receipt_ok <- .spde_slope_gauge_nofit_exact_names(receipt, fields) &&
    identical(receipt$schema, locked$receipt_schema) &&
    identical(receipt$gate, locked$gate) &&
    identical(receipt_root, normal_root) &&
    identical(receipt$commit, locked$commit) &&
    identical(receipt$status, locked$status) &&
    identical(receipt$reason, locked$reason) &&
  ...
  .spde_slope_gauge_nofit_verdict(
    ...
    terminal_reason = locked$reason
  )
}

.spde_slope_gauge_nofit_v2_v3_projection_ok <- function(
  predecessor,
  v3_verdict
) {
...
.spde_slope_gauge_nofit_v2_child_ok <- function(
  child,
  v1_verdict,
  v3_verdict,
  expected_dll
  ...
  if (
    !.spde_slope_gauge_nofit_exact_names(child, fields) ||
      !identical(
        child$schema,
        "PAPER1_SPDE_SLOPE_GAUGE_NOFIT_GATE_V2_CHILD_V1"
      ) ||
      !is.integer(child$parent_pid) ||
      length(child$parent_pid) != 1L ||
      child$parent_pid < 1L ||
      !is.integer(child$child_pid) ||
      length(child$child_pid) != 1L ||
      child$child_pid < 1L ||
      identical(child$parent_pid, child$child_pid) ||
      !.spde_slope_gauge_nofit_scalar_double(child$deadline_s) ||
      child$deadline_s != 1800 ||
      !.spde_slope_gauge_nofit_scalar_double(child$elapsed_s) ||
      child$elapsed_s < 0 ||
      !.spde_slope_gauge_nofit_scalar_character(child$status) ||
      !.spde_slope_gauge_nofit_scalar_character(child$reason) ||
      !.spde_slope_gauge_nofit_scalar_character(child$stage) ||
      !.spde_slope_gauge_nofit_scalar_character(child$started_at) ||
      !.spde_slope_gauge_nofit_scalar_character(child$ended_at) ||
      !is.list(child$object) ||
      !is.integer(child$object$created) ||
      !is.integer(child$object$released) ||
      length(child$object$created) != 1L ||
      length(child$object$released) != 1L ||
      child$object$created < 0L ||
      child$object$released < 0L ||
      child$object$created < child$object$released
  ) {
    return(FALSE)
  }
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      ((identical(child$reason, "v1_forensic_invalid") &&
        identical(child$stage, "v1_forensic")) ||
        (identical(child$reason, "predecessor_bytes_invalid") &&
          identical(child$stage, "predecessor_bytes"))) &&
      is.null(child$predecessor) &&
      is.null(child$dll) &&
      identical(child$object, list(created = 0L, released = 0L)) &&
      is.null(child$nofit) &&
      is.null(child$callback_audit) &&
      .spde_slope_gauge_nofit_scalar_character(child$error)
  ) {
    return(TRUE)
  }
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      identical(child$reason, "time_limit_exceeded") &&
      child$stage %in% c("v1_forensic", "predecessor_bytes", "dll") &&
      is.null(child$predecessor) &&
      is.null(child$dll) &&
      identical(child$object, list(created = 0L, released = 0L)) &&
      is.null(child$nofit) &&
  ...
  partial_predecessor_ok <- isTRUE(v1_verdict$valid) &&
    identical(v1_verdict$reason, "v1_forensic_terminal_valid") &&
    .spde_slope_gauge_nofit_exact_names(
      child$predecessor,
      predecessor_fields
    ) &&
    .spde_slope_gauge_nofit_v2_v3_projection_ok(
      child$predecessor[c("root", "commit", "receipt", "state_md5")],
      v3_verdict
    ) &&
    identical(
      child$predecessor$v1_forensic,
      v1_verdict[c(
        "root",
        "commit",
        "receipt",
  ...
  partial_common <- partial_predecessor_ok &&
    is.null(child$nofit) &&
    is.null(child$callback_audit) &&
    .spde_slope_gauge_nofit_scalar_character(child$error)
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      partial_common &&
      identical(child$reason, "dll_identity_failure") &&
      identical(child$stage, "dll") &&
      is.null(child$dll) &&
      identical(child$object, list(created = 0L, released = 0L))
  ) {
    return(TRUE)
  ...
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      partial_common &&
      identical(child$reason, "historical_v3_replay_failure") &&
      identical(child$stage, "historical") &&
      identical(child$dll, expected_dll) &&
      identical(child$object, list(created = 0L, released = 0L)) &&
      is.character(child$predecessor$historical_reason) &&
      length(child$predecessor$historical_reason) == 1L &&
      is.na(child$predecessor$historical_reason) &&
      identical(child$predecessor$post_replay_gc, FALSE)
  ) {
    return(TRUE)
  }
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      partial_common &&
      identical(child$reason, "fresh_object_unavailable") &&
      identical(child$stage, "factory") &&
      identical(child$dll, expected_dll) &&
      identical(child$object, list(created = 0L, released = 0L)) &&
      identical(child$predecessor$historical_reason, "closeout_recomputed") &&
      identical(child$predecessor$post_replay_gc, TRUE)
  ) {
    return(TRUE)
  }
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      partial_common &&
      ((identical(child$reason, "callback_or_finite_difference_failure") &&
        identical(child$stage, "callback")) ||
        (identical(child$reason, "callback_audit_invalid") &&
          identical(child$stage, "audit"))) &&
      identical(child$dll, expected_dll) &&
      identical(child$object, list(created = 1L, released = 1L)) &&
      identical(child$predecessor$historical_reason, "closeout_recomputed") &&
      identical(child$predecessor$post_replay_gc, TRUE)
  ) {
    return(TRUE)
  ...
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      partial_common &&
      identical(child$reason, "object_release_failure") &&
      identical(child$stage, "release") &&
      identical(child$dll, expected_dll) &&
      identical(child$object, list(created = 1L, released = 0L)) &&
      identical(child$predecessor$historical_reason, "closeout_recomputed") &&
      identical(child$predecessor$post_replay_gc, TRUE)
  ) {
    return(TRUE)
  ...
  if (
    identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD") &&
      ...
      ((identical(child$stage, "factory") &&
        identical(child$object, list(created = 0L, released = 0L))) ||
        (child$stage %in%
          c("callback", "audit", "release") &&
          identical(child$object, list(created = 1L, released = 1L))))
  ) {
    return(TRUE)
  }
  if (identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD")) {
    return(FALSE)
  }
  if (
    !identical(child$stage, "complete") ||
      !isTRUE(v1_verdict$valid) ||
      !identical(v1_verdict$reason, "v1_forensic_terminal_valid") ||
      !.spde_slope_gauge_nofit_exact_names(
        child$predecessor,
        predecessor_fields
  ...
  legacy$stage <- NULL
  legacy$predecessor <- child$predecessor[c(
    "root",
    "commit",
    "state_md5",
    "historical_reason",
  ...
  .spde_slope_gauge_nofit_child_ok(
    legacy,
    v3_verdict,
    expected_dll,
    state = v3_verdict$state
  )
}
...
spde_slope_gauge_nofit_wrap_object_callbacks <- function(
  ...
  if (
    !.spde_slope_gauge_nofit_state_ok(state, locked$state_schema) ||
      !is.list(object) ||
      !is.function(object$fn) ||
      !is.function(object$gr) ||
      !is.numeric(object$par) ||
      !identical(names(object$par), state$block_labels) ||
      length(object$par) != length(state$theta) ||
      !is.integer(object_id) ||
      length(object_id) != 1L ||
      is.na(object_id) ||
      object_id < 1L ||
      !is.character(dll_path) ||
      length(dll_path) != 1L ||
      is.na(dll_path) ||
      !file.exists(dll_path) ||
  ...
  require_theta <- function(theta) {
    if (
      !is.numeric(theta) ||
        length(theta) != length(raw_order) ||
        any(!is.finite(theta))
    ) {
      .spde_slope_gauge_fail(
        "fresh object callback received an invalid raw vector"
      )
    }
  ...
  objective_fn <- function(theta) {
    ...
    value <- .spde_slope_gauge_scalar_double(
      object$fn(input),
      "fresh object objective"
    )
    audit$objective[[length(audit$objective) + 1L]] <- list(
      input = stats::setNames(input, raw_order),
      value = value
    )
    value
  }
  gradient_fn <- function(theta) {
    ...
    if (
      !is.numeric(gradient) ||
        length(gradient) != length(raw_order) ||
        any(!is.finite(gradient))
    ) {
      .spde_slope_gauge_fail(
        "fresh object gradient is not finite positional evidence"
      )
    }
    ...
    if (is.null(names(gradient))) {
      ...
      .spde_slope_gauge_fail(
        "fresh object gradient supplied a noncanonical positional order"
      )
    } else {
      gradient <- stats::setNames(as.double(gradient), raw_order)
    }
...
.spde_slope_gauge_nofit_gate_manifest_ok <- function(root, files) {
  ...
  is.data.frame(manifest) &&
    ...
    identical(
      ...
      unname(tools::md5sum(file.path(root, declared)))
    )
}
...
spde_slope_gauge_nofit_audit_ok <- function(audit, nofit) {
  ...
  if (
    !is.list(audit) ||
      ...
      !is.list(records) ||
      length(records) != 22L ||
      !is.integer(audit$object_id) ||
      length(audit$object_id) != 1L ||
      audit$object_id < 1L ||
      !.spde_slope_gauge_nofit_scalar_character(audit$dll_path) ||
      !.spde_slope_gauge_nofit_md5(audit$dll_md5) ||
      !is.list(audit$objective) ||
      length(audit$objective) != 45L ||
      !is.list(audit$gradient) ||
      length(audit$gradient) != 1L ||
      !is.double(nofit$raw_theta) ||
      !identical(names(nofit$raw_theta), raw_order) ||
      !is.double(nofit$raw_gradient) ||
      !identical(names(nofit$raw_gradient), raw_order) ||
      !all(vapply(
        audit$objective,
        function(record) {
          is.list(record) &&
            identical(names(record), c("input", "value")) &&
            is.double(record$input) &&
            identical(names(record$input), raw_order) &&
            is.double(record$value) &&
            length(record$value) == 1L &&
      ...
      !identical(
        audit$gradient[[1L]]$raw_gradient,
        unname(nofit$raw_gradient)
      ) ||
      !(is.null(audit$gradient[[1L]]$supplied_names) ||
        identical(audit$gradient[[1L]]$supplied_names, raw_order)) ||
      !identical(audit$gradient[[1L]]$named_gradient, nofit$raw_gradient)
  ) {
    return(FALSE)
  }
  expected <- unlist(
...
.spde_slope_gauge_nofit_complete_result_ok <- function(nofit, state) {
  ...
  if (
    !.spde_slope_gauge_nofit_exact_names(nofit, fields) ||
      !is.logical(nofit$valid) ||
      length(nofit$valid) != 1L ||
      is.na(nofit$valid) ||
      !.spde_slope_gauge_nofit_scalar_character(nofit$reason) ||
      !is.double(nofit$phi) ||
      !identical(names(nofit$phi), phi_order) ||
      !is.double(nofit$raw_theta) ||
      !identical(names(nofit$raw_theta), raw_order) ||
      !is.double(nofit$objective) ||
      length(nofit$objective) != 1L ||
      !is.finite(nofit$objective) ||
      !is.double(nofit$raw_gradient) ||
      !identical(names(nofit$raw_gradient), raw_order) ||
      !is.double(nofit$transformed_gradient) ||
      !identical(names(nofit$transformed_gradient), phi_order) ||
      ...
      !all(vapply(
        nofit$errors,
        .spde_slope_gauge_nofit_scalar_double,
        logical(1L)
      )) ||
      !is.list(nofit$finite_difference) ||
      length(nofit$finite_difference) != 22L ||
      any(
        !is.finite(c(
          nofit$phi,
          nofit$raw_theta,
          nofit$raw_gradient,
          nofit$transformed_gradient,
          nofit$transformed_gradient_fd
        ))
      )
  ) {
    return(FALSE)
  }
  records_ok <- all(vapply(
    seq_along(nofit$finite_difference),
    function(j) {
      ...
      is.list(record) &&
        ...
        identical(record$index, as.integer(j)) &&
        identical(record$phi_id, phi_order[[j]]) &&
        is.double(record$h) &&
        length(record$h) == 1L &&
        is.finite(record$h) &&
        is.double(record$phi_plus) &&
        identical(names(record$phi_plus), phi_order) &&
        is.double(record$phi_minus) &&
        identical(names(record$phi_minus), phi_order) &&
        is.double(record$theta_plus) &&
        identical(names(record$theta_plus), raw_order) &&
        is.double(record$theta_minus) &&
        identical(names(record$theta_minus), raw_order) &&
        is.double(record$objective_plus) &&
        length(record$objective_plus) == 1L &&
        is.double(record$objective_minus) &&
        length(record$objective_minus) == 1L &&
        is.finite(record$objective_plus) &&
        is.finite(record$objective_minus)
    },
    logical(1L)
  ))
  if (!records_ok) {
  ...
  recomputed <- tryCatch(
    {
      ...
      transformed <- spde_slope_gauge_full_chain_gradient(
        phi,
        nofit$raw_gradient
      )
      step <- .Machine$double.eps^(1 / 3) * pmax(1, abs(phi))
      fd <- vapply(
        nofit$finite_difference,
        function(record) {
      ...
      errors <- c(
        ...
        objective = abs(nofit$objective - state$objective) /
          max(1, abs(nofit$objective), abs(state$objective)),
        gradient = .spde_slope_gauge_relative_error(
          nofit$raw_gradient,
          state$gradient
        ),
  ...
  if (
    is.null(recomputed) ||
      !identical(nofit$phi, recomputed$phi) ||
      ...
      !identical(unname(nofit$errors), unname(as.list(recomputed$errors)))
  ) {
    return(FALSE)
  }
  for (j in seq_along(nofit$finite_difference)) {
    record <- nofit$finite_difference[[j]]
    delta <- rep(0, 22L)
    names(delta) <- phi_order
    delta[[j]] <- recomputed$step[[j]]
    if (
      !identical(record$h, as.double(recomputed$step[[j]])) ||
        !identical(record$phi_plus, recomputed$phi + delta) ||
        !identical(record$phi_minus, recomputed$phi - delta) ||
        !identical(
          record$theta_plus,
          spde_slope_gauge_theta_from_phi(recomputed$phi + delta)
        ) ||
        !identical(
          record$theta_minus,
          spde_slope_gauge_theta_from_phi(recomputed$phi - delta)
  ...
  identical(nofit$valid, expected_valid) &&
    identical(
      nofit$reason,
...
.spde_slope_gauge_nofit_child_ok <- function(
  child,
  predecessor,
  expected_dll,
  state = NULL
  ...
  if (
    !.spde_slope_gauge_nofit_exact_names(child, fields) ||
      !identical(
        child$schema,
        "PAPER1_SPDE_SLOPE_GAUGE_NOFIT_GATE_V1_CHILD_V1"
      ) ||
      !is.integer(child$parent_pid) ||
      length(child$parent_pid) != 1L ||
      child$parent_pid < 1L ||
  ...
  if (child$status %in% complete_status) {
    return(
      is.list(child$predecessor) &&
        identical(child$predecessor$root, predecessor$root) &&
        identical(child$predecessor$commit, predecessor$commit) &&
        identical(child$predecessor$state_md5, predecessor$state_md5) &&
        identical(child$predecessor$historical_reason, "closeout_recomputed") &&
        isTRUE(child$predecessor$post_replay_gc) &&
        is.list(child$dll) &&
        identical(child$dll$path, expected_dll$path) &&
        identical(child$dll$md5, expected_dll$md5) &&
        identical(child$object$created, 1L) &&
        identical(child$object$released, 1L) &&
        is.list(child$nofit) &&
        is.list(child$callback_audit) &&
        is.character(child$error) &&
        length(child$error) == 1L &&
        is.na(child$error) &&
        is.list(state) &&
        .spde_slope_gauge_nofit_complete_result_ok(child$nofit, state) &&
        spde_slope_gauge_nofit_audit_ok(child$callback_audit, child$nofit) &&
        if (identical(child$status, "SPDE_SLOPE_GAUGE_NOFIT_VALID")) {
          isTRUE(child$nofit$valid)
        } else {
...
.spde_slope_gauge_nofit_process_ok <- function(
  ...
  if (
    !.spde_slope_gauge_nofit_exact_names(process, fields) ||
      ...
      !identical(process$command, command) ||
      !identical(process$arguments, arguments) ||
      !is.integer(process$parent_pid) ||
      length(process$parent_pid) != 1L ||
      process$parent_pid < 1L ||
      !.spde_slope_gauge_nofit_scalar_character(process$started_at) ||
      !.spde_slope_gauge_nofit_scalar_character(process$ended_at) ||
      !.spde_slope_gauge_nofit_scalar_double(process$elapsed_s) ||
      process$elapsed_s < 0 ||
      !.spde_slope_gauge_nofit_scalar_double(process$deadline_s) ||
      process$deadline_s != 1800 ||
      !is.logical(process$timed_out) ||
      length(process$timed_out) != 1L ||
      is.na(process$timed_out) ||
      !.spde_slope_gauge_nofit_md5(process$stdout_md5) ||
      !.spde_slope_gauge_nofit_md5(process$stderr_md5) ||
  ...
  if (is.null(child)) {
    return(
      is.integer(process$child_pid) &&
        length(process$child_pid) == 1L &&
        is.na(process$child_pid) &&
        is.character(process$child_result_md5) &&
        length(process$child_result_md5) == 1L &&
        is.na(process$child_result_md5) &&
        .spde_slope_gauge_nofit_scalar_character(
          process$signal,
          allow_na = TRUE
        )
    )
  ...
  if (!isTRUE(child_evidence_valid)) {
    return(
      !isTRUE(process$timed_out) &&
        identical(process$exit_status, 0L) &&
        is.na(process$signal) &&
        ((is.integer(process$child_pid) &&
          length(process$child_pid) == 1L &&
          is.na(process$child_pid)) ||
          (is.integer(process$child_pid) &&
            length(process$child_pid) == 1L &&
            process$child_pid > 0L &&
            !identical(process$child_pid, process$parent_pid))) &&
        .spde_slope_gauge_nofit_md5(process$child_result_md5)
    )
  }
  !isTRUE(process$timed_out) &&
    identical(process$parent_pid, child$parent_pid) &&
    identical(process$child_pid, child$child_pid) &&
    !identical(process$child_pid, process$parent_pid) &&
    identical(process$exit_status, 0L) &&
    is.na(process$signal) &&
    .spde_slope_gauge_nofit_md5(process$child_result_md5)
...
spde_slope_gauge_nofit_validate_gate_root <- function(
  root,
  source_paths,
  predecessor = spde_slope_gauge_nofit_locked_predecessor(),
  commit = NULL
) {
  normal_root <- tryCatch(
    normalizePath(root, mustWork = TRUE),
  ...
  if (!packet_ok) {
    return(.spde_slope_gauge_nofit_verdict(FALSE, "gate_packet_bytes_invalid"))
  }
  root_receipt <- tryCatch(
    readRDS(file.path(normal_root, "root-receipt.rds")),
    error = function(e) NULL
  )
  process <- tryCatch(
  ...
  if (is.null(commit)) {
    ...
    commit <- if (is.character(commit) && length(commit) == 1L) {
      commit
    } else {
      NA_character_
    }
  }
  predecessor_verdict <- spde_slope_gauge_nofit_validate_predecessor_bytes(
    predecessor$root,
    predecessor
  )
  receipt_ok <- .spde_slope_gauge_nofit_exact_names(
    root_receipt,
    receipt_fields
  ) &&
    ...
    identical(root_receipt$root, normal_root) &&
    identical(root_receipt$commit, commit) &&
    .spde_slope_gauge_nofit_scalar_character(root_receipt$status) &&
    .spde_slope_gauge_nofit_scalar_character(root_receipt$reason) &&
    identical(
      root_receipt$predecessor,
      predecessor_verdict[c("receipt", "state_md5")]
    ) &&
    identical(root_receipt$sources, source_hashes) &&
    .spde_slope_gauge_nofit_exact_names(
      root_receipt$dll,
      c("path", "md5")
    ) &&
    ...
    identical(
      root_receipt$parent_stage$schema,
    ...
    grepl(
      "^\\.PAPER1_SPDE_SLOPE_GAUGE_NOFIT_GATE_V1-",
      basename(root_receipt$parent_stage$stage)
    ) &&
    identical(
      root_receipt$parent_stage$child_output,
    ...
    identical(root_receipt$process, process) &&
    .spde_slope_gauge_nofit_md5(root_receipt$time_estimate_md5) &&
    identical(
      root_receipt$time_estimate_md5,
      unname(tools::md5sum(file.path(normal_root, "time-estimate.md")))[[1L]]
    ) &&
    if (has_result) {
      identical(
        root_receipt$child_result_md5,
        unname(tools::md5sum(file.path(normal_root, "no-fit-result.rds")))[[1L]]
      )
    } else {
      is.na(root_receipt$child_result_md5)
    }
  child_declared_dll <- if (
    has_result && is.list(child) && is.list(child$dll)
  ) {
    child$dll
  } else {
    list(path = NA_character_, md5 = NA_character_)
  }
  raw_child_ok <- has_result &&
    isTRUE(tryCatch(
      .spde_slope_gauge_nofit_child_ok(
        child,
        predecessor_verdict,
        child_declared_dll,
        state = predecessor_verdict$state
      ),
      error = function(e) FALSE
    ))
  evidence_hold <- has_result &&
    identical(
      root_receipt$status,
      "SPDE_SLOPE_GAUGE_NOFIT_INFRASTRUCTURE_HOLD"
    ) &&
    identical(root_receipt$reason, "child_evidence_invalid") &&
    !raw_child_ok
  expected_dll <- root_receipt$dll
  child_ok <- !has_result || evidence_hold || raw_child_ok
  complete_child <- has_result &&
    !evidence_hold &&
    is.list(child) &&
    .spde_slope_gauge_nofit_scalar_character(child$status) &&
    child$status %in%
  ...
  dll_missing <- is.character(root_receipt$dll$path) &&
    length(root_receipt$dll$path) == 1L &&
    is.na(root_receipt$dll$path) &&
    is.character(root_receipt$dll$md5) &&
    length(root_receipt$dll$md5) == 1L &&
    is.na(root_receipt$dll$md5)
  dll_live <- .spde_slope_gauge_nofit_scalar_character(root_receipt$dll$path) &&
    ...
    identical(
      unname(tools::md5sum(root_receipt$dll$path))[[1L]],
      root_receipt$dll$md5
    )
  ...
  process_ok <- .spde_slope_gauge_nofit_process_ok(
    process,
    child,
    expected_command,
    expected_arguments,
    child_evidence_valid = !evidence_hold
  ) &&
    (!has_result ||
      identical(process$child_result_md5, root_receipt$child_result_md5))
  status_ok <- if (!has_result) {
    ...
    identical(root_receipt$status, child$status) &&
  ...
  valid <- packet_ok &&
    isTRUE(predecessor_verdict$valid) &&
    receipt_ok &&
    dll_ok &&
    child_ok &&
    process_ok &&
  ...
  .spde_slope_gauge_nofit_verdict(
    valid,
    if (valid) {
      "nofit_gate_root_valid"
    } else {
      "nofit_gate_evidence_invalid"
    },
    ...
    checks = c(
      packet = packet_ok,
      predecessor = isTRUE(predecessor_verdict$valid),
      receipt = receipt_ok,
      dll = dll_ok,
      child = child_ok,
      process = process_ok,
