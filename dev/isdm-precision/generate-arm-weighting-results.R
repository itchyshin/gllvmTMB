## ---------------------------------------------------------------------------
## Replicate-level results behind Block 7 (READER-BLOCKS.md): the midpoint
## objection, tested across three arm-size designs. Reuses the exact
## simulation and estimator in evidence-arm-weighting.R -- this script only
## adds saveRDS() so the article can plot the full replicate scatter instead
## of the printed aggregates.
##
## Run from the package root:
##   Rscript dev/isdm-precision/generate-arm-weighting-results.R
## ---------------------------------------------------------------------------
...
read_env <- function(gr, lo, la) {
  gr$env[max.col(-(outer(lo, gr$lon, "-")^2 + outer(la, gr$lat, "-")^2))]
}
...
sim_once <- function(fuzz, n_po, n_srv, beta = 0.9, ell = 0.25, seed = 1) {
  ...
  fit1 <- function(dat) {
    ...
    rhs <- if (length(unique(dat$src)) > 1) {
      dat$src <- factor(dat$src); value ~ 0 + trait + trait:env + src
    } else value ~ 0 + trait + trait:env
...
designs <- list(c(220, 220), c(400, 100), c(100, 400))
out <- list()
for (design in designs) {
  for (f in c(0, 0.5, 1.0)) {
    for (r in 1:15) {
      est <- sim_once(f, design[1], design[2], seed = 2000 + r)
      out[[length(out) + 1]] <- data.frame(
        n_po = design[1], n_survey = design[2], fuzz = f, rep = r,
        arm = names(est), beta_hat = unname(est), row.names = NULL)
    }
  }
}
res <- do.call(rbind, out)
saveRDS(res, "dev/isdm-precision/arm-weighting-results.rds")
cat("saved", nrow(res), "rows to dev/isdm-precision/arm-weighting-results.rds\n")
