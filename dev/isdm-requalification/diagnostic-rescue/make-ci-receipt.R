...
raw <- system2("gh", c("run", "view", format(run_id, scientific = FALSE),
                        "--json", paste(c("conclusion", "event", "headSha", "url",
                                          "workflowName", "jobs"), collapse = ",")),
...
if (!identical(run$workflowName, "R-CMD-check") ||
  ...
  stop("CI run is not the manual R-CMD-check workflow")
...
saveRDS(list(schema = "isdm-diagnostic-ci-receipt-v1",
             ...
             workflow_name = run$workflowName, event = run$event,
