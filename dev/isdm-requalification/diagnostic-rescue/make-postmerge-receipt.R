...
if (system2("git", c("merge-base", "--is-ancestor", ci$head_sha, "origin/main")) != 0L)
  stop("feature head is not on origin/main")
...
raw <- system2("gh", c("pr", "view", as.character(pr), "--json",
                        "state,mergedAt,mergeCommit,url"), stdout = TRUE)
...
saveRDS(list(schema = "isdm-diagnostic-postmerge-receipt-v1",
             ...
             pr_url = info$url, ci = ci, package_code_unchanged = TRUE,
             lease_released = TRUE,
