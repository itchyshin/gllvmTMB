...
isdm_respinfo_classify <- function(paired, B = ISDM_RESPINFO_BOOTSTRAP_B, seed = ISDM_RESPINFO_BOOTSTRAP_SEED) {
  ...
  complete <- vapply(groups, function(x) all(is.finite(as.matrix(x[c("shared_D", "full_D", "psi1_D", "psi2_D", "psi3_D")] ))), logical(1L))
  if (any(vapply(groups, nrow, integer(1L)) != ISDM_RESPINFO_N_SEEDS) || any(!complete)) {
    return(list(classification = "EVIDENCE_INCOMPLETE", cell_pass = NULL, n_scoreable = vapply(groups, nrow, integer(1L))))
  }
  ...
  list(classification = classification, cell_pass = pass, target_count = target_count, n_scoreable = vapply(groups, nrow, integer(1L)))
}
