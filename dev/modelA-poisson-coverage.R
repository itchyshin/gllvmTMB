#!/usr/bin/env Rscript
## ADEMP coverage campaign — Poisson Model A, B_lv = Lambda_B alpha^T.
## Extends the certified Gaussian rank-1/rank-2 Model A (LV-09, dev/modelA-rank2-coverage.R)
## to the Poisson S2 cell (family_id 2, admitted onto `latent(..., lv = ~ x)` on 2026-07-16;
## see the LV-05 note in R/lv-predictor.R).
## Corrected vs the Gaussian harness for the Poisson case:
##   - REML = FALSE always (REML is Gaussian-only; R/lv-predictor.R fail-aborts REML=TRUE
...
##   - Wald natural-scale delta-SE recorded as the interior fallback; the shared `species`
##     grouping between latent(lv=~x) and phylo_latent can leave pdHess=FALSE (the same
##     "route via profile" case as Gaussian LV-09), so convergence is the gate, NOT pdHess --
##     Wald simply reports NA when the sdreport Hessian is not PD (extract_lv_effects()
##     already encodes this via uncertainty_status).
## One seed per SLURM array task (Totoro/DRAC). Smoke locally, scale on Totoro (>=500/cell).
## Usage: Rscript modelA-poisson-coverage.R run <cell_id> <task_id> <reps>
##        Rscript modelA-poisson-coverage.R summarise <results_dir>
suppressMessages({ library(gllvmTMB); library(ape) })

CELLS <- list(
  `pois-S120-K1-smoke` = list(S = 120L, T = 5L, K_B = 1L, K_phy = 1L, lambda = 0.4)
)
...
make_truth <- function(cell, design_seed = 20260716L) {
  ...
  LambdaB <- matrix(stats::runif(T * K_B, -0.6, 0.6), T, K_B)
  diag(LambdaB) <- abs(diag(LambdaB)) + 0.3
  alpha <- matrix(stats::runif(K_B, 0.4, 0.7), 1L, K_B)
  LambdaPhy <- matrix(stats::runif(T * cell$K_phy, -0.6, 0.6), T, cell$K_phy) * sqrt(cell$lambda)
  beta <- rep(1.0, T) + stats::rnorm(T, 0, 0.1)  ## baseline ~1.0 -> mean count ~3-4
...
one_rep <- function(tr, seed) {
  ...
  y <- matrix(stats::rpois(S * T, lambda = exp(eta)), S, T)
  ...
  fit <- tryCatch(suppressMessages(suppressWarnings(gllvmTMB(
    ...
    data = df, unit = "species", trait = "trait", family = stats::poisson(), REML = FALSE,
  ...
  truth <- tr$B_lv[1, 1]
  base <- data.frame(seed = seed, truth = truth, converged = FALSE, pdHess = NA,
  ...
  ## Gate on convergence only -- NOT pdHess. The shared-`species` grouping between
  ## latent(lv=~x) and phylo_latent can leave pdHess=FALSE even on a converged fit
  ## (documented Gaussian LV-09 behaviour); profile CIs do not need pdHess, so gating
  ## the whole row on it would silently empty this harness. Wald reports NA on its own
  ## when the sdreport Hessian is not PD.
...
if (identical(mode, "run")) {
  ...
  outdir <- file.path("results", "modelA-poisson-coverage", cell_id)
  ...
  cat(sprintf("[%s task %d] %d reps: converged=%d pdHess=%d prof_ci=%d\n", cell_id, task_id, nrow(rows),
    sum(rows$converged), sum(rows$pdHess, na.rm = TRUE), sum(!is.na(rows$prof_cov))))
