...
run_one <- function(N, seed, nitt = NITT) {
  ...
  c(N = N, seed = seed, rho_gllvm = rho_gllvm, rho_mcmc = rho_mcmc,
    railed_gllvm = as.numeric(is.finite(rho_gllvm) && abs(rho_gllvm) > 0.99))
}

## ---- run the ladder -------------------------------------------------------
grid <- expand.grid(N = N_LADDER, seed = seq_len(N_SEED) + 84L)
cat(sprintf("RECOVERY N=%s x %d seeds, nitt=%d, cores=%d, rho_true=%.2f\n",
            paste(N_LADDER, collapse = "/"), N_SEED, NITT, N_CORES, RHO_TRUE))
res <- parallel::mclapply(seq_len(nrow(grid)),
                          function(i) run_one(grid$N[i], grid$seed[i]),
                          mc.cores = N_CORES)
res <- as.data.frame(do.call(rbind, res))
saveRDS(res, file.path(PKG_DIR, "dev", sprintf("recovery-raw-%s.rds", paste(N_LADDER, collapse = "_"))))
