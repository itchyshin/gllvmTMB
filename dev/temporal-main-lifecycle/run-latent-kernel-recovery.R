## Retained direct-DGP recovery for replicated AR1 temporal_latent + kernel_indep.
...
simulate_fixture <- function(phi, seed) {
  set.seed(seed)
  static <- t(chol(K)) %*% sweep(matrix(rnorm(n_series * 3L), n_series, 3L),
    2L, truth$kernel_sd, "*")
...
plan <- expand.grid(phi = c(-.4, 0, .6), seed = 2609231:2609233)
plan <- plan[order(plan$phi, plan$seed), , drop = FALSE]
if (smoke) plan <- data.frame(phi = .6, seed = 2609231L)
...
if (smoke) {
  ...
  cat("TEMPORAL_LATENT_KERNEL_RECOVERY_SMOKE_PASS\n")
...
utils::write.csv(result, file.path(root,
  "dev/temporal-main-lifecycle/results/latent-kernel-recovery-current-20260914.csv"),
  row.names = FALSE)
utils::write.csv(summary, file.path(root,
  "dev/temporal-main-lifecycle/results/latent-kernel-recovery-current-summary-20260914.csv"),
  row.names = FALSE)
