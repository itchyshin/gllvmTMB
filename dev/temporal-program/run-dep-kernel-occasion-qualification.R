## Direct-DGP, disjoint long-occasion qualification; never calls production simulation.

.temporal_dep_kernel_occasion_plan <- function() {
  x <- expand.grid(phi = c(-.4, 0, .6), seed = 2609371:2609373, KEEP.OUT.ATTRS = FALSE)
  x$n_series <- 80L
  x$n_time <- 32L
  x[order(x$phi, x$seed), c("phi", "seed", "n_series", "n_time")]
}

.temporal_dep_kernel_occasion_validate <- function(phi, seed) {
  ...
  if (length(seed) != 1L || !is.numeric(seed) || !is.finite(seed) ||
      !(as.integer(seed) %in% 2609371:2609373))
    stop("seed must be one disjoint qualification seed: 2609371:2609373", call. = FALSE)
...
.temporal_dep_kernel_occasion_one <- function(phi, seed) {
  started <- proc.time()[["elapsed"]]
  key <- if (identical(as.integer(seed), 2609370L)) .temporal_dep_kernel_occasion_prerun() else .temporal_dep_kernel_occasion_validate(phi, seed)
  out <- tryCatch({
    fixture <- .temporal_dep_kernel_occasion_fixture(key$phi, key$seed)
...
.temporal_dep_kernel_occasion_summarise <- function(result) {
  ...
  list(result = result, summary = summary, contract = "long-occasion qualification only; no general recovery, solver, or release claim")
}

.temporal_dep_kernel_occasion_run <- function(phi = NULL, seed = NULL, pre_run = FALSE) {
  if (isTRUE(pre_run)) {
