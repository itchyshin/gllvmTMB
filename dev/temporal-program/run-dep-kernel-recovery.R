## Retained direct-DGP recovery for replicated AR1 temporal_dep + kernel_indep.
...
K_raw <- exp(-as.matrix(dist(coords)) / .16) + diag(.08, n_series)
...
simulate_fixture <- function(phi, seed) {
  ...
  static <- chol_kernel %*% sweep(matrix(rnorm(n_series * 3L), n_series, 3L),
    2L, truth$kernel_sd, "*")
  ...
  out <- tryCatch({
    ...
    fit <- suppressWarnings(gllvmTMB(
      value ~ 0 + trait + temporal_dep(0 + trait | series, time = occasion,
        replicate = measurement) + kernel_indep(series, K = K, name = "fixed_nonproportional_K"),
    ...
    data.frame(phi = phi, seed = seed, terminal = "success",
      ...
      kernel_1 = par$theta_rr_phy[[1L]]^2,
      kernel_2 = par$theta_rr_phy[[2L]]^2,
...
plan <- expand.grid(phi = c(-.4, 0, .6), seed = 2609221:2609223)
plan <- plan[order(plan$phi, plan$seed), , drop = FALSE]
if (smoke) plan <- data.frame(phi = .6, seed = 2609221L)
result <- do.call(rbind, lapply(seq_len(nrow(plan)), function(i) fit_one(plan$phi[[i]], plan$seed[[i]])))
for (j in 1:3) result[[paste0("kernel_relative_error_", j)]] <-
  abs(result[[paste0("kernel_", j)]] - truth$kernel_sd[[j]]^2) / truth$kernel_sd[[j]]^2
...
if (smoke) {
  ...
  cat("TEMPORAL_DEP_KERNEL_RECOVERY_SMOKE_PASS\n")
...
summary <- do.call(rbind, lapply(split(result, result$phi), function(x) {
  ...
  data.frame(phi = x$phi[[1L]], attempts = nrow(x), strict_successes = sum(strict),
    ...
    median_kernel_1_relative_error = median(x$kernel_relative_error_1[strict]),
    median_kernel_2_relative_error = median(x$kernel_relative_error_2[strict]),
    median_kernel_3_relative_error = median(x$kernel_relative_error_3[strict]),
...
utils::write.csv(result, file.path(root, "dev/temporal-program/results/dep-kernel-recovery-20260911.csv"), row.names = FALSE)
utils::write.csv(summary, file.path(root, "dev/temporal-program/results/dep-kernel-recovery-summary-20260911.csv"), row.names = FALSE)
print(result, row.names = FALSE); print(summary, row.names = FALSE)
if (!all(summary$passes)) stop("Frozen temporal_dep-kernel recovery campaign fails its predeclared thresholds.", call. = FALSE)
