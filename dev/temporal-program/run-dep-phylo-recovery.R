## Retained direct-DGP recovery for replicated AR1 temporal_dep + phylo_indep.
...
if (!requireNamespace("ape", quietly = TRUE)) stop("The direct phylogeny DGP requires ape.", call. = FALSE)
set.seed(2609290L)
tree <- ape::rtree(n_series, tip.label = series)
Cphy <- ape::vcv(tree, corr = TRUE)
Cphy <- Cphy[series, series, drop = FALSE]

simulate_fixture <- function(phi, seed) {
  set.seed(seed)
  chol_time <- t(chol(truth$temporal_covariance))
  chol_phylo <- t(chol(Cphy))
  static <- chol_phylo %*% sweep(matrix(rnorm(n_series * 3L), n_series, 3L),
    2L, truth$phylo_sd, "*")
  ...
  out <- tryCatch({
    ...
    fit <- suppressWarnings(gllvmTMB(
      value ~ 0 + trait + temporal_dep(0 + trait | series, time = occasion,
        replicate = measurement) + phylo_indep(0 + trait | series, vcv = Cphy),
    ...
    data.frame(phi = phi, seed = seed, terminal = "success",
      ...
      phylo_1 = par$theta_rr_phy[[1L]]^2,
      phylo_2 = par$theta_rr_phy[[2L]]^2,
    ...
    temporal_frobenius_relative_error = NA_real_, phylo_1 = NA_real_, phylo_2 = NA_real_,
    phylo_3 = NA_real_, beta_1 = NA_real_, beta_2 = NA_real_, beta_3 = NA_real_,
    stringsAsFactors = FALSE))
  out$elapsed_seconds <- proc.time()[["elapsed"]] - started
  out
}

plan <- expand.grid(phi = c(-.4, 0, .6), seed = 2609291:2609293)
plan <- plan[order(plan$phi, plan$seed), , drop = FALSE]
if (smoke) plan <- data.frame(phi = .6, seed = 2609291L)
...
if (smoke) {
  ...
  cat("TEMPORAL_DEP_PHYLO_RECOVERY_SMOKE_PASS\n")
...
utils::write.csv(result, file.path(root, "dev/temporal-program/results/dep-phylo-recovery-20260911.csv"), row.names = FALSE)
utils::write.csv(summary, file.path(root, "dev/temporal-program/results/dep-phylo-recovery-summary-20260911.csv"), row.names = FALSE)
print(result, row.names = FALSE); print(summary, row.names = FALSE)
if (!all(summary$passes)) stop("Frozen temporal_dep-phylo recovery campaign fails its predeclared thresholds.", call. = FALSE)
