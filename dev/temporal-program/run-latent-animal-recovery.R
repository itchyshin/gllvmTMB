## Retained direct-DGP recovery for replicated AR1 temporal_latent + animal_indep.
...
## Direct, hand-specified relationship matrix for two unrelated founders and
## their full-sibling offspring.  It avoids the package pedigree converter so
## the DGP remains independent of the representation code under test.
A_animal <- matrix(.5, n_series, n_series, dimnames = list(series, series))
diag(A_animal) <- 1
A_animal[1:2, 1:2] <- diag(2)
A_animal[1L, 2L] <- A_animal[2L, 1L] <- 0

simulate_fixture <- function(phi, seed) {
  set.seed(seed)
  static <- t(chol(A_animal)) %*% sweep(matrix(rnorm(n_series * 3L), n_series, 3L),
    2L, truth$animal_sd, "*")
  ...
  out <- tryCatch({
    ...
    fit <- suppressWarnings(gllvmTMB(
      value ~ 0 + trait + temporal_latent(0 + trait | series, time = occasion,
        ...
        animal_indep(0 + trait | series, A = A_animal),
    ...
    data.frame(phi = phi, seed = seed, terminal = "success",
      ...
      animal_1 = par$theta_rr_phy[[1L]]^2, animal_2 = par$theta_rr_phy[[2L]]^2,
...
if (smoke) {
  ...
  cat("TEMPORAL_LATENT_ANIMAL_RECOVERY_SMOKE_PASS\n")
...
utils::write.csv(result, file.path(root, "dev/temporal-program/results/latent-animal-recovery-20260911.csv"), row.names = FALSE)
utils::write.csv(summary, file.path(root, "dev/temporal-program/results/latent-animal-recovery-summary-20260911.csv"), row.names = FALSE)
print(result, row.names = FALSE); print(summary, row.names = FALSE)
if (!all(summary$passes)) stop("Frozen temporal-latent-animal recovery campaign fails its predeclared thresholds.", call. = FALSE)
