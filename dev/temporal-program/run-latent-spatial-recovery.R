## Retained recovery campaign: replicated AR1 rank-one temporal_latent + spatial_indep.
...
simulate_fixture <- function(phi, seed) {
  ...
  temporal <- matrix(0, n_series, n_time)
  temporal[, 1L] <- stats::rnorm(n_series)
  for (time in 2:n_time) temporal[, time] <- phi * temporal[, time - 1L] +
    sqrt(1 - phi^2) * stats::rnorm(n_series)
  data <- template
  group <- match(data$series, series)
  trait <- match(data$trait, traits)
  data$value <- truth$beta[trait] +
    temporal[cbind(group, data$occasion)] * truth$temporal_loading[trait] +
...
fit_one <- function(phi, seed) {
  ...
  out <- tryCatch({
    fit <- suppressWarnings(gllvmTMB(
      value ~ 0 + trait +
        temporal_latent(0 + trait | series, time = occasion, replicate = measurement, d = 1, unique = FALSE) +
    ...
    data.frame(
      ...
      temporal_frobenius_relative_error = sqrt(sum((tcrossprod(as.matrix(temporal$loading)) -
        tcrossprod(truth$temporal_loading))^2)) / sqrt(sum(tcrossprod(truth$temporal_loading)^2)),
    ...
    phi_estimate = NA_real_, temporal_frobenius_relative_error = NA_real_, tau_1 = NA_real_, tau_2 = NA_real_, tau_3 = NA_real_,
...
plan <- expand.grid(phi = c(-.4, 0, .6), seed = 2609261:2609263)
plan <- plan[order(plan$phi, plan$seed), , drop = FALSE]
is_smoke <- identical(Sys.getenv("TEMPORAL_RECOVERY_SMOKE"), "1")
if (is_smoke) plan <- data.frame(phi = .6, seed = 2609261L)
result_path <- if (is_smoke) tempfile("temporal-spatial-smoke-", fileext = ".csv") else
  file.path(root, "dev/temporal-program/results/latent-spatial-recovery-20260911.csv")
...
if (is_smoke) {
  ...
  cat("TEMPORAL_LATENT_SPATIAL_RECOVERY_SMOKE_PASS\n")
...
summary <- do.call(rbind, lapply(split(result, result$phi), function(x) {
  ...
  data.frame(
    ...
    median_temporal_frobenius_relative_error = stats::median(x$temporal_frobenius_relative_error[ok]),
...
summary$passes <- with(summary, strict_successes == 3L &
  ...
  median_temporal_frobenius_relative_error <= .30 & median_tau_1_relative_error <= .35 &
...
utils::write.csv(summary,
  file.path(root, "dev/temporal-program/results/latent-spatial-recovery-summary-20260911.csv"),
  row.names = FALSE)
