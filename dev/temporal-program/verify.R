...
allowed <- c("plan", "simulation", "lifecycle", "remote", "phylo", "dep-kernel", "latent-kernel", "latent-phylo", "latent-animal", "publication", "combinations", "closeout", "self-test")
if (!mode %in% allowed) {
  stop("usage: Rscript --vanilla dev/temporal-program/verify.R {plan|simulation|lifecycle|remote|phylo|dep-kernel|latent-kernel|latent-phylo|latent-animal|publication|combinations|closeout|self-test}", call. = FALSE)
}
