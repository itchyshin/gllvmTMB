...
# Optional cell filters, passed through to the runner's --family / --n-units.
# The 2026-07-29 confirmatory run needs only gaussian n150, so leaving these
# unset would spend ~75% of the compute on cells nothing is waiting on.
FAMILY="${FAMILY:-}"
NUNITS="${NUNITS:-}"
REPSTART="${REPSTART:-}"
REPEND="${REPEND:-}"
FILTERS=""
[ -n "$FAMILY" ]   && FILTERS="$FILTERS --family=$FAMILY"
[ -n "$NUNITS" ]   && FILTERS="$FILTERS --n-units=$NUNITS"
[ -n "$REPSTART" ] && FILTERS="$FILTERS --rep-start=$REPSTART"
[ -n "$REPEND" ]   && FILTERS="$FILTERS --rep-end=$REPEND"
...
if [ "$STAGE" = "smoke" ]; then
  ...
  ssh_t "cd $RWORK && OPENBLAS_NUM_THREADS=1 R_LIBS_USER='$RLIB' \
    Rscript dev/profile-rescore-run.R --mode=shard --shard=1 --n-shards=1 \
      --n-sim=2 --n-boot=10 --out-dir='$OUTDIR/smoke'$FILTERS"
...
if [ "$STAGE" = "grid" ]; then
  ...
  ssh_t "cd $RWORK && seq 1 $CORES | \
    ...
    xargs -P $CORES -I {} Rscript dev/profile-rescore-run.R \
      ...
      --n-sim=$NSIM --n-boot=$NBOOT --out-dir='$OUTDIR'$FILTERS"
