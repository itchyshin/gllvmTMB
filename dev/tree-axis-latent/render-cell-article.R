...
ledger <- readRDS(file.path(root,"validation-G1-G2-G3-GW1-GW2-GW3.rds"))
...
trace(".gllvmTMB_run_nlminb",where=asNamespace("gllvmTMB"),print=FALSE,
  tracer=quote({
    ...
    .article_reference <- readRDS(file.path(.GlobalEnv$root,paste0("fit-",.article_id,".rds")))
...
saveRDS(list(fits=list(morphology=fit_morphology,iid=fit_columns,phylo=fit_phylo_coef),
  standalone_count=33L,primary_render_entries=.article_entries,
