## TASK A (Curie, VA-speed lane): reproduce + INSTRUMENT the 71-split25.R finding.
##
## 71-split25.R (SEEDS 1:3) reported seed 1 at ~25.5s against a much lower median
## across other seeds, all status="healthy". This script:
##   (1) re-runs the SAME DGP at SEEDS <- 1:8 (71 says 1:3; settling that gap is
##       part of the point), and
##   (2) traces stats::nlminb / stats::optim so per-fit optimiser work is
##       counted directly instead of read from fit$best, which is CONTAMINATED
##       when the L-BFGS-B polish escalation fires (fit$best$iterations is then
##       NA_integer_, and fit$best$evaluations reports only the FINAL polish
##       pass's 3/3 calls, not the primary nlminb run).
##
...
cat("\n=== TASK A SUMMARY (seed-ordered, not run-ordered) ===\n")
...
saveRDS(list(data = r, meta = meta), "dev/va-speed/73-split-instrumented.rds")
cat("\nSaved dev/va-speed/73-split-instrumented.rds\n")
cat("DONE\n")
