...
1. **Multivariate GLMMs by reduced-rank regression** -- the
   ...
   (**Σ = ΛΛᵀ + Ψ** by default), exposed via a clean **4 × 3**
...
3. one or more covariance-structure keywords drawn from the
   **4 × 3** grid, with `common = TRUE` as the shared-variance modifier
   of `*_indep()` and `unique = TRUE` as the diagonal-Psi modifier of
   `*_latent()`:

| correlation \ mode | indep | dep | latent |
|---|---|---|---|
...
- **`gllvm`** -- peer GLLVM package, ecology-focused, with
  ...
  the stacked-trait long-format grammar, the explicit 5 x 3 stable-unit grid plus temporal
...
| **Boole** | Formula grammar (4 × 3 keyword grid plus modifiers + `traits()` LHS) | Is the syntax memorable, parseable, internally consistent? |
