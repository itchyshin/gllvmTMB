...
| `column_coef()` / `phylo_coef()` / `animal_coef()` | **partial / public Gaussian point models** | All three helpers are exported through `gllvmTMB()` for long and `traits(...)` wide data. `column_coef()` uses an IID response-column source. `phylo_coef()` accepts a labelled tree, dense covariance, or sparse precision source and either fixes numeric `rho` or estimates one interior value when `rho = NULL`. `animal_coef()` accepts exactly one pedigree, labelled relationship covariance `A`, or labelled precision `Ainv`, and fixes numeric `rho`. `extract_Sigma(level = "column_coef")` reports the ordered coefficient covariance and source metadata. Intervals, non-Gaussian regimes, metadata-by-coefficient interactions, simultaneous coefficient sources, and broad recovery remain unclaimed (validation-debt register FG-20; Design 131; `test-column-coef-public-api.R`; `test-column-coef-phylo-estimated-rho.R`; `test-column-coef-animal-equivalence.R`). |
| `kernel_coef()` / `spatial_coef()` | **reserved / engine-fenced** | The parser reserves these names, but a classed fence stops before likelihood construction. They are not exported or documented as available. Existing `*_slope()` helpers remain current, warning-free, and non-deprecated (validation-debt register FG-20; Design 131). |
