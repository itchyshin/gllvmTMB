...
| `phylo_unique(1 + x \| species)` / `phylo_unique(0 + trait + (0 + trait):x \| species)` | **claimed** | Phase 56.3 parser bridge for augmented-LHS phylogenetic random regression. Parser classification and two-column `Z_phy_aug` construction are exercised by `test-phase56-3-phylo-unique-parser.R`. Do not advertise as covered until Phase 56.4 lands the Gaussian recovery, wide-long byte-identity, and validation-debt evidence. |
