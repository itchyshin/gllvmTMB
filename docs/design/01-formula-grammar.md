...
| `gllvmTMB(..., column_data = metadata)` | **claimed / fixed-metadata foundation** | Exact-keyed response-column metadata are admitted for fixed effects only. Long data key on the resolved `trait` column; wide `traits(...)` data key on a literal `trait` column and reject a non-default `trait =`. Metadata cannot enter grouping, covariance, offset, or coefficient-helper terms. The internal IID `column_coef()` point engine does not admit metadata-by-coefficient interactions; structured coefficient engines remain blocked (validation-debt register FG-20; Design 131). |
...
| `column_coef()` / `phylo_coef()` / `animal_coef()` / `kernel_coef()` / `spatial_coef()` | **partial / internal IID engine only** | Internal parsers validate the response-column coefficient grammar. The unexported Gaussian `column_coef()` route reaches the existing IID matrix-normal point engine; structured helpers retain the classed pre-engine fence. No helper is exported or documented as available, and fixed/estimated `rho`, extractors, intervals, non-Gaussian regimes, and public teaching remain blocked (validation-debt register FG-20; Design 131). |
...
coefficient-helper uses fail validation. The internal IID `column_coef()`
engine therefore accepts row-data coefficient bases only; structured
coefficient helpers still fail before engine construction.
