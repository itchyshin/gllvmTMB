...
| `latent(..., lv = ~ x)` | **planned / blocked** | Predictor-informed latent-score means. Design 73 reserves the term-local fixed-effect formula surface but no parser or TMB runtime support exists yet. C1 target is ordinary Gaussian unit-tier `latent()` only; random terms, offsets, `mi()`, exact fixed-RHS overlap, `REML = TRUE`, non-Gaussian families, unsupported tiers, and source-specific / kernel forms must reject until their validation rows move (validation-debt register FG-18 / LV-01..LV-07). |
...
The C1 parser must reject `lv` until the implementation lands. When it
does land, the first supported surface is ordinary Gaussian unit-tier
