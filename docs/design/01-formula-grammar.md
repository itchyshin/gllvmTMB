...
| `column_coef()` / `phylo_coef()` / `animal_coef()` / `kernel_coef()` | **partial / public Gaussian point models** | All four helpers are exported through `gllvmTMB()` for long and `traits(...)` wide data. `column_coef()` uses IID; `phylo_coef()` uses a labelled tree/covariance/precision with fixed or estimated `rho`; `animal_coef()` accepts one pedigree, `A`, or `Ainv` with fixed `rho`; and `kernel_coef()` accepts one labelled dense covariance with fixed or estimated `rho`. `extract_Sigma(level = "column_coef")` reports the ordered coefficient covariance and source metadata. Intervals, non-Gaussian regimes, metadata-by-coefficient interactions, simultaneous coefficient sources, and broad recovery remain unclaimed (validation-debt register FG-20; Designs 131--132; kernel test files). |
| `spatial_coef()` | **reserved / engine-fenced** | The parser reserves the name, but a classed fence stops before likelihood construction. It is not exported or documented as available. Existing `*_slope()` helpers remain current, warning-free, and non-deprecated (validation-debt register FG-20; Design 131). |
...
`phylo_coef()`, `animal_coef()`, and `kernel_coef()` engines therefore accept
row-data coefficient bases only; the spatial coefficient helper still fails
