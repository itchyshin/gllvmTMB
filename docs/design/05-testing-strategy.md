...
| `gllvm::gllvm()` binary GLLVM | Procrustes-aligned loadings + per-factor $\rho > 0.95$ | binary GLLVM with `latent()`; rotation-aware comparison via `compare_loadings()` | **covered** — `tests/testthat/test-comparator-gllvm.R`, 18 expectations pass (log-likelihood agreement *and* Procrustes loading shape). Note it is `skip_on_cran()`, so it runs locally and in CI (`NOT_CRAN`) but never on CRAN. Status corrected from "claimed (M2 work)" 2026-08-02 after running it. |
