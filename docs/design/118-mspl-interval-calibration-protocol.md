...
| 2026-08-16 | **DEV-5 — H1-q2 hold-out `n_site`.** H1's 3 probit q=2 hold-out cells use `n_site = 96` (not 24): §5.1 gives the cell count only; 96 follows the same exclude-the-smallest pattern H1's non-q2 part already uses to drop C-core's `n_site = 12`. Chosen before any B1 data was seen. | Pre-launch adversarial review T2 (grid reconstruction) | Shinichi (standing authorization, 2026-08-16: *"my authorization for everything finishes"*) |
| 2026-08-16 | **DEV-6 — unstated π-triples.** Cells whose §5.1 definition says "3 π" without listing values (C-ID1, C-ID2, H1's q=2 rows) use {0.03, 0.50, 0.97} — min/median/max of C-core's own 5-value grid. Pre-data. (C-q2 is out of scope here: its π-triple is explicit in §5.1.) | Same review, T2; scope corrected at re-review | Shinichi (same standing authorization) |
| 2026-08-16 | **DEV-7 — `n_trait` where §5.1 is silent.** The blocks actually silent on `n_trait` — C-q2, H1, H2, H3 — use `n_trait = 3`, matching C-core's baseline. C-ID1/C-ID2 are NOT covered by this line: §5.1 varies T there explicitly by design (the c_n/N_eff de-confounding arms). Pre-data; wording corrected at re-review (the harness itself was always correct). | Same review, T2; re-review §8 adjudication | Shinichi (same standing authorization) |
| 2026-08-16 | **DEV-8 — fence line 3 not reinstated under F-AMD.** DEV-3's refusal rule (screen OR attractor-proximity OR root-NA fail-closed) never reads the probe, so Route-A non-convergence no longer gates anything; `half_converged`/`double_converged` are still recorded for §6.1 reporting. | Review T3/T7 (unrecorded-deviation finding) | Shinichi (same standing authorization) |
| 2026-08-16 | **DEV-9 — storage format and shard sizing (review blockers 1–2, fixed in `a3b31e62`).** Per-shard sidecars (one widened profile trace at level 0.99 spanning [0.354, 3.317]; raw bootstrap replicate vectors) replace endpoint-only storage so the §2.4 calibrator re-thresholds at arbitrary α* without refitting; the bootstrap is computed once per dataset (budget restored to the authorized 17.52 M base from a committed 36.7 M defect); bootstrap-bearing shard size and `--time` are provisional pending the Totoro worst-corner measurement. | Pre-launch adversarial review (DO NOT LAUNCH verdict) + fix commit `a3b31e62` | Shinichi (same standing authorization) |
