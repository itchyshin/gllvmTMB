...
| 2026-08-16 | **DEV-6 — unstated π-triples.** Cells whose §5.1 definition says "3 π" without listing values (C-ID1, C-ID2, C-q2, H1's q=2 rows) use {0.03, 0.50, 0.97} — min/median/max of C-core's own 5-value grid. Pre-data. | Same review, T2 | Shinichi (same standing authorization) |
| 2026-08-16 | **DEV-7 — C-q2 / C-ID `n_trait`.** Rows that do not restate `n_trait` use `n_trait = 3`, matching C-core's baseline; §5.1 does not vary T within these blocks. Pre-data. | Same review, T2 | Shinichi (same standing authorization) |
