...
- Inventing Poisson \(W=\mathrm{diag}(\mu)\) KEEP / REPLACE / PARK as a
  **SIGNED** paste (parallel G0 card stays **UNSIGNED**; silent default PARK)
...
doors — not KEEP/REPLACE), they remain **out** of the first profile-led
coverage claim set until a later widen G0.
...
- Resolve Poisson \(W\) KEEP / REPLACE / PARK as a **SIGNED** paste (card stays UNSIGNED; silent default PARK only).
...
(silent default PARK; Gate 1 PARK paste keeps card UNSIGNED);
