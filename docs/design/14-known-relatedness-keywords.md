...
| **V** | **Sampling variance** — the known measurement-error variance in **meta-analysis**. Reserved for `meta_V(V = vi_or_V)`. | `meta_V(V = V)` (`meta_known_V()` is a deprecated alias) |
...
| `animal_*` | bare `id` (or bar-form for `_indep`/`_dep`/`_slope`) | `pedigree = ped` OR `A = A` OR `Ainv = Ainv` | `d = K`, `unique = TRUE/FALSE` on `_latent`; default is loadings-only |
...
| `animal_*(pedigree=)` ≡ `phylo_*(vcv = A)` byte-equivalence | `test-animal-keyword.R`, `test-matrix-animal-nongaussian.R`, `test-animal-latent-unique-fold.R` | logLik agreement to 1e-6 on `animal_scalar`, `_unique`, `_indep`, `_dep`, `_latent`; `animal_latent(..., unique = TRUE)` equivalence to `animal_latent(..., unique = FALSE) + animal_unique()`; default `animal_latent()` remains loadings-only |
