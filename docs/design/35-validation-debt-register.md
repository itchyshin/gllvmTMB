...
| FG-18 | `latent(..., lv = ~ x)` predictor-informed latent-score grammar | `partial` | `test-lv-parser-guard.R`; `test-julia-bridge.R` | Ordinary unit-tier `latent(..., lv = ~ x)` now reaches C1 fits for Gaussian and pure binomial logit/probit/cloglog responses: the parser stores `extra$lv_formula` on the reduced-rank term only, keeps the auto-added `Psi` companion free of `lv` metadata, builds unit-level `X_lv_B`, wires `alpha_lv_B` into the TMB score mean, and reports total/mean/innovation score components plus `B_lv_unit`. Long and `traits(...)` wide Gaussian surfaces are covered by smoke tests; the three standard binary links have long-form multi-trial recovery/algebra gates; the R Julia bridge passes the same unit-level `X_lv` design only for narrow complete-response Gaussian and binomial logit/probit/cloglog point rows. This row remains partial because no Gaussian recovery grid, missing-response compatibility, interval calibration, Bernoulli single-trial depth, other non-Gaussian family support, tier-expanded support, structured-source support, or broad Julia parity is admitted. Random terms, offsets, `mi()`, smooths, response/trait columns, nonconstant-within-unit predictors, rank-deficient designs, exact fixed/LV overlap, `REML = TRUE`, unsupported non-Gaussian families, unsupported tiers, augmented random-regression combinations, ordinary diagonal aliases, source-specific forms, kernel forms, and unsupported Julia bridge `X_lv` combinations still reject until their rows move. |
...
| RE-13 | Ordinary unit-tier predictor-informed latent-score mean `latent(..., lv = ~ x)` | `partial` | `test-lv-parser-guard.R` | Design 73 C1 support is live for ordinary unit-tier Gaussian and pure binomial logit/probit/cloglog fits. The implemented model is `u_i = X_lv,i alpha + e_i`, `e_i ~ N(0, I_K)`, with the ordinary `latent()` `Lambda Lambda^T + Psi` decomposition preserved where identifiable and `B_lv = Lambda alpha^T` reported as the primary trait-scale estimand. Evidence currently covers parser preflight, long/wide Gaussian smoke fits, finite TMB reports, extractor dimensions/algebra, and pure-binomial multi-trial `B_lv` recovery/algebra gates for the three standard binary links. The row remains partial until Gaussian recovery of `B_lv`, `Sigma`, and `Psi`, Bernoulli single-trial binary depth, and interval calibration land; `REML = TRUE`, unsupported non-Gaussian families, unsupported tiers/sources, and augmented random-regression combinations remain rejected. |
...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| LV-01 | Ordinary Gaussian unit-tier `latent(..., lv = ~ x)` end-to-end support | `partial` | `test-lv-parser-guard.R`; `test-julia-bridge.R` | C1 ordinary Gaussian unit-tier support now reaches TMB, extractors, and a narrow Julia bridge point route: parser metadata and unit-level `X_lv_B` preflight are retained, `alpha_lv_B` is estimated on the TMB path, the zero-mean innovation `e_i` remains in the likelihood, the ordinary `Psi` companion is preserved on TMB unless the user asks for `unique = FALSE`, `B_lv_unit = Lambda alpha^T` is reported/ADREPORTed, and extractor smoke tests verify score decomposition and trait-scale algebra. The Julia bridge route is complete-response Gaussian only, requires no fixed-effect `X`, no response mask, no CIs, and fits the reduced-rank `latent(..., unique = FALSE, lv = ~ x)` point route with retained `X_lv`, `lv_effects`, `alpha_lv`, `scores_mean`, and `scores_innovation`. The row remains partial because the current evidence is a CRAN-safe smoke/algebra/bridge-payload gate, not Gaussian recovery of `B_lv`, `Sigma`, and `Psi`, interval calibration, missing-response compatibility, Julia `X_lv` CIs, or broad Julia parity. Must reject `REML = TRUE`, exact fixed/LV overlap, unsupported tiers/sources, augmented random-regression combinations, and unsupported Julia bridge `X_lv` combinations. |
