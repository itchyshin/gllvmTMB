...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| JUL-01 | Lean `engine = "julia"` reduced-rank bridge through `GLLVM.bridge_fit` | `partial` | `test-julia-bridge.R`; `docs/dev-log/2026-06-16-nb1-gamma-bridge-parameterisation-audit.md`; `docs/dev-log/2026-06-16-nb1-reduced-rank-parity-audit.md`; `docs/dev-log/2026-06-16-nb1-reduced-rank-fisher-fix.md` | Main-dispatch route admits one `latent(..., d = K)` block on complete balanced trait x unit data, plus no-latent (`d = 0`) rows where the paired Julia bridge admits them, with Gaussian fixed-effect covariates only and no structured covariance terms. The 2026-06-16 grouped-dispersion slice maps R `nbinom2()` / `nbinom1()` / `Beta()` / `Gamma()` to the paired Julia grouped-dispersion payload, labels `dispersion`, `dispersion_group`, and `dispersion_group_id`, and adds public-scale fields for NB2 (`sigma = 1/sqrt(r)`, `phi = 1/r`), NB1 (`phi` identity), Beta (`sigma = 1/sqrt(phi)`), and Gamma (`sigma = 1/sqrt(alpha)`). The grouped-dispersion main-dispatch smoke exercises all four rows through `gllvmTMB(..., engine = "julia")`; NB2 and Beta carry selected small-fixture native objective parity against `engine = "tmb"`. NB1 is parameterisation-aligned with native `phi_nbinom1` (`Var = mu * (1 + phi)`), and `test-julia-bridge.R` now includes three NB1 evidence layers: a fixed-parameter kernel check where Julia `nb1_grouped_marginal_loglik_laplace()` with zero loadings matches the native linear-variance `dnbinom(mu = mu, size = mu / phi)` likelihood to `1e-10`, a no-latent fitted-object check where `value ~ 0 + trait` matches native `engine = "tmb"` log-likelihood, df, labels, and `phi` scale on the small fixture, and a reduced-rank fitted-object check where `value ~ 0 + trait + latent(0 + trait | unit, d = 1)` matches native log-likelihood/df and checks per-trait intercepts, loadings, and `phi` on the same small complete balanced fixture after the paired Julia tiny-`phi` Fisher-information fix. Gamma now uses one shared grouped-Gamma shape in the bridge to match current native ordinary Gamma's scalar `sigma_eps` coefficient of variation; the small reduced-rank fixture checks `df = 5`, native-vs-Julia log-likelihood delta about `2.8e-07`, `dispersion_group_id = c(1, 1)`, and public `sigma` equality to native `sigma_eps`. Native per-trait Gamma CV/shape remains a future R/TMB expansion and is not claimed here. The paired ordinal slice labels per-trait ordinal cutpoint matrices, per-trait category counts, `cutpoint_mode`, and `cutpoint_link`, while marking ordinal bridge CIs unavailable. Still partial: broad native-vs-Julia parity rows beyond selected fixtures, grouped-dispersion CI endpoints, per-trait ordinal CI endpoints, response masks, non-Gaussian X through the main dispatch, mixed-family promotion, structured terms, and post-fit extractor parity remain planned/gated. |
