...
| RE-13 | Ordinary unit-tier predictor-informed latent-score mean `latent(..., lv = ~ x)` | `partial` | `test-lv-parser-guard.R`; `test-lv-gaussian-recovery.R` | Design 73 C1 support is live for ordinary unit-tier Gaussian and pure binomial logit/probit/cloglog fits. The implemented model is `u_i = X_lv,i alpha + e_i`, `e_i ~ N(0, I_K)`, with the ordinary `latent()` `Lambda Lambda^T + Psi` decomposition preserved where identifiable and `B_lv = Lambda alpha^T` reported as the primary trait-scale estimand. Evidence currently covers parser preflight, long/wide Gaussian smoke fits, finite TMB reports, extractor dimensions/algebra, focused native TMB Gaussian recovery of rotation-stable `B_lv` and `Sigma` targets, and pure-binomial multi-trial `B_lv` recovery/algebra gates for the three standard binary links. The row remains partial until 500-rep Gaussian interval coverage, Bernoulli single-trial binary depth, missing-response compatibility, and broader bridge parity land; `REML = TRUE`, unsupported non-Gaussian families, unsupported tiers/sources, and augmented random-regression combinations remain rejected. |
...
2026-06-26): the R bridge now admits only complete-response Gaussian and
binomial logit/probit/cloglog `X_lv` point routes for
...
requests stop at `GJL-GATE-XLV-CI`. Other non-Gaussian `X_lv`, response masks
with `X_lv`, fixed-effect `X` plus `X_lv`, mixed-family `X_lv`, and broad
native-vs-Julia parity remain gated (`GJL-GATE-XLV-FAMILY`,
...
| LV-05 | Non-Gaussian predictor-informed latent scores | `partial` | `test-lv-parser-guard.R` | Pure `binomial()` ordinary unit-tier `latent(..., lv = ~ x)` is admitted for the package's three standard binary links: logit, probit, and cloglog. The gate verifies family/link IDs, finite reports, `total = innovation + mean`, and trait-scale `B_lv = Lambda alpha^T` recovery on small multi-trial fixtures for all three links. This is a narrow pure-binomial point/recovery gate only: Bernoulli single-trial depth, ordinal, count, Gamma, Beta, mixed-family, delta/hurdle, interval calibration, and CI-08/CI-10 coverage remain blocked until their own family-specific recovery and link-scale diagnostics land. |
