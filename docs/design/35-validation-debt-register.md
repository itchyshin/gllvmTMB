...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| JUL-01 | Lean `engine = "julia"` reduced-rank bridge through `GLLVM.bridge_fit` | `partial` | `test-julia-bridge.R`; `docs/dev-log/2026-06-16-nb1-gamma-bridge-parameterisation-audit.md`; `docs/dev-log/2026-06-16-nb1-reduced-rank-parity-audit.md` | Main-dispatch route admits one `latent(..., d = K)` block on complete balanced trait x unit data, plus no-latent (`d = 0`) rows where the paired Julia bridge admits them, with Gaussian fixed-effect covariates only and no structured covariance terms. The 2026-06-16 grouped-dispersion slice maps R `nbinom2()` / `nbinom1()` / `Beta()` / `Gamma()` to the paired Julia grouped-dispersion payload, labels `dispersion`, `dispersion_group`, and `dispersion_group_id`, and adds public-scale fields for NB2 (`sigma = 1/sqrt(r)`, `phi = 1/r`), NB1 (`phi` identity), Beta (`sigma = 1/sqrt(phi)`), and Gamma (`sigma = 1/sqrt(alpha)`). The grouped-dispersion main-dispatch smoke exercises all four rows through `gllvmTMB(..., engine = "julia")`; NB2 and Beta carry selected small-fixture native objective parity against `engine = "tmb"`. NB1 is parameterisation-aligned with native `phi_nbinom1` (`Var = mu * (1 + phi)`), and `test-julia-bridge.R` now includes two NB1 evidence layers: a fixed-parameter kernel check where Julia `nb1_grouped_marginal_loglik_laplace()` with zero loadings matches the native linear-variance `dnbinom(mu = mu, size = mu / phi)` likelihood to `1e-10`, and a no-latent fitted-object check where `value ~ 0 + trait` matches native `engine = "tmb"` log-likelihood, df, labels, and `phi` scale on the small fixture. Reduced-rank (`K > 0`) NB1 fitted-object objective parity remains unpromoted: the reduced-rank audit found boundary-dominated near misses and a best non-boundary converged seed-search gap of about `0.07678` logLik units, with nuisance/loading allocation drift. Gamma is route/shape evidence only: Julia grouped Gamma estimates per-trait/grouped `alpha`, while current native ordinary Gamma uses shared scalar `sigma_eps` as the coefficient of variation. A maintainer decision is required before any Gamma native-parity claim: route bridge Gamma through shared grouping for the current oracle, or expand native R/TMB Gamma to per-trait CV/shape. The paired ordinal slice labels per-trait ordinal cutpoint matrices, per-trait category counts, `cutpoint_mode`, and `cutpoint_link`, while marking ordinal bridge CIs unavailable. Still partial: broad native-vs-Julia parity rows, reduced-rank NB1 parity, grouped-dispersion CI endpoints, per-trait ordinal CI endpoints, response masks, non-Gaussian X through the main dispatch, mixed-family promotion, structured terms, and post-fit extractor parity remain planned/gated. |
