...
| FG-18 | `latent(..., lv = ~ x)` predictor-informed latent-score grammar | `blocked` | `test-lv-parser-guard.R` (parser/API preflight only) | Design 73 reserves the term-local fixed-effect formula surface but no accepted TMB runtime support exists yet. The current parser stores `extra$lv_formula` on the reduced-rank term only, keeps the auto-added `Psi` companion free of `lv` metadata, builds the future unit-level `X_lv_B` design, validates long and `traits(...)` wide surfaces, and then aborts before TMB construction so releases cannot silently ignore score predictors. C1 target is ordinary Gaussian unit-tier `latent()` only. Random terms, offsets, `mi()`, smooths, response/trait columns, nonconstant-within-unit predictors, rank-deficient designs, exact fixed/LV overlap, `REML = TRUE`, non-Gaussian families, unsupported tiers, augmented random-regression combinations, ordinary diagonal aliases, source-specific forms, and kernel forms reject until LV-01..LV-07 move. |
...
| RE-13 | Ordinary unit-tier predictor-informed latent-score mean `latent(..., lv = ~ x)` | `blocked` | `test-lv-parser-guard.R` (parser/API preflight only) | Design 73 only. The future model is `z_i = M_i alpha + e_i`, `e_i ~ N(0, I_K)`, with the ordinary `latent()` `Lambda Lambda^T + Psi` decomposition preserved and `B_lv = Lambda alpha^T` as the primary trait-scale estimand. Current parser/API evidence validates the ordinary Gaussian unit-tier surface and builds the future unit-level `X_lv_B` design, but no TMB likelihood, ADREPORT, extractor, or recovery support exists yet; the current runtime aborts before fitting if `lv` is supplied. |
...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| LV-01 | Ordinary Gaussian unit-tier `latent(..., lv = ~ x)` end-to-end support | `blocked` | `test-lv-parser-guard.R` (parser/API preflight only) | C1 target. Parser metadata and unit-level `X_lv_B` preflight now exist, but end-to-end support still requires TMB `alpha_lv_B`, preserved zero-mean innovation `e_i`, ordinary `Psi` companion, `B_lv = Lambda alpha^T` ADREPORT, and extractor output. Current evidence proves the reserved `lv` formula is normalized, validated, and not silently ignored; it does not prove a likelihood. Must reject `REML = TRUE`, non-Gaussian families, exact fixed/LV overlap, unsupported tiers/sources, and augmented random-regression combinations. |
...
| LV-04 | Factor predictors and rank-deficiency guards in `lv` | `blocked` | `test-lv-parser-guard.R` (parser/API preflight only) | Parser/API tests now cover factor expansion, intercept dropping, equivalence of `lv = ~ x` and `lv = ~ 0 + x`, and typed rejection for rank-deficient or aliased `X_lv` designs. The row remains blocked because no TMB likelihood, extractor, or recovery evidence exists. |
