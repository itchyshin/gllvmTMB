...
| FG-18 | `latent(..., lv = ~ x)` predictor-informed latent-score grammar | `blocked` | `test-lv-parser-guard.R` (reserved-surface fail-loud guard only) | Design 73 reserves the term-local fixed-effect formula surface but no accepted parser/TMB runtime support exists yet. The current parser preserves the reserved `lv` formula in term metadata, and the fitter now aborts before TMB construction so releases cannot silently ignore score predictors. C1 target is ordinary Gaussian unit-tier `latent()` only. Random terms, offsets, `mi()`, smooths, response/trait columns, nonconstant-within-unit predictors, exact fixed/LV overlap, `REML = TRUE`, non-Gaussian families, unsupported tiers, and source-specific / kernel forms must reject until LV-01..LV-07 move. |
...
| RE-13 | Ordinary unit-tier predictor-informed latent-score mean `latent(..., lv = ~ x)` | `blocked` | `test-lv-parser-guard.R` (reserved-surface fail-loud guard only) | Design 73 only. The future model is `z_i = M_i alpha + e_i`, `e_i ~ N(0, I_K)`, with the ordinary `latent()` `Lambda Lambda^T + Psi` decomposition preserved and `B_lv = Lambda alpha^T` as the primary trait-scale estimand. No accepted parser, TMB, or extractor support exists yet; the current runtime aborts before fitting if `lv` is supplied. |
...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| LV-01 | Ordinary Gaussian unit-tier `latent(..., lv = ~ x)` end-to-end support | `blocked` | `test-lv-parser-guard.R` (reserved-surface fail-loud guard only) | C1 target. Requires parser metadata, unit-level `X_lv_B`, TMB `alpha_lv_B`, preserved zero-mean innovation `e_i`, ordinary `Psi` companion, `B_lv = Lambda alpha^T` ADREPORT, and extractor output. Current evidence only proves the reserved `lv` formula is not silently ignored. Must reject `REML = TRUE`, non-Gaussian families, exact fixed/LV overlap, unsupported tiers/sources, and augmented random-regression combinations. |
