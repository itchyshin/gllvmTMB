...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| JUL-01 | Lean `engine = "julia"` reduced-rank bridge through `GLLVM.bridge_fit` | `partial` | `test-julia-bridge.R` | Main-dispatch route admits one `latent(..., d = K)` block on complete balanced trait x unit data, with Gaussian fixed-effect covariates only and no structured covariance terms. The 2026-06-16 grouped-dispersion slice maps R `nbinom2()` / `nbinom1()` / `Beta()` / `Gamma()` to the paired Julia grouped-dispersion payload, labels `dispersion`, `dispersion_group`, and `dispersion_group_id` by trait, and adds public-scale fields for NB2 (`sigma = 1/sqrt(r)`, `phi = 1/r`), NB1 (`phi` identity), Beta (`sigma = 1/sqrt(phi)`), and Gamma (`sigma = 1/sqrt(alpha)`). The paired ordinal slice labels per-trait ordinal cutpoint matrices, per-trait category counts, `cutpoint_mode`, and `cutpoint_link`, while marking ordinal bridge CIs unavailable. Still partial: native-vs-Julia parity rows, grouped-dispersion CI endpoints, per-trait ordinal CI endpoints, response masks, non-Gaussian X through the main dispatch, mixed-family promotion, structured terms, and post-fit extractor parity remain planned/gated. |
