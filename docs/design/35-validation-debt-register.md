...
| FG-18 | `latent(..., lv = ~ x)` predictor-informed latent-score grammar | `partial` | `test-lv-parser-guard.R` | Ordinary Gaussian unit-tier `latent(..., lv = ~ x)` now reaches a small C1 fit: the parser stores `extra$lv_formula` on the reduced-rank term only, keeps the auto-added `Psi` companion free of `lv` metadata, builds unit-level `X_lv_B`, wires `alpha_lv_B` into the TMB score mean, and reports total/mean/innovation score components plus `B_lv_unit`. Long and `traits(...)` wide surfaces are covered by smoke tests. This row remains partial because no Gaussian recovery grid, missing-response compatibility, interval calibration, non-Gaussian support, tier-expanded support, structured-source support, or Julia parity is admitted. Random terms, offsets, `mi()`, smooths, response/trait columns, nonconstant-within-unit predictors, rank-deficient designs, exact fixed/LV overlap, `REML = TRUE`, non-Gaussian families, unsupported tiers, augmented random-regression combinations, ordinary diagonal aliases, source-specific forms, and kernel forms still reject until their rows move. |
...
| RE-13 | Ordinary unit-tier predictor-informed latent-score mean `latent(..., lv = ~ x)` | `partial` | `test-lv-parser-guard.R` | Design 73 C1 support is live for ordinary Gaussian unit-tier fits. The implemented model is `u_i = X_lv,i alpha + e_i`, `e_i ~ N(0, I_K)`, with the ordinary `latent()` `Lambda Lambda^T + Psi` decomposition preserved and `B_lv = Lambda alpha^T` reported as the primary trait-scale estimand. Evidence currently covers parser preflight, long/wide Gaussian smoke fits, finite TMB reports, and extractor dimensions/algebra. The row remains partial until Gaussian recovery of `B_lv`, `Sigma`, and `Psi` lands; `REML = TRUE`, non-Gaussian families, unsupported tiers/sources, and augmented random-regression combinations remain rejected. |
