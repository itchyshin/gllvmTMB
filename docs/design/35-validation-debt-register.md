...
| EXT-31 | `extract_lv_effects()` and `extract_ordination(component = ...)` for predictor-informed latent scores | `partial` | `test-lv-parser-guard.R` | `extract_lv_effects()` now returns point-estimate rows for `B_lv = Lambda alpha^T` as the preferred trait-scale table and raw `alpha` only as `type = "axis_effect"` with rotation-dependent status metadata. `extract_ordination(component = c("total", "innovation", "mean"))` now separates the score entering the linear predictor, the zero-mean innovation, and the predictor-informed mean for ordinary unit-tier fits. The row remains partial because standard errors are `NA`, ADREPORT uncertainty is not calibration-tested, Julia bridge fits accept only `component = "total"`, and no recovery/interval grid has moved. |
