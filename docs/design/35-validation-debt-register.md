...
| COE-04 | Two-component recovery and kernel-separation diagnostics for Paper 2 Option B | `partial` (near-orthogonal Gaussian latent-only recovery covered; broader grid open) | `test-coevolution-two-kernel.R` (fitted-object kernel-similarity diagnostic; heavy near-orthogonal two-component recovery) | First COE-04 slice landed 2026-06-18. A predeclared off-diagonal Frobenius-style kernel-similarity diagnostic is stored on multi-kernel fits as `fit$kernel_diagnostics`; it classifies low-overlap versus high-overlap kernel pairs. The heavy DGP fits two latent-only named kernels (`phy`, `non`) with distinct low-overlap cross-lineage kernels; the full model beats either one-component model by >50 log-likelihood units, `extract_Gamma(level = "phy")` and `extract_Gamma(level = "non")` each recover their own `Gamma_shape` truth with correlation >0.95, and each component fails to match the other component's truth. Still open: moderately overlapping and highly overlapping recovery/failure cases, selective-absence and block-null calibration, explicit failure language for unidentified component separation, `rho` profiling/estimation, interval coverage, mixed/non-Gaussian gates, and the separate post-arc Psi/deprecation grammar plan. Do not promote full scientific coverage until those rows move with evidence. |
