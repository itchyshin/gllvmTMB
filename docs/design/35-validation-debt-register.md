...
| FG-18 | `latent(..., lv = ~ x)` predictor-informed latent-score grammar | `partial` | `test-lv-parser-guard.R`; `test-julia-bridge.R`; `test-lv-gaussian-recovery.R` | Ordinary unit-tier `latent(..., lv = ~ x)` now reaches C1 fits for Gaussian and pure binomial logit/probit/cloglog responses: the parser stores `extra$lv_formula` on the reduced-rank term only, keeps the auto-added `Psi` companion free of `lv` metadata, builds unit-level `X_lv_B`, wires `alpha_lv_B` into the TMB score mean, and reports total/mean/innovation score components plus `B_lv_unit`. Long and `traits(...)` wide Gaussian surfaces are covered by smoke tests, the native TMB Gaussian path now has focused rank-1/rank-2 recovery evidence for rotation-stable `B_lv` and `Sigma`, the three standard binary links have long-form multi-trial recovery/algebra gates, and the R Julia bridge passes the same unit-level `X_lv` design only for narrow complete-response Gaussian, Poisson, and binomial logit/probit/cloglog point rows. This row remains partial because no 500-rep coverage grid, missing-response compatibility, interval calibration, Bernoulli single-trial depth, native count-family `lv` support, non-Gaussian bridge families beyond Poisson and the three binomial standard links, tier-expanded support, structured-source support, or broad Julia parity is admitted. Random terms, offsets, `mi()`, smooths, response/trait columns, nonconstant-within-unit predictors, rank-deficient designs, exact fixed/LV overlap, `REML = TRUE`, unsupported native non-Gaussian families, unsupported bridge families, unsupported tiers, augmented random-regression combinations, ordinary diagonal aliases, source-specific forms, kernel forms, and unsupported Julia bridge `X_lv` combinations still reject until their rows move. |
...
| RE-13 | Ordinary unit-tier predictor-informed latent-score mean `latent(..., lv = ~ x)` | `partial` | `test-lv-parser-guard.R`; `test-lv-gaussian-recovery.R`; `test-julia-bridge.R` | Design 73 C1 support is live for ordinary unit-tier Gaussian and pure binomial logit/probit/cloglog native TMB fits; the R-to-Julia bridge additionally admits a complete-response Poisson point route under the `JUL-01` / `JUL-01A` restrictions. The implemented native model is `u_i = X_lv,i alpha + e_i`, `e_i ~ N(0, I_K)`, with the ordinary `latent()` `Lambda Lambda^T + Psi` decomposition preserved where identifiable and `B_lv = Lambda alpha^T` reported as the primary trait-scale estimand. Evidence currently covers parser preflight, long/wide Gaussian smoke fits, finite TMB reports, extractor dimensions/algebra, focused native TMB Gaussian recovery of rotation-stable `B_lv` and `Sigma` targets, pure-binomial multi-trial `B_lv` recovery/algebra gates for the three standard binary links, and R bridge payload routing for Gaussian, Poisson, and binomial point rows. The row remains partial until 500-rep Gaussian interval coverage, Bernoulli single-trial binary depth, missing-response compatibility, and broader bridge parity land; `REML = TRUE`, unsupported native non-Gaussian families, unsupported bridge families, unsupported tiers/sources, and augmented random-regression combinations remain rejected. |
...
| EXT-31 | `extract_lv_effects()` and `extract_ordination(component = ...)` for predictor-informed latent scores | `partial` | `test-lv-parser-guard.R`; `test-julia-bridge.R`; `test-lv-gaussian-recovery.R` | `extract_lv_effects()` now returns rows for `B_lv = Lambda alpha^T` as the preferred trait-scale table and raw `alpha` only as `type = "axis_effect"` with rotation-dependent status metadata. Trait-scale `std.error` values are copied from TMB's delta-method `ADREPORT(B_lv_unit)` output when `se = TRUE` yields a positive-definite `sdreport()` and are labelled `wald_sdreport_no_ci_validation`; the rank-1 Gaussian recovery gate checks those SEs against an independent manual delta calculation. Gaussian, Poisson, and binomial logit/probit/cloglog Julia bridge `X_lv` fits return point-only `lv_effects`/`alpha_lv` extractor rows with `std.error = NA` and `uncertainty_status = "julia_bridge_point_estimate_only_no_ci_validation"`. `extract_ordination(component = c("total", "innovation", "mean"))` now separates the score entering the linear predictor, the zero-mean innovation, and the predictor-informed mean for ordinary unit-tier TMB fits and narrow Gaussian, Poisson, and binomial Julia bridge `X_lv` fits. The row remains partial because interval coverage is not calibration-tested, non-Gaussian Julia bridge `X_lv` rows beyond Poisson and the three binomial standard links remain gated, and no 500-rep coverage grid has moved. |
...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| LV-01 | Ordinary Gaussian unit-tier `latent(..., lv = ~ x)` end-to-end support | `partial` | `test-lv-parser-guard.R`; `test-julia-bridge.R`; `test-lv-gaussian-recovery.R` | C1 ordinary Gaussian unit-tier support now reaches TMB, extractors, focused recovery tests, and a narrow Julia bridge point route: parser metadata and unit-level `X_lv_B` preflight are retained, `alpha_lv_B` is estimated on the TMB path, the zero-mean innovation `e_i` remains in the likelihood, the ordinary `Psi` companion is preserved on TMB unless the user asks for `unique = FALSE`, `B_lv_unit = Lambda alpha^T` is reported/ADREPORTed, extractor smoke tests verify score decomposition and trait-scale algebra, and native TMB Gaussian recovery checks cover rotation-stable `B_lv` plus `Sigma = Lambda Lambda^T + Psi` targets. The Julia bridge route is complete-response Gaussian only, requires no fixed-effect `X`, no response mask, no CIs, and fits the reduced-rank `latent(..., unique = FALSE, lv = ~ x)` point route with retained `X_lv`, `lv_effects`, `alpha_lv`, `scores_mean`, and `scores_innovation`. The row remains partial because the current evidence is focused recovery and bridge-payload evidence, not 500-rep interval calibration, missing-response compatibility, Julia `X_lv` CIs, or broad Julia parity. Must reject `REML = TRUE`, exact fixed/LV overlap, unsupported tiers/sources, augmented random-regression combinations, and unsupported Julia bridge `X_lv` combinations. |
| LV-02 | Gaussian recovery of `B_lv`, `Sigma`, and `Psi` | `partial` | `test-lv-gaussian-recovery.R`; `dev/lv-wald-coverage.R`; `test-lv-wald-coverage-harness.R` | Focused native TMB Gaussian recovery now covers a CRAN-safe rank-1 DGP and a heavy rank-2 DGP. The rank-1 gate checks convergence, positive-definite `sdreport()`, finite `ADREPORT(B_lv_unit)` SEs, agreement between extractor SEs and report SEs, independent manual delta-method SEs for `B_lv`, primary trait-scale `B_lv` recovery, and secondary `Sigma = Lambda Lambda^T + Psi` recovery with finite `Psi`. The heavy rank-2 gate checks the same rotation-stable recovery targets while deliberately avoiding raw `alpha` or raw `Lambda` as pass/fail targets because they are rotation-dependent for `K > 1`. A dev-only Wald coverage campaign harness now defines four ordinary Gaussian cells, assigns one recorded seed per replicate/SLURM array task, writes per-replicate RDS outputs, and summarises coverage, MCSE, bias, RMSE, convergence, positive-definite Hessian, `sdreport()` availability, CI availability, and failed-fit denominators for trait-scale `B_lv`; the current harness/pilot is infrastructure, not coverage calibration. This row remains partial, not covered, until a 500-rep-per-cell Gaussian coverage grid reports MCSE, failed-fit denominators, and Wald/profile/bootstrap interval behaviour. |
...
| LV-04 | Factor predictors and rank-deficiency guards in `lv` | `partial` | `test-lv-parser-guard.R` | Parser/API tests cover factor expansion, intercept dropping, equivalence of `lv = ~ x` and `lv = ~ 0 + x`, and typed rejection for rank-deficient or aliased `X_lv` designs. The underlying TMB path accepts the resulting `X_lv_B` column count, but this row remains partial because factor-predictor runtime smoke, rare-level behaviour, and recovery evidence have not landed. |
| LV-05 | Non-Gaussian predictor-informed latent scores | `partial` | `test-lv-parser-guard.R`; `test-julia-bridge.R` | Pure `binomial()` ordinary unit-tier native TMB `latent(..., lv = ~ x)` is admitted for the package's three standard binary links: logit, probit, and cloglog. The native gate verifies family/link IDs, finite reports, `total = innovation + mean`, and trait-scale `B_lv = Lambda alpha^T` recovery on small multi-trial fixtures for all three links. The R-to-Julia bridge additionally admits a complete-response Poisson point route for `latent(..., unique = FALSE, lv = ~ x)` with no fixed-effect `X`, no response mask, no CIs, and retained `X_lv`/score-mean payloads. This is a narrow pure-binomial native point/recovery gate plus bridge-only Poisson point route: Bernoulli single-trial depth, native count-family support, ordinal, NB, Gamma, Beta, mixed-family, delta/hurdle, interval calibration, and CI-08/CI-10 coverage remain blocked until their own family-specific recovery and link-scale diagnostics land. |
