...
| FG-18 | `latent(..., lv = ~ x)` predictor-informed latent-score grammar | `partial` | `test-lv-parser-guard.R` | Ordinary unit-tier `latent(..., lv = ~ x)` now reaches C1 fits for Gaussian and pure binomial logit/probit/cloglog responses: the parser stores `extra$lv_formula` on the reduced-rank term only, keeps the auto-added `Psi` companion free of `lv` metadata, builds unit-level `X_lv_B`, wires `alpha_lv_B` into the TMB score mean, and reports total/mean/innovation score components plus `B_lv_unit`. Long and `traits(...)` wide Gaussian surfaces are covered by smoke tests; the three standard binary links have long-form multi-trial recovery/algebra gates. This row remains partial because no Gaussian recovery grid, missing-response compatibility, interval calibration, Bernoulli single-trial depth, other non-Gaussian family support, tier-expanded support, structured-source support, or Julia parity is admitted. Random terms, offsets, `mi()`, smooths, response/trait columns, nonconstant-within-unit predictors, rank-deficient designs, exact fixed/LV overlap, `REML = TRUE`, unsupported non-Gaussian families, unsupported tiers, augmented random-regression combinations, ordinary diagonal aliases, source-specific forms, and kernel forms still reject until their rows move. |
...
| EXT-31 | `extract_lv_effects()` and `extract_ordination(component = ...)` for predictor-informed latent scores | `partial` | `test-lv-parser-guard.R` | `extract_lv_effects()` now returns rows for `B_lv = Lambda alpha^T` as the preferred trait-scale table and raw `alpha` only as `type = "axis_effect"` with rotation-dependent status metadata. Trait-scale `std.error` values are copied from TMB's delta-method `ADREPORT(B_lv_unit)` output when `se = TRUE` yields a positive-definite `sdreport()` and are labelled `wald_sdreport_no_ci_validation`. `extract_ordination(component = c("total", "innovation", "mean"))` now separates the score entering the linear predictor, the zero-mean innovation, and the predictor-informed mean for ordinary unit-tier fits. The row remains partial because interval coverage is not calibration-tested, Julia bridge fits accept only `component = "total"`, and no recovery/interval grid has moved. |
...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| LV-01 | Ordinary Gaussian unit-tier `latent(..., lv = ~ x)` end-to-end support | `partial` | `test-lv-parser-guard.R` | C1 ordinary Gaussian unit-tier support now reaches TMB and extractors: parser metadata and unit-level `X_lv_B` preflight are retained, `alpha_lv_B` is estimated, the zero-mean innovation `e_i` remains in the likelihood, the ordinary `Psi` companion is preserved unless the user asks for `unique = FALSE`, `B_lv_unit = Lambda alpha^T` is reported/ADREPORTed, and extractor smoke tests verify score decomposition and trait-scale algebra. The row remains partial because the current evidence is a CRAN-safe smoke/algebra gate, not Gaussian recovery of `B_lv`, `Sigma`, and `Psi`, interval calibration, or missing-response compatibility. Must reject `REML = TRUE`, exact fixed/LV overlap, unsupported tiers/sources, and augmented random-regression combinations. |
