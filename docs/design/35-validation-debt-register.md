...
| DIA-11 | Fitted-model predictive diagnostic plots | `partial` | `test-ppcheck-diagnostics-prototype.R` | Design 51. Dev-only prototype returns `ggplot` objects with attached diagnostic data for Gaussian, Poisson, and NB2 fits. Not exported; not a Bayesian posterior-predictive claim. |
| DIA-12 | Simulation-rank residual prototype | `partial` | `test-ppcheck-diagnostics-prototype.R` | Design 51. Dev-only simulation-rank residual table keeps non-finite rows with explicit status. Exact family-CDF randomized-quantile residuals remain future work. |
