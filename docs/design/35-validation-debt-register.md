...
| COE-03 | Two-kernel coevolution model (`K_phy` + `K_non`) and two-Psi identifiability guardrail | `partial` (fixed latent-only two-component engine and component-specific `Gamma_shape` extraction covered; recovery/separation/inference/Psi gates pending) | `test-coevolution-two-kernel.R` (fast C3.1 extraction + C3.2 guardrail; heavy legacy replicated-Psi recovery) | Design 65 C3. Fast evidence now fits two independent named latent kernel tiers and extracts separate shared `Sigma`/`Gamma` blocks by component name. This supports the Paper 2 fixed-Option-B shape syntax, not scientific promotion. Explicit `kernel_unique()` / `*_unique()` Psi is deliberately not part of the Paper 2 multi-kernel first wave because it is a poor default for non-Gaussian and cross-family coevolution models; the current `*_unique()` surface remains compatibility syntax and should be redesigned/deprecated after this arc. No kernel-separation recovery grid, null/selective absence calibration, `rho` profile/estimation, or interval coverage has passed. **C3.2 (legacy guardrail still covered):** when two `kernel_unique` tiers are requested WITHOUT within-species replication, the two-Psi split (`Psi_phy + Psi_non`) is confounded, so `R/fit-multi.R` drops the extra uniqueness covstruct and warns before the multi-kernel gate. |
| COE-04 | Two-component recovery and kernel-separation diagnostics for Paper 2 Option B | `blocked` | planned heavy ADEMP grid | Needs predeclared DGPs for near-orthogonal, moderately overlapping, and highly overlapping kernels; Frobenius kernel-similarity diagnostics; component-specific `Gamma_shape` recovery thresholds; block-diagonal and selective-absence nulls; explicit failure language when component separation is not identified; and a separate post-arc Psi/deprecation grammar plan. Do not promote scientific coverage until this row moves with evidence. |
