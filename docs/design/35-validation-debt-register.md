...
| PHY-18 | `phylo_dep(1 + x \| sp)` augmented full-unstructured slope under **non-Gaussian** families | `partial` (Gaussian `covered`; non-Gaussian reserved) | `test-matrix-slope-phylo-dep.R`, `test-phylo-dep-slope-gaussian.R` | Design 56 §9.5c. The `dep` engine is implemented and Gaussian-validated (`Sigma_b_dep` full `C x C`, `C = 2*n_traits`; recovery + wide/long byte-identity green). The `R/fit-multi.R` guard was converted to an allowlist but holds **gaussian only** (`family_id == 0`): unlike the diagonal `phylo_indep` (PHY-12..16) and block-diagonal `phylo_latent` (PHY-17) paths, the full unstructured covariance was not converging for the non-Gaussian families at the original validation fixtures (small `n_sp`, `n_rep` up to 10) -- every non-Gaussian dep fit returned conv != 0 / non-PD Hessian, so the cells were reserved fail-loud per the #388 discipline. **Reframed 2026-06-02 (identifiability sweep, GAP-B1):** this is **finite-sample power, NOT structural non-identifiability**. A no-engine-change N-sweep (`docs/dev-log/spikes/2026-06-01-phylo-dep-slope-identifiability-N-sweep.R`, Actions runs #26808484707 / #26812586011; after-task `2026-06-02-dep-slope-identifiability-sweep.md`) reaches conv == 0 + PD reliably: poisson at every N (incl. n_sp=80), Gamma/Beta/nbinom2/ordinal_probit by n_sp=300, Bernoulli binomial by n_sp=1200, with the gaussian control PD throughout. Status held at `partial` pending production recovery cells. Close path (Track B, maintainer-sequenced): add per-family recovery cells reading `report$Sigma_b_dep` (NOT the 2-vector `sd_b` channel, which is incompatible with the `dep` engine's `C`-wide `Sigma_b_dep`) at the N the sweep flags, then relax the `c(0L)` family allowlist one family at a time, mirroring PHY-11..17 (ZERO new C++). The current `test-matrix-slope-phylo-dep.R` cells still honest-skip at the converge/PD guard (7/7 skip, 0 fail) until those cells land. |
