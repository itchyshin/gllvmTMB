...
| EXT-31 | `extract_lv_effects()` and `extract_ordination(component = ...)` for predictor-informed latent scores | `partial` | `test-lv-parser-guard.R`; `test-julia-bridge.R` | `extract_lv_effects()` now returns rows for `B_lv = Lambda alpha^T` as the preferred trait-scale table and raw `alpha` only as `type = "axis_effect"` with rotation-dependent status metadata. Trait-scale `std.error` values are copied from TMB's delta-method `ADREPORT(B_lv_unit)` output when `se = TRUE` yields a positive-definite `sdreport()` and are labelled `wald_sdreport_no_ci_validation`. Gaussian Julia bridge `X_lv` fits return point-only `lv_effects`/`alpha_lv` extractor rows with `std.error = NA` and `uncertainty_status = "julia_bridge_point_estimate_only_no_ci_validation"`. `extract_ordination(component = c("total", "innovation", "mean"))` now separates the score entering the linear predictor, the zero-mean innovation, and the predictor-informed mean for ordinary unit-tier TMB fits and narrow Gaussian Julia bridge `X_lv` fits. The row remains partial because interval coverage is not calibration-tested, non-Gaussian Julia bridge `X_lv` rows remain gated, and no recovery/interval grid has moved. |
...
JUL-01 / JUL-01A predictor-informed latent-score addendum (2026-06-25): the
R bridge now admits only the complete-response Gaussian `X_lv` point route for
...
requests stop at `GJL-GATE-XLV-CI`. Non-Gaussian/binary `X_lv`, response masks
