...
| FG-04 | `latent(0 + trait \| unit, d = K)` standalone | `covered` | `test-stage2-rr-diag.R`, `test-keyword-grid.R`, `test-canonical-keywords.R` | Ordinary Psi fold is covered by default `latent()` evidence; scalar common-Psi spelling added 2026-06-18 via `latent(..., common = TRUE)`, objective-equivalent to the legacy paired `latent(..., residual = FALSE) + unique(..., common = TRUE)` form for ordinary intercept-only terms. |
| FG-05 | `unique(0 + trait \| unit)` standalone | `covered` | `test-stage2-rr-diag.R`, `test-cross-sectional-unique.R` | |
| FG-06 | `latent + unique` explicit-Psi compatibility pair | `covered` | `test-stage2-rr-diag.R`, `test-mixed-response-sigma.R`, `test-canonical-keywords.R`, `test-unique-family-deprecation.R` | Compatibility pair remains accepted while ordinary `latent()` now emits Psi by default. Common scalar paired re-home added 2026-06-18: `test-canonical-keywords.R` asserts `latent(..., common = TRUE)` is objective-equivalent to legacy `latent(..., residual = FALSE) + unique(..., common = TRUE)`, estimates one shared ordinary Psi SD, and rejects `residual = FALSE, common = TRUE` plus augmented random-regression `common = TRUE`. Source-specific and `kernel_*()` paired-Psi folds remain future work. |
...
| SPA-02 | `spatial_latent` + spatial unique total covariance | `partial` | `test-keyword-grid.R` (`spatial_latent(unique = TRUE)` parser / random blocks / report / extractor), `test-spatial-latent-recovery.R`, `test-spatial-pair-binary.R`; gap audit `docs/dev-log/audits/2026-07-03-spatial-derived-profile-gap.md` | 2026-07-03 implementation fold: `spatial_latent(..., unique = TRUE)` now keeps both `omega_spde_lv` (shared spatial latent fields) and `omega_spde` (per-trait unique fields) active, reports `Sigma_spde_shared`, `Psi_spde_unique`, and total `Sigma_spde = Lambda_spde Lambda_spde' + diag(Psi_spde)`, and `extract_Sigma(level = "spatial", part = "total")` uses that total covariance. The same row stays `partial`, not `covered`, because the new non-degenerate rank-1 total-correlation check is Gaussian and fast; full source/family profile-interval coverage for spatial total covariance still needs a heavy gate before final Ayumi-style spatial correlation/commonality claims. |
...
| CI-06 | `profile_ci_communality()` (PR #120) | `covered` for unit/unit_obs and phylogenetic-tier routes; calibration separate | `test-profile-ci.R`, `test-derived-phylo-ci-audit.R` | 2026-07-04 adds `tier = "phy"` / `communality:phy:<trait>` routing for phylogenetic-tier communality. Boundary fits may still produce one-sided or unavailable endpoints; empirical interval calibration remains CI-08 / CI-10, not part of this row. |
| CI-07 | `profile_ci_correlation()` (PR #122) | `partial` for source-specific total covariance | `test-profile-ci.R`; spatial smoke in `test-spatial-pair-binary.R` / `test-spatial-depindep-binary.R`; spatial total-covariance target updated in `R/profile-derived.R`; gap audit `docs/dev-log/audits/2026-07-03-spatial-derived-profile-gap.md` | Profile CIs for ordinary/phylogenetic low-rank-plus-diagonal tiers are supported by the existing tests, and spatial low-rank `rho:spatial` has smoke evidence. The 2026-07-03 fold updates the spatial branch so `spatial_latent(unique = TRUE)` correlation targets add `exp(-2 * log_tau_spde)` to the diagonal, matching `extract_Sigma(level = "spatial", part = "total")`. The 2026-07-04 extractor route fix only makes unsupported spatial bootstrap requests fall back to labelled Wald rows; it does not add spatial bootstrap calibration. Status remains `partial` because the full constrained-refit profile inversion for spatial total covariance is too slow for the fast regression slice and still needs a heavy-profile gate before publication-grade interval claims. |
...
| CI-11 | Internal profile route matrix for covariance tiers and structural split targets | `covered` for route-ledger coverage only; interval calibration separate | `test-profile-route-matrix.R`; `docs/design/73-profile-likelihood-route-matrix.md` | 2026-07-04 adds `.profile_route_matrix()` as a truth table across `unit`, `unit_obs`, `cluster`, `cluster2`, `phy`, `spatial`, and augmented split tiers. This row covers the route policy and controlled statuses, not empirical interval calibration. Direct `sd_cluster` / `sd_cluster2` profile labels are covered through `profile_targets()`; `Sigma_cluster` / `Sigma_cluster2` matrix tokens remain planned; augmented split profile routes remain blocked until symbolic targets are declared. |
| CI-12 | Derived profile-curve LR baseline for repeatability, phylogenetic signal, communality, correlation, and variance proportions | `covered` for curve-baseline contract only; calibration separate | `test-profile-derived-curves.R` | 2026-07-04 fixes profile-derived curve `delta_deviance` to use the joint MLE objective as the likelihood-ratio baseline, not the best objective on a finite candidate grid. Correlation and variance-proportion curve objects now also carry refit-inversion bounds so plot/curve summaries stay aligned with the bracket-bisect profile CI path. This row does not promote new source-specific, mixed-family, or empirical coverage claims. |
