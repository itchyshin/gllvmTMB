...
| COE-02 | Validated cross-lineage coevolution engine with `extract_Gamma()` | `covered` | `test-coevolution-recovery.R` (fast extractor tests; heavy recovery/sensitivity gate); `vignettes/articles/cross-lineage-coevolution.Rmd` (buildable internal workflow) | Design 65 C2. `extract_Gamma()` slices the named kernel tier's shared Sigma block. Heavy evidence covers known-`Gamma` recovery on block-missing host/partner data, a block-diagonal zero-`Gamma` null with lower logLik, rotation-invariant Gamma extraction, loading-orientation checks on the fitted dense-kernel recovery fixture, and a sparse-versus-dense single-`W` sensitivity case. The internal article shows paired long/wide syntax, null comparison, fixed-`rho` sensitivity-grid code, and the data-condition warning. Uncertainty intervals and in-engine `rho` estimation remain future claims. |
