...
| SPA-10 | `spatial_dep(1 + x \| site)` augmented full-unstructured 2T×2T field-covariance slope under **non-Gaussian** families | `covered` for gaussian + the families whose VALIDATION cell passes in CI; any family whose cell still skips at the largest tried `n_sites` stays **reserved** | `test-matrix-slope-spatial-dep.R`, `test-spatial-dep-slope-gaussian.R`, `test-spatial-dep-slope-nongaussian.R` | Design 64 §2. The `use_spde_dep_slope` engine (full unstructured `Sigma_field`, `C x C`, `C = 2*n_traits`, built from `theta_spde_dep_chol`; nests under `use_spde_slope`) is implemented and Gaussian-validated (recovery + analytic `Sigma_field (x) Q^{-1}` prior nll to 1e-9 + wide/long byte-identity). **Activated 2026-06-03 (SPA-10):** the spatial analogue of PHY-18 (#422/#424). The retired #425 spike (hand-built precision `Q`, with `solve(Q)` singular / `chol(Q)` "leading minor not positive") produced ZERO usable cells and is RETIRED; the activation instead uses real-API recovery cells on the package's OWN validated Gaussian spatial DGP — the exact stable `backsolve(chol(Q), z)` GMRF draw from the SPA-08 spatial_indep cells (#427), here with a NON-ZERO per-trait intercept/slope cross-field correlation so the recovery asserts the off-diagonal `R[1, 2]` too. `test-spatial-dep-slope-nongaussian.R` adds one `*_VALIDATION` cell per family (poisson 2, Gamma 4, Beta 7, binomial-logit-multitrial 1, nbinom2 5, ordinal_probit 14) reading marginal field SDs + cross-field correlation from `extract_Sigma(level = "spatial")` and the intercept/slope field BLUPs from `omega_spde_aug`, at `n_sites = 400` (cutoff 0.08), escalating where a cell still skips. Per the #388 discipline the `R/fit-multi.R` `use_spde_dep_slope` guard was relaxed from gaussian-only (`c(0L)`) to the CI-trimmed allowlist `c(0L, 1L, 2L, 4L)` — **gaussian, binomial, poisson, Gamma**, the families whose cell passes NON-SKIPPED in `.github/workflows/spatial-dep-slope-nongaussian-recovery.yaml` (first gate run `1 failed, 0 errored, 2 skipped`). **nbinom2 (5) and ordinal_probit (14)** converge PD but recover out-of-band at `n_sites = 400` (honest-skip, partial pending bigger n); **Beta (7)** hits a 0/1 response boundary at construction under the strong dep cross-correlation (a DGP artifact, not identifiability — the strongest follow-up candidate via a clamped response). All three stay reserved fail-loud and their VALIDATION cells honest-skip on the dep guard. The base `spatial_unique`/`spatial_indep` `else if` branch (SPA-08, #427) is UNTOUCHED. `test-matrix-slope-spatial-dep.R` honest-skips at construction for any still-reserved family. The gaussian guard-fires probe in `test-spatial-dep-slope-gaussian.R` now uses `tweedie()` (runtime family id 6, still reserved) since the former poisson probe is now admitted. See `docs/dev-log/after-task/2026-06-03-spatial-dep-slope-nongaussian.md` for the per-family CI outcome table and the `n_sites` each needed. |
