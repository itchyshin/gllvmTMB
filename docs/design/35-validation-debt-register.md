...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| JUL-01A | Julia bridge raw unit-tier covariance and ordination accessors | `partial` | `test-julia-bridge.R`; `docs/dev-log/after-task/2026-06-16-r-bridge-extractor-admission.md`; `docs/dev-log/after-task/2026-06-16-r-bridge-mixed-postfit-admission.md`; `docs/dev-log/after-task/2026-06-16-r-bridge-raw-extractor-payload-matrix.md` | `extract_Sigma()`, `extract_Sigma_B()`, `getResidualCov()`, `getResidualCor()`, `extract_ordination()`, `getLoadings()`, and `getLV()` route for `gllvmTMB_julia` objects on the retained GLLVM.jl engine scale. Pure-R tests cover fake payloads, fallback `Lambda Lambda^T` reconstruction, derived correlations, default argument forcing, transposed score payloads, bad Sigma/correlation/loading/score/family gates, structured-level gates, rotation gates, and mixed-family raw accessor admission; live R tests cover grouped-dispersion, ordinal-probit, and mixed-family rows through `GLLVM.jl-integration`, including retained Sigma/correlation equality, labels, finite loadings/scores, symmetry, and unit-diagonal correlations. Still partial: `unit_obs`, structured tiers, augmented slopes, cluster tiers, link-residual augmentation, rotated ordinations, interval-bearing extractors, and broad/native extractor parity remain gated. |
