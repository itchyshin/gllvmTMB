...
| ID | Capability | Status | Test evidence | Notes |
|----|------------|--------|---------------|-------|
| JUL-01 | Lean `engine = "julia"` reduced-rank bridge through `GLLVM.bridge_fit` | `partial` | `test-julia-bridge.R`; `docs/dev-log/2026-06-16-nb1-gamma-bridge-parameterisation-audit.md` | Main-dispatch route admits one `latent(..., d = K)` block on complete balanced trait x unit data, with Gaussian fixed-effect covariates only and no structured covariance terms. The 2026-06-16 grouped-dispersion slice maps R `nbinom2()` / `nbinom1()` / `Beta()` / `Gamma()` to the paired Julia grouped-dispersion payload, labels `dispersion`, `dispersion_group`, and `dispersion_group_id`, and adds public-scale fields for NB2 (`sigma = 1/sqrt(r)`, `phi = 1/r`), NB1 (`phi` identity), Beta (`sigma = 1/sqrt(phi)`), and Gamma (`sigma = 1/sqrt(alpha)`). The grouped-dispersion main-dispatch smoke exercises all four rows through `gllvmTMB(..., engine = "julia")`; NB2 and Beta carry selected small-fixture native objective parity against `engine = "tmb"`. NB1 is parameterisation-aligned with native `phi_nbinom1` (`Var = mu * (1 + phi)`), and `test-julia-bridge.R` now includes a fixed-parameter kernel check: Julia `nb1_grouped_marginal_loglik_laplace()` with zero loadings matches the native linear-variance `dnbinom(mu = mu, size = mu / phi)` likelihood to `1e-10`. NB1 still needs fitted-object objective evidence before parity promotion. Gamma is route/shape evidence only: Julia grouped Gamma estimates per-trait/grouped `alpha`, while current native ordinary Gamma uses shared scalar `sigma_eps` as the coefficient of variation. A maintainer decision is required before any Gamma native-parity claim: route bridge Gamma through shared grouping for the current oracle, or expand native R/TMB Gamma to per-trait CV/shape. The paired ordinal slice labels per-trait ordinal cutpoint matrices, per-trait category counts, `cutpoint_mode`, and `cutpoint_link`, while marking ordinal bridge CIs unavailable. Still partial: broad native-vs-Julia parity rows, grouped-dispersion CI endpoints, per-trait ordinal CI endpoints, response masks, non-Gaussian X through the main dispatch, mixed-family promotion, structured terms, and post-fit extractor parity remain planned/gated. |
