...
| FG-18 | `latent(..., lv = ~ x)` predictor-informed latent-score grammar | `partial` | `test-lv-parser-guard.R` | Ordinary unit-tier `latent(..., lv = ~ x)` now reaches C1 fits for Gaussian and pure binomial-probit responses: the parser stores `extra$lv_formula` on the reduced-rank term only, keeps the auto-added `Psi` companion free of `lv` metadata, builds unit-level `X_lv_B`, wires `alpha_lv_B` into the TMB score mean, and reports total/mean/innovation score components plus `B_lv_unit`. Long and `traits(...)` wide Gaussian surfaces are covered by smoke tests; binomial-probit has a long-form multi-trial recovery/algebra gate. This row remains partial because no Gaussian recovery grid, missing-response compatibility, interval calibration, Bernoulli single-trial depth, other non-Gaussian family/link support, tier-expanded support, structured-source support, or Julia parity is admitted. Random terms, offsets, `mi()`, smooths, response/trait columns, nonconstant-within-unit predictors, rank-deficient designs, exact fixed/LV overlap, `REML = TRUE`, unsupported non-Gaussian families/links, unsupported tiers, augmented random-regression combinations, ordinary diagonal aliases, source-specific forms, and kernel forms still reject until their rows move. |
...
| RE-13 | Ordinary unit-tier predictor-informed latent-score mean `latent(..., lv = ~ x)` | `partial` | `test-lv-parser-guard.R` | Design 73 C1 support is live for ordinary unit-tier Gaussian and pure binomial-probit fits. The implemented model is `u_i = X_lv,i alpha + e_i`, `e_i ~ N(0, I_K)`, with the ordinary `latent()` `Lambda Lambda^T + Psi` decomposition preserved where identifiable and `B_lv = Lambda alpha^T` reported as the primary trait-scale estimand. Evidence currently covers parser preflight, long/wide Gaussian smoke fits, finite TMB reports, extractor dimensions/algebra, and a pure binomial-probit multi-trial `B_lv` recovery/algebra gate. The row remains partial until Gaussian recovery of `B_lv`, `Sigma`, and `Psi`, Bernoulli single-trial binary depth, and interval calibration land; `REML = TRUE`, unsupported non-Gaussian families/links, unsupported tiers/sources, and augmented random-regression combinations remain rejected. |
...
| LV-05 | Non-Gaussian predictor-informed latent scores | `partial` | `test-lv-parser-guard.R` | Pure `binomial(link = "probit")` ordinary unit-tier `latent(..., lv = ~ x)` is admitted on the same score-mean engine as Gaussian: the gate verifies family/link IDs, finite reports, `total = innovation + mean`, and trait-scale `B_lv = Lambda alpha^T` recovery on a small multi-trial fixture. This is a narrow binary-probit point/recovery gate only: Bernoulli single-trial depth, binary logit/cloglog, ordinal, count, Gamma, Beta, mixed-family, delta/hurdle, interval calibration, and CI-08/CI-10 coverage remain blocked until their own family-specific recovery and link-scale diagnostics land. |
