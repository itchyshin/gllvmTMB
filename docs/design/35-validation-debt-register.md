...
| EXT-26 | `plot_Sigma_comparison()` estimate-vs-truth plot helper | `covered` | `test-plot-covariance-tables.R` | Plots `compare_Sigma_table()` rows as row-labelled error plots or estimate-vs-truth scatter plots; visual comparison helper only, no simulation or calibration claim. |
...
| DIA-08 | `check_gllvmTMB(fit)` machine-readable fit-health table | `covered` | `test-sanity-multi.R` | Design 49. Returns stable rows for optimizer convergence, gradient, `sdreport()` availability, `pdHess`, fixed-effect SEs, restart provenance, selected restart, and simple boundary flags. Treats `pdHess = FALSE` as an inference / identifiability warning rather than automatic model failure. |
...
| DIA-11 | Fitted-model predictive diagnostic plots | `partial` | `test-ppcheck-diagnostics-prototype.R` | Design 51. Non-exported prototype under `inst/prototypes/` returns `ggplot` objects with attached diagnostic data for Gaussian, Poisson, and NB2 fits. Not exported; not a Bayesian posterior-predictive claim. |
| DIA-12 | Simulation-rank residual prototype | `partial` | `test-ppcheck-diagnostics-prototype.R` | Design 51. Non-exported simulation-rank residual table keeps non-finite rows with explicit status. Exact family-CDF randomized-quantile residuals remain future work. |
...
| MIS-22 | Morphometrics cached bootstrap correlation fixture | `covered` | `test-example-morphometrics.R` | Ships `inst/extdata/examples/morphometrics-bootstrap-r.rds` for article rendering and visual QA of `plot_correlations(bootstrap_Sigma, style = "raindrop")` and `plot(type = "correlation_ellipse", boot = boot)`; the fixture is not interval-calibration evidence. |
