...
| Dimension-aware ordination | Partial: 1D strip, 2D biplot, 3D pair grid, d > 3 selected axes, varimax default, axis ordering, user-supplied sign anchors, and standardized loading arrows are implemented. | Interactive 3D remains planned; rotation/sign conventions need continued tutorial guidance and visual snapshots. |
