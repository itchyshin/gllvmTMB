...
comparator rows for four ordinary Gaussian `B_lv` cells. The same dev
harness now has launch-ready pure-binomial logit/probit/cloglog cells, but no
binomial interval coverage result has been collected or promoted. Missing
`lv` predictors, non-Gaussian response masks, binomial interval calibration,
