...
| `spatial_dep` slope | yes (augmented `1 + x`; full unstructured 2T×2T field cov) | yes (`use_spde_slope` / `use_spde_dep_slope`) | **C** Gaussian (test-spatial-dep-slope-gaussian.R); **R** non-Gaussian (RESERVED gaussian-only `c(0L)`; all 7 non-Gaussian non-PD at n=100 — identifiability, the spatial analogue of phylo_dep/PHY-18) | non-Gaussian dep-slope identifiability (bigger n / reparam) |
| `spatial_latent` slope | yes (augmented `1 + x`, `d = K`) | yes (`use_spde_latent_slope`, separate block) | **C** Gaussian + binomial-probit/poisson/Gamma/Beta (PR, allowlist `c(0L,1L,2L,4L,5L,7L,14L)`); **R-fixture** binomial-logit/ordinal_probit/nbinom2 (converge + PD at alt seed/n=150, honest-skip at default fixture) | wider-n recovery for the 3 fixture-skip families |
...
| `spatial_dep` slope | augmented `1+x` parsed; full unstructured 2T×2T field cov | done (`use_spde_slope` / `use_spde_dep_slope`) | yes | **RESERVED gaussian-only** (R/fit-multi.R `use_spde_dep_slope` guard, `c(0L)`; all 7 non-Gaussian non-PD at n=100, the spatial analogue of phylo_dep) | test-spatial-dep-slope-gaussian.R (recovery + analytic prior nll, 1e-9); test-matrix-slope-spatial-dep.R (7 honest-skip) |
| `spatial_latent` slope | augmented `1+x`, `d = K`, own engine block | done (`use_spde_latent_slope`) | yes | **ACTIVATED** across families (R/fit-multi.R `use_spde_latent_slope` allowlist `c(0L,1L,2L,4L,5L,7L,14L)`, ZERO new C++); 4 covered + 3 honest-skip (PD at alt seed/n=150) at the default fixture | test-spatial-latent-slope-gaussian.R (recovery); test-matrix-slope-spatial-latent.R (24 pass, 0 fail) |
