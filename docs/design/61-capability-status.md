...
| Predictor-informed latent scores (`latent(..., lv = ~ x)`) | Design 73 is the source-of-truth spec. Ordinary Gaussian unit-tier `lv` formulas now validate, build `X_lv_B`, fit through the C1 TMB path with `alpha_lv_B`, report `B_lv_unit`, and expose extractor/ordination components. `extract_lv_effects()` returns trait-scale `B_lv` SEs from `ADREPORT(B_lv_unit)` only when `se = TRUE` yields a positive-definite `sdreport()`. Pure binomial logit/probit/cloglog is admitted on the same ordinary unit-tier path with trait-scale `B_lv` recovery/algebra evidence. This is still partial: no interval calibration, no Bernoulli single-trial depth, no other non-Gaussian families, and no Julia bridge parity. | Next slice is Gaussian recovery for `B_lv`, `Sigma`, and `Psi`, plus Bernoulli binary depth and missing-response/factor-runtime smoke before any broader docs article. |
