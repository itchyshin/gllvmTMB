...
- [x] **C3.3 -- first COE-04 recovery/separation gate:** a heavy Gaussian latent-only DGP now covers the near-orthogonal case. The fixture uses two named kernels with low off-diagonal Frobenius-style similarity, fits `kernel_latent(..., name = "phy") + kernel_latent(..., name = "non")`, verifies that the full model beats either one-component model, and checks that component-specific `extract_Gamma()` estimates recover their own `Gamma_shape` truths while not matching the other component. **Still partial:** moderate/high-overlap kernels, selective-absence/null calibration, explicit unidentified-separation language, `rho` profiles/estimation, intervals, mixed/non-Gaussian gates, and the post-arc `*_unique()` lifecycle/deprecation plan remain future work.
...
- **C3:** fixed two-kernel fit + two-Psi guardrail; STOP if two uniqueness tiers are requested without replication. Near-orthogonal two-component recovery has first heavy evidence, but scientific promotion still waits for the broader recovery/separation/null/interval grid (`COE-03` partial, `COE-04` partial).
