...
- **C3:** fixed two-kernel fit + two-Psi guardrail; STOP if two uniqueness tiers are requested without replication. Near-orthogonal two-component recovery, two-cell moderate-overlap recovery plus one harder moderate-edge boundary cell, high-overlap duplicate and near-duplicate collapse-equivalence, two-sided selective absence, block-null smoke plus a 12-seed null diagnostic and medium-signal grid, bounded Poisson construction fixtures plus a two-cell Poisson recovery gate, fixed-`rho` profile/sensitivity support via `profile_cross_rho()`, fixed-`rho` component `Gamma_effect` extraction, pair-specific point covariance via `predict_cross_covariance()` including real-fit retained `make_cross_kernel()` metadata, and high-overlap fit/extraction warning language have evidence, but scientific promotion still waits for the broader recovery/separation/null/interval grid (`COE-03` partial, `COE-04` partial).
