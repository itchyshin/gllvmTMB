...
- [x] **C3.3 -- first COE-04 recovery/separation gates:** heavy Gaussian latent-only DGPs now cover the near-orthogonal two-component case and a first near-orthogonal selective-absence case. The two-component fixture uses two named kernels with low off-diagonal Frobenius-style similarity, the fitted object stores the pairwise diagnostic in `fit$kernel_diagnostics`, fits `kernel_latent(..., name = "phy") + kernel_latent(..., name = "non")`, verifies that the full model beats either one-component model, and checks that component-specific `extract_Gamma()` estimates recover their own `Gamma_shape` truths while not matching the other component. The selective-absence fixture sets the non component's loadings to zero; the two-kernel fit recovers the present phy `Gamma_shape`, collapses the absent non `Gamma_shape` below `1e-3`, and does not materially improve over the present-only model. **Still partial:** moderate/high-overlap kernels, block-null calibration, explicit unidentified-separation language, `rho` profiles/estimation, intervals, mixed/non-Gaussian gates, and the post-arc `*_unique()` lifecycle/deprecation plan remain future work.
...
- **C3:** fixed two-kernel fit + two-Psi guardrail; STOP if two uniqueness tiers are requested without replication. Near-orthogonal two-component recovery and a first near-orthogonal selective-absence case have heavy evidence, but scientific promotion still waits for the broader recovery/separation/null/interval grid (`COE-03` partial, `COE-04` partial).
