...
- [x] **C2.3 -- fixed-`rho` sensitivity + shape/effect extraction:** since
  ...
  interval evidence.
- [x] **C2.4 -- identifiability constraints + single-`W` sensitivity (literature-mandated; scout):** (a) ensure `Gamma` is uniquely identified, not rotation/reflection-ambiguous -- `Lambda_H` lower-triangular with positive diagonal (**the parenthetical assumption that gllvmTMB's existing loadings constraint already provides this was FALSE -- corrected 2026-08-03; see the OPEN ITEM below**). (b) Add a sensitivity sim varying **association richness** (sparse vs dense `W`, number of realised host-partner links): show recovery degrades and uncertainty needs sensitivity assessment as `W` thins, because a single shared `W` is only one replicate of the coevolution signal (Boettiger et al. 2012). C2 engine tests verify the loading orientation and sparse-versus-dense single-`W` degradation; `?extract_Gamma` and `vignettes/articles/cross-lineage-coevolution.Rmd` carry the data-condition warning and internal workflow.
  - **OPEN ITEM (C2, 2026-08-03) -- the reflection half of C2.4(a) is UNMET by default.** Verified in the engine: `src/gllvmTMB.cpp:902` reads the reduced-rank loadings diagonal as a plain value (`vector<Type> lam_diag = theta_rr_B.head(rank);`) and `cpp:909` writes it unchanged (`Lambda_B(i, j) = lam_diag(j);`) -- no `exp()`, no `fabs()`, no lower bound -- while the lower-triangular ZEROS **are** implemented (`cpp:906-907`). The two indeterminacies therefore have different status: **rotation** (continuous, `d(d-1)/2`) is fixed by the triangular zeros; **reflection** (discrete, `2^d` sign modes) is NOT, because the paired flip `(Lambda_.k, z_.k) -> (-Lambda_.k, -z_.k)` is an exact invariance of the joint density. At `d = 1` the triangular constraint is vacuous and sign is the whole story. Note also that the existing C2/C3 loading-orientation and `Gamma_shape` gates compare with `abs(stats::cor(...))` (`tests/testthat/test-coevolution-recovery.R:165`, `test-coevolution-two-kernel.R:102`), so they are orientation-agnostic and are NOT evidence of a sign-identified `Gamma`. The remedy is available and opt-in, not missing: `lambda_constraint = list(B = diag(1, n_traits, d))` pins the leading loading of each factor to 1 (`R/lambda-constraint.R`, the galamm-style pattern; the same mechanism `gllvm` applies by default), which fixes sign as well as rotation. **Decision needed by whoever runs C2 -- not taken here:** whether a `Gamma` slice that claims identification must REQUIRE that constraint explicitly rather than assume the default supplies it, and if so for `Lambda_H`, `Lambda_P`, or both -- weighing that `diag(1, ...)` is also a SCALE constraint and so changes the meaning of the latent scores. The maintainer decision of 2026-08-03 is that the engine default does NOT change; this is a documentary correction plus an open design call for C2.
...
- [x] **C3.1 -- fit two kernel tiers** (`name="phy"` with `K*_phy`, `name="non"` with `K*_non = scale(W)` tip-level); confirm both tiers' `Sigma` + `Gamma` extract by `name`. **First fixed multi-kernel wave landed (2026-06-18):** two or more named dense `kernel_latent()` tiers now activate a generic TMB block with per-tier `K_r`, `Lambda_r`, and latent field. The one-name `kernel_*()` path remains on the C1 phylo-equivalent engine, preserving the `<1e-6` KER-02 equivalence gate. `test-coevolution-two-kernel.R` fits two named latent tiers and extracts component-specific shared `Sigma`, `Gamma_shape`, and, when the tier was built from `make_cross_kernel()`, fixed-`rho` `Gamma_effect = rho_r * Gamma_shape_r`. The fitted object now stores the aligned fixed `K_r` matrices by component name so `predict_cross_covariance()` can return pair-specific point covariance as `Gamma_shape_r * K_r[i, j]` without asking the user to re-supply `K`; real fitted `make_cross_kernel()` matrices retain host/partner metadata and fixed `rho` after level alignment, so omitted `row_levels`/`col_levels` and `kernel_includes_rho` are covered by the fast regression gate. This Paper 2 path is deliberately latent-only in the first wave: explicit multi-kernel-level Psi remains deferred because it is a poor default for non-Gaussian and cross-family coevolution models. The 2026-07-03 explicit Psi arc covers the single-tier spelling `kernel_latent(..., unique = TRUE)` and keeps `kernel_unique()` as compatibility syntax; it does not promote multi-kernel Psi. This is an engine and extractor gate, not scientific promotion: `COE-03` remains partial until component recovery, kernel-separation diagnostics, null/selective-absence tests, `rho` profiling/estimation, interval calibration, and multi-kernel Psi decisions pass.
...
- [x] **C3.3 -- first COE-04 recovery/separation gates:** heavy Gaussian latent-only DGPs now cover the near-orthogonal two-component case, a near-orthogonal fixed-effect/shared-Sigma magnitude gate, a three-cell moderate-overlap grid plus one harder moderate-edge claim-boundary cell, high-overlap collapse-equivalence cases plus a non-identical high-overlap failure-calibration cell, selective absence in both directions, a block-null smoke case, a 12-seed null diagnostic, empirical 95% null-tail threshold scaffold, and medium-signal grid, bounded Poisson construction fixtures plus a two-cell Poisson recovery gate, a mixed-family Gaussian+Poisson construction smoke plus a two-cell mixed-family recovery gate, pre-fit kernel separability screening via `diagnose_kernel_separability()`, a deterministic pre-fit kernel-collinearity simulation gate, fixed-`rho` sensitivity via `profile_cross_rho()`, fixed-`rho` effect-scale extraction, pair-specific point covariance extraction, and point-estimate coevolutionary module extraction via `extract_coevolution_modules()`. The two-component fixture uses two named kernels with low off-diagonal Frobenius-style similarity, the fitted object stores the pairwise diagnostic in `fit$kernel_diagnostics`, and the pre-fit `diagnose_kernel_separability()` helper applies the same claim-boundary logic to candidate kernels before fitting. The raw-versus-residualized candidate gate marks an aliased raw `K_tip` as `high` overlap with recommendation `collapse_or_single_covariance`, while a residualized/opposed candidate is `near_orthogonal` with recommendation `separable_candidate`. The deterministic collinearity gate then blends the residualized `K_tip` candidate toward `K_phy`, verifies monotone off-diagonal similarity, and requires the claim classes to move from `near_orthogonal` to `moderate` to `high` with recommendations `separable_candidate`, `sensitivity_required`, and `collapse_or_single_covariance`. The model tests fit `kernel_latent(..., name = "phy") + kernel_latent(..., name = "non")`, verify that the full model beats either one-component model, and check that component-specific `extract_Gamma()` estimates recover their own `Gamma_shape` truths while not matching the other component. A centered-latent companion gate recovers planted trait intercepts within 0.20 and checks each component's shared `Sigma` Frobenius magnitude plus host, partner, and cross-block shape against `Lambda_r Lambda_r^T`; the cross-block shape check uses absolute correlation because the block-missing host/partner design leaves cross-lineage orientation ambiguous. For cross-kernel tiers with recorded `rho`, `extract_Gamma(scale = "effect")` returns `rho_r * Gamma_shape_r` for each component, while `predict_cross_covariance()` uses `Gamma_shape_r * K_r[i, j]` because `K_r[i, j]` already contains the fixed `rho` supplied to `make_cross_kernel()`. `extract_coevolution_modules()` now standardizes a component-specific cross-lineage block as `R = Sigma_H^{-1/2} Gamma Sigma_P^{-1/2}` and returns singular-value axes for coupled host/partner trait modules. The near-orthogonal heavy gate compares the returned `R` matrices and singular values against an independent test-side inverse-square-root/SVD calculation, verifies `n_modules` truncation, and confirms `scale = "effect"` scales the module matrix and singular values by the fixed recorded `rho`. This module helper is point-estimate output only; it does not provide uncertainty, rank selection, `rho` estimation, or null-threshold calibration. The moderate grid blends the non association pattern 25%, 30%, and 35% toward the phy pattern, lands all three cells in the moderate overlap class, and still recovers both component `Gamma_shape` matrices. A tested 40% moderate-edge cell converges and strongly beats either one-component model, but it fails the stricter component-separation thresholds (`Gamma_phy` recovery below 0.95 and cross-component match above 0.25), so it is recorded as a boundary rather than promoted evidence. The high-overlap fixtures cover an exact-duplicate kernel pair and a diagonal-shrink near-duplicate pair, verify the separated two-tier fit is not materially better than one collapsed rank-2 kernel tier, and keep component-specific `extract_Gamma()` warning while the collapsed extraction stays quiet and finite. A separate non-identical high-overlap fixture mixes `K_non` 85% toward `K_phy` before simulation (`similarity = 0.999876`), still detects two-component signal over either one-component comparator, but fails promoted separation (`Gamma_shape` own correlations 0.64 and 0.87, with one cross-component match 0.52), so it is failure-calibration evidence only. The selective-absence fixture sets either component's loadings to zero; the two-kernel fit recovers the present `Gamma_shape`, collapses the absent component below `1e-3`, and does not materially improve over the present-only model. The block-null smoke fixture sets both loading blocks to zero and verifies both extracted component `Gamma_shape` norms collapse below `1e-3`; the 12-seed null diagnostic keeps null component norms bounded near zero and records the full-vs-intercept overfit tail (median below `2`, at most two seeds above `3`, maximum below `8` log-likelihood units, empirical 95% tail marker below `8`), while the paired medium-signal fixtures recover both component `Gamma_shape` blocks, strongly beat one-component fits, and clear that fixed-grid tail marker by more than `10x`. The Poisson evidence now has two layers: construction smokes fit bounded count fixtures with the latent-only two-kernel syntax and finite component-specific `Gamma` point blocks, while two known-DGP recovery cells (`seed = 2801` and `2804`) recover both planted Poisson log-link component `Gamma_shape` blocks with correlations above `0.98`, beat either one-component Poisson comparator by more than `40` log-likelihood units, and keep cross-component matches below `0.10`. Mixed-family evidence also now has two layers: a construction smoke fits long-format Gaussian host traits plus Poisson partner traits with per-row family dispatch and finite component shared-Sigma/Gamma blocks, while two known-DGP Gaussian-host/Poisson-partner recovery cells (`seed = 2912` and `2913`) converge for full and one-component comparators, recover both planted component `Gamma_shape` blocks with correlations above `0.90`, beat either one-component comparator by more than `200` log-likelihood units, and keep cross-component matches below `0.12`. This remains narrow shape-recovery evidence only, not interval calibration or broad heterogeneous-trait coverage. `profile_cross_rho()` rebuilds the phy component over `rho = c(0, 0.25, 0.55, 0.85)` while holding the non component fixed; positive-`rho` fits strongly beat `rho = 0`, but the best grid point can sit at the high edge, so this is fixed-kernel profile/sensitivity evidence only, not in-engine `rho` estimation or profile-interval coverage. High-overlap fitted tier pairs now warn that component-specific `Gamma_shape` separation is weak evidence, and `extract_Gamma(level = ...)` repeats the warning when the requested component participates in a high-overlap pair. **Still partial:** harder moderate-overlap calibration beyond the promoted 0.25-0.35 grid, broader high-overlap truth-recovery/failure calibration beyond the collapse-equivalence, non-identical failure-calibration, and warning gates, formal reusable null-threshold / Type-I calibration beyond this diagnostic scaffold, in-engine `rho` estimation, `rho` profile intervals, broader interval coverage, module uncertainty/rank calibration, broader non-Gaussian/mixed-family recovery gates beyond the narrow Poisson and mixed-family cell pairs, and the post-arc `*_unique()` lifecycle/deprecation plan remain future work. On in-engine `rho` specifically, the 2026-06-21 decision (`docs/dev-log/decisions.md`) is to keep fixed-`rho` profiling and NOT add a TMB-estimated `rho` in this arc, because identifiability is fragile on a single shared `W`; the design note (`docs/dev-log/2026-06-20-coevolution-in-engine-rho-design.md`) records the rationale and the zero-engine pure-R identifiability simulation that would gate any future promotion.

**C3.3 reciprocal-dependence addendum (2026-06-19):** a fast
`COE-04` diagnostic gate now builds
`W_recip = sqrt(p(j|i) p(i|j))` from deterministic link counts. The raw
reciprocal-dependence tip kernel lands in the `moderate` overlap class
with recommendation `sensitivity_required`; a residualized-plus-opposed
tip candidate lands in the `near_orthogonal` class with recommendation
`separable_candidate`. This directly supports the Paper 2 rule that raw
network kernels must pass a separability screen before phylogenetic
versus tip-level component claims are advertised. It is diagnostic
evidence only, not fitted recovery, interval calibration, in-engine
`rho` estimation, or a causal coevolution claim.
...
- **C3:** fixed two-kernel fit + two-Psi guardrail; STOP if two uniqueness tiers are requested without replication. Near-orthogonal two-component recovery, three-cell moderate-overlap recovery plus one harder moderate-edge boundary cell, high-overlap duplicate and near-duplicate collapse-equivalence, non-identical high-overlap failure calibration, two-sided selective absence, block-null smoke plus a 12-seed null diagnostic, empirical 95% null-tail threshold scaffold, and medium-signal grid, bounded Poisson construction fixtures plus a two-cell Poisson recovery gate, a mixed-family Gaussian+Poisson construction smoke plus a two-cell mixed-family shape-recovery gate, pre-fit kernel separability screening via `diagnose_kernel_separability()`, fixed-`rho` profile/sensitivity support via `profile_cross_rho()`, fixed-`rho` component `Gamma_effect` extraction, pair-specific point covariance via `predict_cross_covariance()` including real-fit retained `make_cross_kernel()` metadata, point-estimate module extraction via `extract_coevolution_modules()` with an independent standardized-SVD oracle, and high-overlap fit/extraction warning language have evidence, but scientific promotion still waits for the broader recovery/separation/null/interval grid (`COE-03` partial, `COE-04` partial).
- **C3 addendum:** the pre-fit separability screen includes a
  reciprocal-dependence raw-vs-residualized `W` gate; the raw
  degree-normalized network requires sensitivity, while the residualized
  candidate is separable under the fixed thresholds.
