# Design 66 -- Capstone power / accuracy / coverage simulation study (DRAFT)

**Status:** DRAFT design contract. Pre-spec for the end-of-road capstone
(issue #349, milestone `power-study`). NOT yet approved; open questions in
section 12 require a maintainer decision before this becomes a build
contract.
...
~50-cell spoke design) is a maintainer decision (section 12, Q-b).
...
## 12. OPEN QUESTIONS for the maintainer (decide before freezing)

- **Q-a (compute).** Is an HPC cluster (or a large cloud spot allocation)
  available for the production sweep, or is this GHA-only? The whole budget
  plan (section 8) branches here. GHA-only forces a drastic core-cell /
  n_boot / n_sim cut.
- **Q-b (core vs full).** Is the core the ~50-cell backbone-plus-spokes
  fraction, or the full 192-cell cross (section 4.2)? This is the single
  biggest cost lever after compute.
- **Q-c (headline claims).** Are H1-H4 (section 2) the exact claims the
  paper/CRAN will make? In particular: is the gate 94 % (the existing
  audit-1 threshold) or strict 95 % nominal? The grid must be sized to
  whichever is chosen.
- **Q-d (n_sim).** MCSE gives a floor of 2000 (0.49 pp) for gate
  adjudication, 1000 (0.69 pp) as the minimum defensible. Which, given the
  budget ceiling?
- **Q-e (coevolution / Gamma).** Is Design 65 / #361 (`kernel_*()`, Gamma
  coevolution) in scope for the capstone, or deferred? Default in this
  draft: deferred (the engine is not built on origin/main).
- **Q-f (families).** Core 4-family representative subset (gaussian,
  nbinom2, binomial-probit, ordinal-probit), or all 14 wired families?
  Including nbinom1 (FAM-07, review-branch-wired) and mixed-family
  (CI-10, worst-calibrated) materially changes scope and risk.
- **Q-g (signal parametrization).** Define "moderate" and "strong"
  signal operationally per RE source (a variance ratio? a phylo/spatial
  signal lambda?) so the power curve's x-axis is interpretable. Proposed:
  a between-unit variance share of total, but the maintainer should fix
  the metric.
