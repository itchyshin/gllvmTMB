...
- `phy_unique_slope`: block-local 2 by 2 `(intercept, slope)` source random
  effect assembled from `report$sd_b` and `report$cor_b`.
...
- `spde_base_slope`: block-local 2 by 2 `(intercept, slope)` SPDE field
  covariance assembled from `report$sd_spde_b` and `report$cor_spde_b`.
...
| `phy_unique_slope` | `extract_Sigma(level = "phy")` returning `level = "phy_unique_slope"` | `D R D` 2 by 2 block from `sd_b` and `cor_b` | the single intercept-slope correlation | not applicable; no shared-loading numerator | not public yet: block trace or coefficient denominator must be declared |
...
| `spde_base_slope` | `extract_Sigma(level = "spatial")` returning `level = "spde_base_slope"` | 2 by 2 SPDE field covariance from `sd_spde_b` and `cor_spde_b` | the single intercept-slope field correlation | not applicable; no shared-loading numerator | not public yet: field-scale or marginal-scale denominator must be chosen |
...
5. Boundary handling is explicit for variance near zero and correlations near
   `+/-1`.
6. Mission Control and the validation register move only the tested route, not
   the whole augmented family.
...
- Do not add broad non-Gaussian or source-specific profile claims until the
  Gaussian canary and boundary gate pass.
