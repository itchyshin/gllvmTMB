...
Retain Laplace; do not run `q = 4/6` or reopen
...
> as an already-admitted region. The nearest thing to Gate-3 evidence now on record
> is `dev/va-usability/A2-ATTENUATION.md` (3 families x 2 n x 50 seeds, with a
> Laplace control) -- informative, and explicitly **not** a Gate-3 run: it is not
> pre-registered, reports no MCSE-backed pass/fail, and covers `p = 8` only.
