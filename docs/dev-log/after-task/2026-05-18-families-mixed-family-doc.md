...
- Added a short `\dontrun{}` mixed-family example that demonstrates the
  selector column and list-of-families usage (including an explicit
  selector-level ordering that matches the family list) without requiring
  a CI-time model fit.
...
- Other `man/*.Rd` files regenerated by roxygen (output drift only)
...
- `docs/dev-log/recovery-checkpoints/2026-05-19-030922-codex-checkpoint.md`
...
- The local shell cannot resolve `github.com`, so the branch cannot be
  pushed and CI cannot be triggered from this environment yet.
- The GitHub integration available to Codex in this sandbox appears
  read-only (`403 Resource not accessible by integration` when trying to
  create a branch ref), so it cannot be used to open the PR as a
  workaround for the DNS outage.
