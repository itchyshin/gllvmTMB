...
- `covariance-correlation` still opens with roughly 5,400 rendered characters of exposition (the "From model to Sigma" section, two display equations, the six-row object table) before `library(gllvmTMB)` at line 131 and the first fit at line 180. The question is in sentence one and the limits sentence is on the first screen, but the first screen does not open with a route the reader can paste. This page structure predates the branch; the branch moved the two `eval = FALSE` sketches out of the top but did not reorder the sections. Not closed here. A follow-up slice would move "A side-by-side demonstration" (library, loader, fits, log-likelihood check, reading sentence) above "From model to Sigma".

## 10a. Review pass (2026-09-21)

Two SHOULD-FIX findings from the review of PR #1318, each with its disposition.

- Applied. `model-selection-latent-rank.Rmd`, after the candidate table: the new sentence told the reader to read `health` first, and the `d = 2` row it goes on to select shows `health WARN` with nothing on the page saying why. Re-measured from the purled route in a clean `Rscript --vanilla` session: `check_gllvmTMB(fits$d2)` has one non-PASS row, `rotation_convention_unit WARN rotation_ambiguous` ("Lambda_B is identified up to rotation/sign convention"); `check_gllvmTMB(fits$d3)` has three, `pd_hessian WARN FALSE`, the same rotation note, and `weak_axis_unit WARN min=0.0471`; `d1` and `d0` have none. Added two sentences naming the `d = 2` source as the rotation-convention note, not a numerical failure, and saying the `d = 3` row carries that note too and also warns on `pd_hessian` and `weak_axis`, which the page prints further down. The reviewer's suggested wording named only `pd_hessian` and `weak_axis` for `d = 3`; the measured row set includes the rotation note as well, so the sentence says so.
- Applied as a recorded residual, not a reorder. `covariance-correlation.Rmd`: the runnable route arrives after roughly 5,400 rendered characters of exposition. The reviewer offered either a follow-up slice that moves the demonstration section above "From model to Sigma" or an explicit residual. Chose the residual (Known Residuals above): the reorder moves the page's teaching model below its code, which is a page-structure decision outside a first-route repair and outside this brief; recording it keeps the gap visible rather than read as closed.

Gates re-run after the edit, from the worktree: `git diff --check` exit 0; `bash tools/check-reader-surface.sh` `READER-SURFACE CHECK: PASS`, exit 0; `NOT_CRAN=true` `test-get-started-reader-route.R` `[ FAIL 0 | WARN 0 | SKIP 0 | PASS 54 ]`; `test-article-prescribed-calls.R` `[ FAIL 0 | PASS 5 ]`; `test-reader-facing-no-register-codes.R` `[ FAIL 0 | PASS 8 ]`; `pkgdown::build_article("articles/model-selection-latent-rank", lazy = FALSE)` into a fresh scratch destination, 20 s, exit 0, no `#> Error`, both figure chunks (`ic-plot`, `sigma-scatter`) rendered. The earlier "3 images" count for this page in section 5 was the two figures plus the site logo; the source has two figure chunks. `covariance-correlation.Rmd` text did not change, so it was not re-rendered.
