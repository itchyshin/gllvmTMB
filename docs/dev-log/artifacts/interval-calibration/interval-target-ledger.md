...
| --- | --- | --- | --- |
| CI-08 | `V_t = Sigma_unit[t,t] = (Lambda Lambda^T)[t,t] + psi_t^2`; intended two-sided 95% LR profile on `log(V_t)` | The historical certificate remains restricted to Gaussian ordinary unit tier, `n_units=150`, `d=1/2`. PVT-02 at `n_units=400,d=2` passed its numerical gates for traits 1 and 2, but the retained penalty-profile endpoints cannot verify constrained-refit convergence or exact target attainment. | historical exact cells certified; PVT-02 blocked; limited elsewhere |
...
| CI-13 | Confirmatory mapped-free standardized loading `Lambda[t,k] / sqrt(Sigma[t,t])` | All targets in `n=150,d=1` failed the lower-band gate. All targets in `n=150,d=2`, `n=400,d=1`, and `n=400,d=2` passed and cleared the independent D-43 panel. These are native, pinned, unrotated cells only; the global route remains limited. | three exact cells certified; limited globally |
...
- CI-13's native standardized-loading estimand, full joint-delta gradient, and
  replicate-clustered MCSE passed mathematical review. Cell-specific results
  remain separate; no rotated, unconstrained, or neighbouring cell inherits
  them. The independent D-43 panel certified exactly `(n=150,d=2)`,
