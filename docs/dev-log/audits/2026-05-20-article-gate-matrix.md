...
|---|---|---|---|---|---|---|---|---|---|---|
| `morphometrics` | Applied user | How do I fit and interpret a Gaussian latent + unique morphology model? | Visible, under HTML review | `gllvmTMB()`, `traits()`, `extract_Sigma()`, `extract_ordination()`, `extract_communality()`, `plot.gllvmTMB_multi()` | FG-02, FG-03, FG-06, FAM-01, EXT-01, EXT-05, EXT-09 | Both shown | Known Gaussian truth from `morphometrics-example.rds`; one-dataset truth-vs-fit check; broader simulation gate remains separate | Needed | Ada, Pat, Darwin, Fisher, Florence, Grace, Rose | Maintainer visual HTML review; figure wording and biological interpretation accepted. |
| `covariance-correlation` | Applied + technical user | Why does `unique()` change correlations and communality? | Visible, under HTML review | `gllvmTMB()`, `traits()`, `extract_Sigma()`, `extract_correlations()`, `extract_communality()` | FG-02, FG-03, FG-06, EXT-01, EXT-04, EXT-05 | Both shown through `covariance-edge-cases-example.rds`; needs final HTML check | Known Gaussian truth from `covariance-edge-cases-example.rds`; one-dataset `unique()` edge-case test | Needed | Ada, Pat, Fisher, Florence, Rose | Uses covariance-edge object; no count/mixed-family overclaim. |
...
| `convergence-start-values` | Applied user | What should I do when fitting is hard? | Visible, wording review | `check_gllvmTMB()`, `gllvmTMBcontrol()`, `sanity_multi()`, `confint_inspect()` | DIA-01..DIA-10, EXT-13 partial | Both shown | Diagnostic example | None unless diagnostic plots added | Ada, Fisher, Grace, Rose | `pdHess = FALSE` framed as uncertainty warning; bootstrap/profile limits named. |
| `pitfalls` | Applied user | What mistakes look like package bugs, and how do I fix them? | Visible, under HTML review | `gllvmTMB()`, `extract_Sigma()`, `rotate_loadings()`, `suggest_lambda_constraint()`, `meta_V()` | FG-02, EXT-01, EXT-09, LAM-04, MET-01, MET-02 | Long shown; wide equivalence softened until audited | Small reproducible diagnostics | None | Ada, Pat, Boole, Rose | Each pitfall keeps symptom -> diagnostic -> fix -> next action; hidden articles not linked as next steps. |
