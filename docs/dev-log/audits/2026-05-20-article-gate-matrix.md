...
lanes. The 2026-06-08 random-slope restoration is an evidence-led exception:
it promotes the covered `s = 1` structured random-regression grid while keeping
non-Gaussian `s >= 2`, delta/hurdle covariance, and interval calibration out of
scope. Any correlation-matrix figure that displays interval columns uses
