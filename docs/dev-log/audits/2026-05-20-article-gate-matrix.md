...
Public expansion remains one article at a time. The current public set is
`morphometrics`, `model-selection-latent-rank`, `joint-sdm`,
`covariance-correlation`, `api-keyword-grid`, `response-families`,
`fit-diagnostics`, `convergence-start-values`, `profile-likelihood-ci`,
`troubleshooting-profile`, `pitfalls`, and `missing-data`. Do not promote
`mixed-family-extractors`, `psychometrics-irt`, or `lambda-constraint` while
this correction is open.
...
lanes. The 2026-06-08 random-slope work stayed internal after reader review:
ordinary Gaussian reaction norms and structured slope grids remain buildable
drafts, not first-click teaching pages. The 2026-06-09 model-selection article
is a narrow public restoration because it uses a tested Gaussian rank fixture,
long and wide calls, AIC/BIC routing, and fit-health rows. Any
correlation-matrix figure that displays interval columns uses
...
| `model-selection-latent-rank` | Applied user | How many latent dimensions should I fit? | Visible after 2026-06-09 model-selection slice | `gllvmTMB()`, `traits()`, `logLik()`, `AIC()`, `BIC()`, `check_gllvmTMB()`, `extract_Sigma()` | FG-04, FG-06, DIA-03, DIA-08, DIA-10 | Both shown through `model-selection-rank-example.rds` | Known Gaussian `d = 2` teaching truth; deterministic one-dataset AIC/BIC and `Sigma` recovery check | AIC/BIC difference plot and truth-versus-fit covariance scatter must render; no universal selection-rate or interval-calibration claim | Ada, Curie, Fisher, Pat, Boole, Rose, Grace | Keep public if the article renders, the tested fixture selects `d = 2` by AIC/BIC, fit health is shown before interpretation, and prose does not claim AIC/BIC prove the biological rank. |
| `joint-sdm` | Applied ecology user | How do I fit and interpret a binary joint species distribution model? | Visible; binary caveats and diagnostics remain under active audit | `gllvmTMB()`, `traits()`, binary families, diagnostics, extractors, plots | FAM-02/FAM-03, FG-02, FG-03, DIA rows, EXT rows | Present but must be rechecked before stronger claims | Known binary truth + caveats | Needed before any publication-grade figure claim | Pat, Darwin, Fisher, Florence, Grace, Rose | Keep public only with binary validation caveats, diagnostic table, and no loading-constraint overclaim. Re-run rendered review if example data, fitted figures, or binary-scope wording changes. |
...
| `fit-diagnostics` | Applied user | Can I trust this fitted model enough to interpret it? | Visible after 2026-06-09 diagnostics-article slice | `gllvmTMB()`, `traits()`, `check_gllvmTMB()`, `residuals()`, `predictive_check()`, `diagnostic_table()`, `gllvmTMBcontrol()` | DIA-08, DIA-10, DIA-11, DIA-12, DIA-13 | Both shown | Small Poisson behavioural-count simulation; diagnostic example rather than covariance-recovery truth table | Rendered Q-Q and rootogram must be inspected in HTML; diagnostic-only, no interval-calibration claim | Ada, Pat, Rose, Fisher, Florence, Grace | Keep public if the article renders, shows long + wide calls, keeps first-use diagnostic terms plain, and does not call fitted-model diagnostics posterior predictive checks, formal residual tests, latent-rank proof, or interval calibration. |
...
| `profile-likelihood-ci` | Technical user | When should I use profile/bootstrap intervals instead of Wald intervals? | Visible; guarded methods page | `confint()`, `tmbprofile_wrapper()`, `profile_targets()`, bootstrap helpers | CI-01..CI-10, EXT-13 | Not applicable | API and fallback evidence; calibrated coverage remains separate | Required before stronger interval-display claims | Fisher, Gauss, Noether, Grace, Rose | Keep public only if the page distinguishes API coverage from calibrated coverage, reports failure/fallback behaviour plainly, and avoids M3 coverage overclaim. |
| `troubleshooting-profile` | Technical user | How do I troubleshoot profile intervals? | Visible companion methods page | profile helpers, `confint_inspect()`, diagnostics | CI rows, EXT-13 | Not applicable | Profile failure-mode evidence | None unless figures are added | Fisher, Grace, Rose | Keep public as a troubleshooting companion while it names fallback paths and does not imply profile intervals always calibrate. |
...
| `missing-data` | Applied user | What happens when responses or predictors are missing? | Visible; scoped missing-data methods page | `gllvmTMB()`, `traits()`, `miss_control()`, `predict_missing()`, `impute_model()`, `mi()` | MIS rows, FG rows where examples use formulas | Both shown where response-masking examples use `traits(...)` | Masked-response and scoped predictor examples; broader missingness mechanisms remain planned | Needed before any publication-grade imputation figure claim | Ada, Fisher, Grace, Rose | Keep public only with explicit response-vs-predictor scope, no unsupported multiple-`mi()` promise, and current MIS row labels. |

## Hidden Article Rows

| Article | Reader | Main question | Public status | Required functions | Capability rows | Long/wide status | Truth or comparator | Figure gate | Reviewer signoff | Return condition |
|---|---|---|---|---|---|---|---|---|---|---|
| `random-regression-reaction-norms` | Behavioural ecology / animal personality user | How do I fit individual-level random regression and interpret personality-plasticity covariance? | Hidden/internal after 2026-06-08 rendered-reader review | ordinary Gaussian `latent + unique` random regression at `unit = "individual"`; non-Gaussian augmented `unique()` guarded | RE-12 partial; Gaussian `Lambda_aug Lambda_aug^T + Psi_B,aug` implemented with focused recovery; non-Gaussian augmented `unique()` still pending | Long and wide formulas rendered from a shipped behavioural reaction-norm example object | Behavioural reaction-norm truth object with diagnostics, augmented-covariance recovery figure, and repeatability curves | Point-estimate/recovery display exists; public prose gate failed on ledger-style opening and needs a plain-language reader pass | Ada, Pat, Rose, Fisher, Grace | Return only after the article opens with reader language, removes validation-ledger jargon from the public path, and the rendered HTML passes Pat/Rose/Fisher review. Does not use deprecated `phylo_slope()` or structured dependence as a substitute. |
| `random-slopes-nongaussian` | Applied ecology/evolution user with non-Gaussian traits | How do I fit structured single-slope random regressions for counts, proportions, ordinal traits, phylogenies, and space? | Hidden/internal after 2026-06-08 article-tier review | `phylo_indep()`, `phylo_latent()`, `phylo_dep()`, `spatial_indep()`, `spatial_latent()`, `spatial_dep()`, `extract_Sigma()`, `check_gllvmTMB()` | PHY-11..PHY-18, SPA-08..SPA-10, ANI-11..ANI-12, RE-03, FAM-17, MIX-10, CI-08, CI-10 | Both shown for the syntax; heavy validation cells shown `eval = FALSE` | Recovery numbers from validation tests; build-time chunks are small syntax/recovery illustrations | No interval or publication-figure claim | Ada, Pat, Rose, Fisher, Grace | Return after the phylogenetic GLLVM / structured-dependence reader path is public. Do not imply calibrated CIs, non-Gaussian `s >= 2`, or delta/hurdle slope covariance. |
