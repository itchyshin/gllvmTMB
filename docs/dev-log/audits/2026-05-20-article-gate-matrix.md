...
| `random-regression-reaction-norms` | Behavioural ecology / animal personality user | How do I fit individual-level random regression and interpret personality-plasticity covariance? | Hidden implementation article after 2026-06-08 rendered preview | ordinary Gaussian `latent + unique` random regression at `unit = "individual"`; non-Gaussian augmented `unique()` guarded | RE-12 partial; Gaussian `Lambda_aug Lambda_aug^T + Psi_B,aug` implemented with focused recovery, broader coverage still pending | Long and wide formulas documented; public runnable example object still pending | Appendix-B behavioural reaction-norm target; Gaussian component runnable, public biological example/diagnostics/figures still pending | Needed before public restoration | Ada, Pat, Rose, Fisher, Grace | Return only after the behavioural-syndrome example object, rendered long + wide examples, diagnostics/figure review, and non-Gaussian boundary wording are implemented/tested/documented. Do not use deprecated `phylo_slope()` or structured dependence as a substitute. |
