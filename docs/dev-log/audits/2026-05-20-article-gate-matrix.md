...
|---|---|---|---|---|---|---|---|---|---|---|
| `morphometrics` | Applied user | How do I fit and interpret a Gaussian latent + unique morphology model? | Visible; launch-level HTML review passed; final figure/prose audit pending | `gllvmTMB()`, `traits()`, `extract_Sigma()`, `extract_ordination()`, `extract_communality()`, `plot.gllvmTMB_multi()`, `plot_correlations()`, `plot_Sigma_comparison()` | FG-02, FG-03, FG-06, FAM-01, EXT-01, EXT-05, EXT-09, EXT-19, EXT-23, EXT-25, EXT-26, MIS-22 | Both shown | Known Gaussian truth from `morphometrics-example.rds`; one-dataset truth-vs-fit check; cached bootstrap correlation fixture supports visual uncertainty examples but is not calibration evidence | Needed | Ada, Pat, Darwin, Fisher, Florence, Grace, Rose | Final article pass checks figure captions, biological interpretation, and rendered visual quality. |
| `covariance-correlation` | Applied + technical user | Why does `unique()` change correlations and communality? | Visible; launch-level HTML review passed; final figure/prose audit pending | `gllvmTMB()`, `traits()`, `extract_Sigma()`, `extract_Sigma_table()`, `compare_Sigma_table()`, `plot_Sigma_table()`, `plot_correlations()` | FG-02, FG-03, FG-06, EXT-01, EXT-04, EXT-05, EXT-18, EXT-19, EXT-25, EXT-26, EXT-30 | Both shown through `covariance-edge-cases-example.rds` | Known Gaussian truth from `covariance-edge-cases-example.rds`; one-dataset `unique()` edge-case test | Needed | Ada, Pat, Fisher, Florence, Rose | Final article pass checks that the matrix display is explained as display infrastructure, not interval calibration. |
| `api-keyword-grid` | Technical user | Which formula keyword names which covariance mode and correlation row? | Visible technical reference | Formula parser, `traits()`, covariance keywords | FG-01, FG-02, FG-03, FG-06, PHY-01..PHY-10 partial, SPA-01..SPA-07 partial, MET-01 partial | Both shown for ordinary bar-style terms | Not applicable | None | Ada, Boole, Grace, Rose | Status labels match validation register; hidden worked examples are not advertised as ready. |
| `response-families` | Applied + technical user | Which family can I fit, and what is its validation status? | Visible technical reference | `family_to_id()`, `gllvmTMB()`, `extract_Sigma()`, `extract_Omega()` | FAM-01..FAM-15, MIX-01..MIX-08, EXT-01..EXT-08 | Both shown for single-family; mixed-family wide not applicable yet | Not applicable | None | Ada, Fisher, Grace, Rose | Family table labels covered/partial/blocked honestly; delta/mixed boundaries clear. |
| `convergence-start-values` | Applied user | What should I do when fitting is hard? | Visible; final wording audit pending | `check_gllvmTMB()`, `gllvmTMBcontrol()`, `sanity_multi()`, `confint_inspect()`, `extract_Sigma_table()` | DIA-01..DIA-10, EXT-13 partial, EXT-18 | Both shown where examples use the public fit path | Diagnostic example | None unless diagnostic plots added | Ada, Fisher, Grace, Rose | Final wording pass keeps `pdHess = FALSE` as an uncertainty/identifiability warning and names bootstrap/profile limits. |
| `pitfalls` | Applied user | What mistakes look like package bugs, and how do I fix them? | Visible; launch-level HTML review passed; final prose audit pending | `gllvmTMB()`, `extract_Sigma()`, `rotate_loadings()`, `suggest_lambda_constraint()`, `meta_V()` | FG-02, EXT-01, EXT-09, LAM-04, MET-01, MET-02 | Long shown; wide equivalence softened until audited | Small reproducible diagnostics | None | Ada, Pat, Boole, Rose | Final article pass keeps each point general, with any specific model used only as an example; hidden articles stay out of recommended next steps. |
...
| `mixed-family-extractors` | Technical user | How do extractors behave when traits use different likelihoods? | Hidden technical reference | mixed-family fits, extractor tables | MIX-01..MIX-08, EXT rows partial | Not applicable for mixed-family table | Known mixed-family truth | Needed if plots added | Fisher, Emmy, Rose | Mixed-family example object and report-ready extractor tables. |
...
| `psychometrics-irt` | Methods + applied user | How does binary IRT relate to `lambda_constraint`? | Hidden | binary fits, `lambda_constraint`, comparator tools | LAM-02, LAM-03, FAM-02 | Missing/needs rebuild | mirt/galamm comparator | Needed | Noether, Fisher, Pat | Binary/IRT helper; constraint validation; comparator evidence. |
| `lambda-constraint` | Technical user | How do I constrain loading matrices safely? | Hidden | `lambda_constraint`, `suggest_lambda_constraint()`, rotation helpers | LAM-02..LAM-04, EXT-14, EXT-15 | Present but must be rechecked | Confirmatory loading truth | Maybe | Boole, Noether, Fisher, Rose | Confirmatory helper and reliability checks pass. |
