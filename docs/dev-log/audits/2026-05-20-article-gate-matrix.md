...
| `random-slopes-nongaussian` | Applied ecology/evolution user with non-Gaussian traits | How do I fit structured single-slope random regressions for counts, proportions, ordinal traits, phylogenies, and space? | Visible after 2026-06-08 status sync | `phylo_indep()`, `phylo_latent()`, `phylo_dep()`, `spatial_indep()`, `spatial_latent()`, `spatial_dep()`, `extract_Sigma()`, `check_gllvmTMB()` | PHY-11..PHY-18, SPA-08..SPA-10, ANI-11..ANI-12, RE-03, FAM-17, MIX-10, CI-08, CI-10 | Both shown for the public syntax; heavy validation cells shown `eval = FALSE` | Recovery numbers from validation tests; build-time chunks are small syntax/recovery illustrations | No interval or publication-figure claim | Ada, Pat, Rose, Fisher, Grace | Keep public as a point-estimate/recovery article. Do not imply calibrated CIs, non-Gaussian `s >= 2`, or delta/hurdle slope covariance. |
