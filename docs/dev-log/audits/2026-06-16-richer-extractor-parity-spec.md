...
| --- | --- | --- | --- |
| EXT-JL-RAW | Raw unit-tier payload shape and labels | R accessors preserve trait names, dimensions, symmetric `Sigma`, unit-diagonal `R`, finite loadings/scores, point-only Sigma/correlation table schema, and point-only estimate-vs-truth comparison rows for every admitted family row. Direct R fake-payload tests and live JuliaCall tests agree with the paired GLLVM.jl bridge payload. | Partly admitted through `JUL-01A`; raw payload is retained on the object/summary, public covariance extraction now applies native scale semantics, `extract_Sigma_table()` exposes point-only unit-tier rows with `validation_row = "JUL-01A"`, and `compare_Sigma_table()` can join those rows to one supplied truth matrix. |
