...
| `joint-sdm` | Articles / Model guide | Tier 1 worked example | Keep public with binary caveats | FAM-02/FAM-03, FG-02, FG-03, DIA rows, EXT rows | Stronger binary or loading-constraint claims need rerendered review | Pat, Darwin, Fisher, Florence, Rose, Grace | Re-review before using it as support for `lambda_constraint` promotion | `Rscript --vanilla -e 'pkgdown::build_articles(lazy = FALSE)'` |
| `lambda-constraint` | Articles / Loading constraints | Candidate Tier 1, not yet safe | Rewrite or hide until rewrite passes | LAM-02..LAM-04, FAM-02/FAM-03, EXT-14/15 | Article-gate matrix says binary loading-constraint lane must be coherent before promotion; current navbar visibility is a mismatch to resolve | Ada, Boole, Noether, Fisher, Florence, Pat, Rose, Grace | Rework around a binary species/JSDM-style example; if not ready, remove from public dropdown in the navbar slice | `Rscript --vanilla -e 'pkgdown::build_articles(lazy = FALSE)'` |
| `lambda-constraint-suggest` | Articles / Loading constraints | Tier 2 companion | Keep only after main lambda article is coherent | LAM-04, EXT-14/15 | Companion should not be public before the main teaching path is stable | Boole, Pat, Fisher, Rose, Grace | Keep as technical companion or move out with `lambda-constraint` in the navbar slice | `Rscript --vanilla -e 'pkgdown::build_articles(lazy = FALSE)'` |
...
| `data-shape-flowchart` | Internal drafts | Candidate entry-path helper | Merge into Get Started or restore as lightweight guide | FG-01..FG-03 | Needs duplication audit against Get Started | Pat, Boole, Rose | Decide merge vs standalone after public surface stabilizes | `Rscript --vanilla -e 'pkgdown::build_articles(lazy = FALSE)'` |
...
| `gllvm-vocabulary` | Internal drafts | Candidate Tier 2 glossary | Restore only if it reduces jargon | Cross-cutting terminology rows | Needs terminology sweep and cross-link plan | Boole, Pat, Rose | Merge glossary terms into visible pages or restore compact reference | `Rscript --vanilla -e 'pkgdown::build_articles(lazy = FALSE)'` |
...
| `stacked-trait-gllvm` | Internal drafts | Merge or rewrite | Keep internal | FG-01..FG-06 | Likely duplicates Get Started / concepts | Pat, Boole, Rose | Duplication audit: merge into Get Started or rewrite as conceptual primer | `Rscript --vanilla -e 'pkgdown::build_articles(lazy = FALSE)'` |
