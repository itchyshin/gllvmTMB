...
`coef()` and `summary()`. The slice intentionally keeps prediction, residuals,
simulation, extractor parity, and confidence intervals gated.
...
- Capability ledger guard:
  ...
  -> `coef()` and `summary()` are routed; prediction, residuals, simulation,
  and ordination/extractor rows remain `FALSE`.
- Generated-method check:
  `rg -n 'S3method\\((coef|summary),gllvmTMB_julia\\)|S3method\\(print,summary.gllvmTMB_julia\\)|gllvmTMB_julia-methods' NAMESPACE man/gllvmTMB_julia-methods.Rd`
  -> new S3 methods are registered and documented.
- Whitespace:
  `git diff --check` -> clean.
- Stale wording scan:
  ...
  -> only current scoped post-fit wording and historical reports updated to
  mention the later `coef()` / `summary()` admission.
- Dashboard smoke:
  `curl -fsS 'http://127.0.0.1:8770/?v=precommit' | rg -n 'Updated: 2026-06-16 10:57 MDT|coef\\(\\)|summary\\(\\)|source commit pending|Local source commit pending|prediction/residual/simulation/extractor parity'`
  -> local widget reflects the pending post-fit slice.
- Files updated:
  `R/julia-bridge.R`;
  ...
  `docs/dev-log/after-task/2026-06-16-r-bridge-coef-summary-admission.md`;
...
- Full `devtools::test()`, `devtools::check()`, `pkgdown::check_pkgdown()`,
  CRAN-style checks, and article renders. This slice adds a narrow S3
  point-summary surface and updates one generated Rd topic plus bridge tests and
  ledgers.
...
## 2026-06-02 -- binary JSDM package-description citation

Goal: add a package-level literature anchor for binary
presence-absence / binomial-family joint species distribution models.

Files changed:

- `DESCRIPTION`
- `man/gllvmTMB-package.Rd` (generated by roxygen from `DESCRIPTION`)
- `docs/dev-log/check-log.md`
- `docs/dev-log/after-task/2026-06-02-binary-jsdm-package-citation.md`

Article selected:

- Pollock, Tingley, Morris, Golding, O'Hara, Parris, Vesk, and
  McCarthy (2014), *Methods in Ecology and Evolution* 5:397-406,
  DOI `10.1111/2041-210X.12180`. Reason: the paper explicitly presents
  a binary presence-absence JSDM / multivariate probit SDM that
  decomposes species co-occurrence into environmental and residual
  components, matching the package-description claim.
...
- `gh pr list --state open --json number,title,headRefName,baseRefName,url --limit 20`
  ...
  #420, #369. No direct `DESCRIPTION` / package-Rd collision detected.
...
- `grep -c '^\\keyword' man/gllvmTMB-package.Rd && tail -n 8 man/gllvmTMB-package.Rd`
  -> one `\keyword{}` entry only; tail shows the expected final
  `\keyword{internal}` block.
- `git diff --check` -> clean.
- `Rscript --vanilla -e 'pkgdown::check_pkgdown()'`
  -> `No problems found.`
- Recreated split branch `codex/binary-jsdm-citation` on 2026-06-03 from
  `origin/main` at `6d08513` after #428/#430 landed; regenerated
  `man/gllvmTMB-package.Rd` from `DESCRIPTION` and restored unrelated Rd
  link-formatting churn from `devtools::document()`.
- Rebased 2026-06-03 onto `origin/main` at `8da7f61` after #420 landed.
  The only conflict was this append-only check-log file; the merged VA,
  Track A, and binary-citation entries were all preserved.
- `git diff --check origin/main...HEAD` after the 2026-06-03 rebase onto
  `8da7f61`
  -> clean.
- `Rscript --vanilla -e 'pkgdown::check_pkgdown()'` after the 2026-06-03
  rebase onto `8da7f61`
  -> `No problems found.`
- `git diff --check` on the split branch
  -> clean.
- `Rscript --vanilla -e 'pkgdown::check_pkgdown()'` on the split branch
  -> `No problems found.`
- `gh issue list --repo itchyshin/gllvmTMB --state open --search 'binary OR binomial OR "joint SDM" OR JSDM OR "species distribution" OR Pollock' --json number,title,url,labels,updatedAt --limit 20`
  -> inspected #351, #341, #340, #332, #348, #349, #342, #346, #230.
  No issue was specific to package-description citation hygiene, so no
  issue comment or closure was made.

Stale-wording / citation scans:
...
- `rg -n "Pollock|2041-210X\\.12180|joint species distribution|binomial-family|presence-absence" DESCRIPTION man/gllvmTMB-package.Rd vignettes/articles/lambda-constraint.Rmd vignettes/articles/joint-sdm.Rmd`
  ...
  existing lambda-constraint article; confirms the DOI is visible in
...
- `rg -n "in prep|in preparation|meta_known_V|gllvmTMB_wide|phylo_rr|block_V|deprecated.*0\\.1|\\\\bS_B\\\\b|\\\\bS_W\\\\b|\\\\\\\\bf S" DESCRIPTION man/gllvmTMB-package.Rd`
  ...
  S/U notation was introduced into the package description.
- `rg -n "FAM-02|FAM-03|FAM-04|FG-04" docs/design/35-validation-debt-register.md`
  -> confirmed the advertised binary / latent JSDM surface maps to
  covered validation rows for `latent(0 + trait | unit, d = K)` and
  binomial logit/probit/cloglog family support.
