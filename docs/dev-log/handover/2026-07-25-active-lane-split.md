...
**Refresh 2026-08-15 #2 (Cursor): LA-MSPL point-continue RUNNING.**
Lane **`cursor/mspl-point-programme-continue`** (name stuck; supersedes the
handover's `cursor/mspl-gaussian-point-evidence` label) · WT
`/private/tmp/gllvmtmb-mspl-estimator-programme-roadmap` · LOOP
`docs/dev-log/lanes/cursor-mspl-point-continue/LOOP/` · PR
[#971](https://github.com/itchyshin/gllvmTMB/pull/971). GOAL A+B+C in flight.
Prior refresh below remains true for #963–#969 closeout.
...
| --- | --- | --- | --- | --- |
| **LA-MSPL point-continue** | Cursor | **RUNNING.** `cursor/mspl-point-programme-continue` · [#971](https://github.com/itchyshin/gllvmTMB/pull/971). #963–#969 on main; catch-up/gaussian kits CLOSED. A multi-seed Gaussian point + B Poisson Phase-4 prep | `2026-08-15-cursor-handover.md`; LOOP `docs/dev-log/lanes/cursor-mspl-point-continue/LOOP/`; programme after-task 2026-08-14 | LA-MSPL only; never root `LOOP/`; **no Gaussian SE**; Binary SE **PROTECTED**; Poisson planned only; no NEWS covered; no campaign without G0+receipt |
