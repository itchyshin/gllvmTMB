...
- **A script built by copy-editing another script can silently watch the wrong thing.** A merge
  watcher generated by text-substituting a previous one reported `VERIFIED ... MERGED` for a
  *different* pull request than the one intended — the substitution missed, and the script cheerfully
  confirmed something already true. Parameterise such helpers rather than editing copies, and confirm
  the outcome against the object itself, not against the helper's own report.
...
## Status of this repo for the incoming lane (added 2026-09-04, on close)

**gllvmTMB is FREE.** The outgoing Claude lane is finished: `main` @ `65301cf62`, nothing of its own
open, all worktrees but its own removed. Take the repo and commit freely; no coordination with it is
needed.

**One arc was started and deliberately NOT finished here:** the `ordination_uncertainty()` coverage
campaign. The maintainer approved it and it was dispatched, but the Cursor lane started while it was
still in local preparation, so it was stopped before anything ran — **no compute was used, no partial
results exist, and nothing needs cleaning up on Totoro.** The design (Section 9a) and its pre-run are
both on `main`, so the arc is fully specified and costed: roughly **5 core-hours as a ceiling**, and
the sparse solve that made the original estimate uncertain is measured and negligible. It is a good
