...
Shinichi 2026-08-15: catch-up GOAL closed via #963; then KEEP GOING
toward finishing LA-MSPL inside fences (not stop at oracles).
1B expansion / campaign / C++ / admission / NEWS remain STOP.
...
| U | IN PROGRESS | Ψ uniqueness map (pick C pinned FA) + E5b | PR this branch; STOP before C++ |
| B-complete | PENDING | Cheap Bernoulli registry harden only | no admit/number change; no B2 reopen |
| G-oracle | PENDING / partial | Expand pure-R oracles under pick C | E5b landed; no Gaussian MSPL fit |
...
| P3-admit | GATED | Live Gaussian MSPL route | C++ + Shinichi (uniqueness map CLOSED for pick C) |
