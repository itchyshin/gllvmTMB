...
- A1 — RED: helpers missing (`could not find function ".st_fixture"` / `".ord_fixture"`).
...
RESUME:
```text
Student-t + ordinal_probit Phase-4 planned prep verified 51+45.
HARD STOP: registry row, prepare widen, R/mspl.R, src/, admit,
NEWS covered, public se=TRUE, merge.
```
