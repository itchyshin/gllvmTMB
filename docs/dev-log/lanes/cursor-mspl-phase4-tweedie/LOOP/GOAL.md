...
```text
Solo: Cursor
Deliverable: Tweedie LA-MSPL Phase-4-style prep — symbolic power/dispersion + mass-at-zero note, pure-R oracles, planned only (NOT admitted)
...
DISCIPLINE: planned only; no prepare widen; no src/; verify by logs; structured test counts
```

## Headline

Earn Tweedie on paper + oracles as a **planned** cell whose
information atom and zero mechanism are *not* Poisson's. Do not
admit. Do not widen `.gllvmTMB_mspl_prepare()`.
...
2. Pure-R oracles E1–E8 + fence tests; structured expectation
   counts; no live `estimator = "mspl"`.
