...
Recorded under **D-157** new construction + this card SIGNED. **No new vault/repo D-number** (D-12 already heroizes profile; this paste locks the triad roles for the post-B1 path). Poisson \(W\) G0 is **SIGNED — REPLACE** (Shinichi *"as you recommended"* after Cursor REPLACE recommendation; #1096 §3 paste) — prior PARK-from-approve-all is **superseded**; see `docs/dev-log/research/2026-08-17-mspl-poisson-W-G0.md` and provenance **RESOLVED**. Tape programme unlocked for Codex; SE doors stay closed until rematch green; no `src/` in the signature PR. #1077 stays draft/fenced; no public profile intervals; no Design 118 / B1 / Totoro reopen; no public `se=TRUE`. Binomial profile **computability** on `main` (`#1090`) is not a calibrated CI claim.
