...
Florence diagnostic-report gate, #222 fitted-model diagnostic
prototype, and article-surface reset / infrastructure-first article
gate).*
...
## Since last refresh (2026-05-16 -> 2026-05-20)
...
- PR #201 added the M3.3 failure-mode ledger: every uncovered
  converged row missed above the profiled `psi` upper bound, the
  small `glmmTMB` nbinom2 comparator was useful but not a direct
  multivariate-`psi` standard, and `galamm` is not a direct nbinom2
  comparator for this slice.
- PR #202 added the target-scale audit: `covered_prof` in the
  production grid is `psi` coverage, while the primary promotion
  target remains total `Sigma_unit[tt]`. This makes it high time to
  refresh the M3.3 roadmap before another full production rerun.
- PR #213 added fitted NB2 dispersion and link-residual diagnostics
  to the M3 grid rows. The corrected stress pilots now show that
  NB2 `Sigma_unit_diag` underestimation is entangled with fitted
  dispersion calibration, not just interval construction.
- PR #214 added a development-only known-phi point diagnostic for
  NB2. Fixing `phi_nbinom2` at its DGP value improved median
  `Sigma_unit_diag` estimate/truth ratios in all three small
  scenarios, but did not repair the baseline cell completely. EXT-13,
  CI-08, and CI-10 therefore remain `partial`.
- The 2026-05-20 drmTMB cross-learning audit translated the sister
  package's Phase 18 discipline into the next M3.3 shape: surface
  admission gates, method-labelled interval evidence, small rendered
  diagnostic reports, and failure ledgers before any broad rerun. It
...
- The robust-modeling roadmap is now captured in
  ...
  status, `gllvmTMBcontrol(se = FALSE)`, `fit_health`, and the
  machine-readable `check_gllvmTMB()` table.
- The 2026-05-20 article-surface reset pauses article production
  behind infrastructure readiness. Public examples now require a
  rendered HTML review with the maintainer, paired long-format and
  wide `traits(...)` formulas for Tier-1 worked examples, truth-vs-fit
  recovery when simulated data are used, and explicit validation-row
  scope boundaries before any page is revealed.

## Next small steps (proposed)

These are intentionally bounded lanes. Treat each as its own PR.

1. **Article surface reset and roadmap rewrite (Ada + Pat + Rose +
   Grace)**: keep all article files on disk, hide premature pages from
   the visible navbar, classify every article as ready candidate /
   halfway / technical reference / infrastructure-blocked, and rewrite
   the roadmap around infrastructure gates before restoring any page.
   No article reveal without rendered HTML review and long + wide
   formula checks. Copy drmTMB's navigation discipline: model guides,
   tutorials, simulation/comparison, and developer notes must stay
   separate. Issue ledger: #230.
2. **NB2 dispersion/variance source-map decision (Fisher + Curie +
   ...
   estimand.
3. **Start-strategy and local-basin probe (Fisher + Gauss + Rose)**:
   ...
   cells are slow.
4. **Fixed-phi bootstrap design checkpoint (Gauss + Noether +
...
5. **Rendered diagnostic figure gate (Florence + Pat + Grace)**:
...
| Phase 1d | Article surface reset + roadmap rewrite | 🟡 Reopened | `█░░░` 1/4 | Public article work paused 2026-05-20. Minimal visible set, full article classification, infrastructure-first roadmap, and rendered HTML sign-off gate now precede any article reveal. |
...
## 🟡 Phase 1d -- Article surface reset + roadmap rewrite -- `█░░░` 1/4

**Goal**: stop the article overexposure loop. Keep article source
files on disk, but show only the minimum public surface until the
modelling, simulation, extraction, uncertainty, diagnostics, and
plotting infrastructure can support each page.

Every restored Tier-1 worked example must show both the canonical
long-format call and the wide data-frame `traits(...)` formula call,
unless the article explicitly explains why the wide companion is not
supported for that workflow. Every restored article also needs a
rendered HTML review with the maintainer before reveal.

### Work

- 🟡 Hide premature pages from the visible article dropdown without
  deleting source files.
- 🟡 Classify every current article as ready candidate, halfway,
  technical reference, infrastructure-blocked, or project/internal.
- ⚪ Rewrite the roadmap around infrastructure-first gates:
  simulation fixtures, plotting functions, modelling functions,
  profile/bootstrap uncertainty, extraction, and diagnostics before
  examples.
- ⚪ Add scenario-level simulation helpers so articles can describe a
  biological world and fit one precious model without page-long DGP
  code blocks.
- ⚪ Revise the landing page and Get Started path after the hidden-page
  plan is settled.
- ⚪ Add the rendered-HTML sign-off gate to the after-task / article
  promotion protocol.
- ⚪ `pkgdown::check_pkgdown()` clean after the navigation change.

### Cross-refs

- [`docs/dev-log/audits/2026-05-20-article-surface-reset.md`](https://github.com/itchyshin/gllvmTMB/blob/main/docs/dev-log/audits/2026-05-20-article-surface-reset.md)
- [Issue #230: article surface reset and user-first tooling gate](https://github.com/itchyshin/gllvmTMB/issues/230)
