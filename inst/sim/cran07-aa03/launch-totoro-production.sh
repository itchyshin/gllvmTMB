...
if [ "$workers" -lt 1 ] || [ "$workers" -gt "$TOTORO_CORE_CAP" ] || [ "$workers" -gt "$(( host_cores - 4 ))" ]; then
...
NWORKERS="$workers" TOTORO_NPROC="$host_cores" OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
