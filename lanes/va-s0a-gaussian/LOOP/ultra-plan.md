...
## G0 OVERLAY (Shinichi 2026-08-07) — BINDING; overrides reuse default above

Copied into the LOOP kit for execution. This overlay is immutable for the run.

1. **FRESH seeds** — do not reuse Arc-2 confirmation rows as the primary verdict.
2. **Abs caps FLEXIBLE** — start from β RMSE < 0.35 and rel cov Frobenius < 0.50;
   may propose and document alternate caps if evidence warrants; record both
   default and chosen. Prefer abs-availability eligibility (≥0.90 completed VA
   with finite metrics), not paired Laplace. Still report Laplace completion and
   paired ratios as secondary diagnostics.
3. **STOP after S0a** for Shinichi checkpoint before any S0b
   (poisson/lognormal/gamma).

Execution choices locked by this LOOP:

- Estimator revision for Totoro fits: `022b4eab` (identical `R/`/`src/` to
  `a5b34529`; Gate-E/runtime already proven there).
- Seeds: `10001:10300` (n=300), cell=`gaussian_identity`, H=7, q∈{2,5},
  estimators=`va,laplace`, n=120, p=8.
- Campaign root (Totoro):
  `/home/snakagaw/gllvm_work/va-s0a-gaussian-022b4eab-20260807`
- No push/PR; no fence/threshold edits; no Arc-2 re-label.
