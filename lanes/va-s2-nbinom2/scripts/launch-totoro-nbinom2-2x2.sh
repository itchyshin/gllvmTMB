...
# Totoro launcher — NB2 (nbinom2) 2×2: gtmb VA-GH / gtmb LA / gllvm VA / gllvm LA.
# Design-110 DGP n=120 p=8 q=2; H=7; matched n_starts=1; se=FALSE; warm DLL.
...
PROBE_SCRIPT="$SCRIPT_DIR/probe-nbinom2-2x2-smoke.R"

ACTION="${ACTION:-dry-run}"
HOST="${TOTORO_HOST:-totoro}"
REMOTE_ROOT="${REMOTE_ROOT:-/home/snakagaw/gllvm_work/va-s2-nbinom2-2x2-20260807}"
...
ACTION=dry-run|sync|smoke|full|status|pull
  ...
  smoke    Remote: 1 seed, 1 core
  full     Remote: N_SEED (default 16), PILOT_CORES
  status   tail remote log + ls results
  pull     scp results to LOCAL_PULL (/private/tmp/...)

Env: PROBE_N_SEED PROBE_SEED0 PROBE_Q PROBE_VA_H PROBE_N_STARTS PROBE_PHI
...
case "$ACTION" in
  -h|--help) usage; exit 0 ;;
  dry-run)
    echo "S2 nbinom2 2×2 Totoro dry-run"
    echo "  HOST=$HOST"
    echo "  REMOTE_ROOT=$REMOTE_ROOT"
    echo "  REMOTE_CHECKOUT=$REMOTE_CHECKOUT"
    echo "  LOCAL_PROBE=$LOCAL_PROBE"
    echo "  N_SEED=$N_SEED SEED0=$SEED0 Q=$Q H=$VA_H n_starts=$N_STARTS phi=$PHI CORES=$CORES"
    echo "Say ACTION=sync then ACTION=smoke|full to launch."
    ;;
  sync)
    ssh "$HOST" "mkdir -p '$REMOTE_ROOT'/{scripts,results,logs}"
    scp "$LOCAL_PROBE" "$HOST:$REMOTE_ROOT/scripts/probe-nbinom2-2x2-smoke.R"
...
nohup env PROBE_REPO="\$PROBE_REPO" PROBE_OUT="\$PROBE_OUT" \\
  PROBE_N_SEED=1 PROBE_SEED0=$SEED0 PROBE_Q=$Q PROBE_VA_H=$VA_H \\
  PROBE_N_STARTS=$N_STARTS PROBE_PHI=$PHI \\
  PILOT_CORES=1 PROBE_CORE_CAP=1 \\
  Rscript --vanilla '$REMOTE_ROOT/scripts/probe-nbinom2-2x2-smoke.R' \\
...
mkdir -p "\$PROBE_OUT"
cd "\$PROBE_REPO"
# Refuse to start if our previous full job is still running; leave others alone.
...
nohup env PROBE_REPO="\$PROBE_REPO" PROBE_OUT="\$PROBE_OUT" \\
  PROBE_N_SEED=$N_SEED PROBE_SEED0=$SEED0 PROBE_Q=$Q PROBE_VA_H=$VA_H \\
  PROBE_N_STARTS=$N_STARTS PROBE_PHI=$PHI \\
...
EOF
    ;;
  pull)
    LOCAL_PULL="${LOCAL_PULL:-/private/tmp/va-s2-nbinom2-2x2-smoke-20260807}"
    mkdir -p "$LOCAL_PULL"
