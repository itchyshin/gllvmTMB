...
## Same DGP as 2×2 smoke (Design-110 loadings-only; unique=FALSE).
## Grid: n ∈ {120,250,400,1000} (PROBE_N_GRID), p=8, q=2.
## Arms: gtmb LA / gtmb VA-GH H=7 / gllvm LA;
##       gllvm VA only at n in PROBE_GLLVM_VA_NS (default 120) to reconfirm collapse.
...
OUT <- Sys.getenv(
  "PROBE_OUT",
  unset = "/private/tmp/va-s2-nbinom2-nladder-20260807"
)
...
N_GRID <- as.integer(strsplit(Sys.getenv("PROBE_N_GRID", "120,250,400,1000"), ",")[[1L]])
...
cat(sprintf(
  "n=%s p=%d q=%d H=%d n_starts=%d seeds=%d cores=%d phi=%g gllvm_va_n=%s\n",
  paste(N_GRID, collapse = ","), P, Q, VA_H, N_STARTS, N_SEED, CORES, PHI_TRUE,
  paste(GLLVM_VA_NS, collapse = ",")
))
...
cat("\n======== NB2 n-ladder (p=8 q=2; abs caps β≤0.35 / Σ≤0.50) ========\n")
