...
# Totoro launcher — betabinomial n-ladder: does Σ recover with n?
# Design-110 loadings-only; n ∈ {120,400,1000}; trials=10; phi=3.
# Arms: gtmb LA/VA; gllvm LA/VA attempted (expect N/A on gllvm 2.0.13).
# Matched n_starts=1; se=FALSE. Do NOT kill unrelated Totoro jobs.
# D-50: results stay on Totoro + /private/tmp.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LANE_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="${CHECKOUT:-$(cd "$LANE_ROOT/../.." && pwd)}"
PROBE_SCRIPT="$SCRIPT_DIR/probe-betabinomial-nladder.R"

ACTION="${ACTION:-dry-run}"
HOST="${TOTORO_HOST:-totoro}"
REMOTE_ROOT="${REMOTE_ROOT:-/home/snakagaw/gllvm_work/va-s3-betabinomial-nladder-20260807}"
...
Env: PROBE_N_SEED PROBE_SEED0 PROBE_N_GRID PROBE_Q PROBE_P PROBE_VA_H
     PROBE_N_STARTS PROBE_PHI PROBE_N_TRIALS PROBE_DO_GLLVM_LA PROBE_DO_GLLVM_VA
...
export PROBE_N_TRIALS=$N_TRIALS
...
case "$ACTION" in
  -h|--help) usage; exit 0 ;;
  dry-run)
    echo "S3 betabinomial n-ladder Totoro dry-run"
    echo "  HOST=$HOST"
    ...
    echo "  n_starts=$N_STARTS phi=$PHI trials=$N_TRIALS CORES=$CORES"
    echo "Say ACTION=sync then ACTION=smoke|full to launch."
    ;;
  sync)
    ssh "$HOST" "mkdir -p '$REMOTE_ROOT'/{scripts,results,logs}"
    scp "$LOCAL_PROBE" "$HOST:$REMOTE_ROOT/scripts/probe-betabinomial-nladder.R"
...
nohup env PROBE_REPO="\$PROBE_REPO" PROBE_OUT="\$PROBE_OUT" \\
  ...
  PROBE_PHI=$PHI PROBE_N_TRIALS=$N_TRIALS \\
  PROBE_DO_GLLVM_LA=$DO_GLLVM_LA PROBE_DO_GLLVM_VA=$DO_GLLVM_VA \\
  PILOT_CORES=2 PROBE_CORE_CAP=2 \\
  Rscript --vanilla '$REMOTE_ROOT/scripts/probe-betabinomial-nladder.R' \\
...
EOF
    ;;
  pull)
    LOCAL_PULL="${LOCAL_PULL:-/private/tmp/va-s3-betabinomial-nladder-20260807}"
    mkdir -p "$LOCAL_PULL"
