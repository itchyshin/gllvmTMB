#!/usr/bin/env Rscript
## Betabinomial n-ladder — does Σ recover with n?
## Design-110-ish loadings-only DGP (unique=FALSE; Σ = ΛΛ').
## Grid: n ∈ {120,400,1000} (PROBE_N_GRID), p=8, q=2.
## Trials: PROBE_N_TRIALS=10 (Design-59 / recovery default; not Bernoulli).
## Phi: PROBE_PHI=3 (Stoklosa-ish moderate overdispersion).
## Arms: gtmb LA / gtmb VA-GH H=7; gllvm VA/LA attempted then N/A if API refuses
##   (gllvm 2.0.13: beta.binomial VA unimplemented; LA errors on Ntrials).
...
OUT <- Sys.getenv(
  "PROBE_OUT",
  unset = "/private/tmp/va-s3-betabinomial-nladder-20260807"
)
...
SEEDS <- as.integer(Sys.getenv("PROBE_SEED0", "11301")) + seq_len(N_SEED) - 1L
...
PHI_TRUE <- as.numeric(Sys.getenv("PROBE_PHI", "3"))
N_TRIALS <- as.integer(Sys.getenv("PROBE_N_TRIALS", "10"))
...
cat(sprintf(
  "n=%s p=%d q=%d H=%d n_starts=%d seeds=%d cores=%d phi=%g trials=%d\n",
  paste(N_GRID, collapse = ","), P, Q, VA_H, N_STARTS, N_SEED, CORES,
  PHI_TRUE, N_TRIALS
))
cat("NOTE: trials=", N_TRIALS,
    " (Design-59 / recovery default; multi-trial required for identifiable phi)\n",
    sep = "")
...
simulate_dgp <- function(seed, n, q = Q, p = P, phi = PHI_TRUE,
                         n_trials = N_TRIALS) {
  ## Design-110 loadings-only; beta-binomial via Beta-then-Binom.
  set.seed(seed)
  stopifnot(p >= q, n_trials >= 2L)
  ...
  for (j in seq_len(p)) {
    p_rand <- rbeta(n, mu[, j] * phi, (1 - mu[, j]) * phi)
    Y[, j] <- rbinom(n, size = n_trials, prob = p_rand)
  }
  dat <- data.frame(
    unit = factor(rep(seq_len(n), each = p)),
    trait = factor(rep(sprintf("t%02d", seq_len(p)), times = n)),
    succ = as.vector(t(Y)),
    fail = as.integer(n_trials) - as.vector(t(Y)),
    stringsAsFactors = FALSE
  )
  list(
    seed = seed, n = n, p = p, q = q, phi = phi, n_trials = n_trials,
...
fit_gtmb_va <- function(dgp) {
  ...
  result <- tryCatch(
    engine(
      y = as.numeric(dat$succ),
      n_trials = rep.int(as.numeric(dgp$n_trials), n_obs),
      X = X,
  ...
  if (!is.null(par) && !is.null(names(par))) {
    lp <- as.numeric(par[names(par) == "log_phi_betabinom"])
...
fit_gtmb_la <- function(dgp) {
  ...
  form <- as.formula(sprintf(
    "cbind(succ, fail) ~ 0 + trait + latent(0 + trait | unit, d = %d, unique = FALSE)",
    dgp$q
  ))
  fit <- tryCatch(
    gllvmTMB(
  ...
  phi_hat <- tryCatch({
    ph <- as.numeric(fit$report$phi_betabinom)
...
fit_gllvm <- function(dgp, method) {
  ...
  t0 <- proc.time()[[3L]]
  ## Attempt documented; gllvm 2.0.13 refuses beta.binomial under VA and
  ## errors under LA for every Ntrials shape tried in pre-flight.
  f <- tryCatch(
    gllvm::gllvm(
      y = dgp$Y,
...
arm_row <- function(a, seed, n) {
  data.frame(
    seed = seed, n = n, p = P, q = Q, n_trials = N_TRIALS,
    arm = a$arm,
    ok = isTRUE(a$ok),
...
wu <- tryCatch(one_job(99961L, N_GRID[[1L]]), error = function(e) {
...
run_one <- function(i) {
  tryCatch(one_job(jobs$seed[[i]], jobs$n[[i]]), error = function(e) {
    data.frame(
      seed = jobs$seed[[i]], n = jobs$n[[i]], p = P, q = Q, n_trials = N_TRIALS,
      arm = "ERROR",
...
cat("\n======== Betabinomial n-ladder (p=8 q=2 trials=", N_TRIALS,
    "; abs caps β≤0.35 / Σ≤0.50) ========\n", sep = "")
