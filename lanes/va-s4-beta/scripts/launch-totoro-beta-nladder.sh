...
# Totoro launcher — beta (continuous proportion) n-ladder: does Σ recover with n?
# Design-110 loadings-only; n ∈ {120,400,1000}; phi=5.
# Arms: gtmb LA/VA + gllvm LA; gllvm VA attempted (expect N/A on gllvm 2.0.13).
...
PROBE_SCRIPT="$SCRIPT_DIR/probe-beta-nladder.R"

ACTION="${ACTION:-dry-run}"
HOST="${TOTORO_HOST:-totoro}"
REMOTE_ROOT="${REMOTE_ROOT:-/home/snakagaw/gllvm_work/va-s4-beta-nladder-20260807}"
...
Env: PROBE_N_SEED PROBE_SEED0 PROBE_N_GRID PROBE_Q PROBE_P PROBE_VA_H
     PROBE_N_STARTS PROBE_PHI PROBE_DO_GLLVM_LA PROBE_DO_GLLVM_VA
...
case "$ACTION" in
  -h|--help) usage; exit 0 ;;
  dry-run)
    echo "S4 beta n-ladder Totoro dry-run"
    echo "  HOST=$HOST"
    ...
    echo "  n_starts=$N_STARTS phi=$PHI CORES=$CORES"
    echo "Say ACTION=sync then ACTION=smoke|full to launch."
    ;;
  sync)
    ssh "$HOST" "mkdir -p '$REMOTE_ROOT'/{scripts,results,logs}"
    scp "$LOCAL_PROBE" "$HOST:$REMOTE_ROOT/scripts/probe-beta-nladder.R"
...
nohup env PROBE_REPO="\$PROBE_REPO" PROBE_OUT="\$PROBE_OUT" \\
  ...
  PROBE_PHI=$PHI PROBE_DO_GLLVM_LA=$DO_GLLVM_LA PROBE_DO_GLLVM_VA=$DO_GLLVM_VA \\
  PILOT_CORES=2 PROBE_CORE_CAP=2 \\
  Rscript --vanilla '$REMOTE_ROOT/scripts/probe-beta-nladder.R' \\
...
EOF
    ;;
  pull)
    LOCAL_PULL="${LOCAL_PULL:-/private/tmp/va-s4-beta-nladder-20260807}"
    mkdir -p "$LOCAL_PULL"
