...
NEXT: **park** — wait for a **new** G0 (default stop; optional truncnb2/delta_ln or separate Arc-1 merge only with explicit go)
OPEN GATES (need human): none for this goal (G0=1 closed). Future menu items are **new** G0s, not continuations.
TRUTH LIVES IN:
...
RESUME:
  ...
  STATE is done. Do NOT start truncnb2/delta_ln, Arc-1 merge, fence, or Totoro unless Shinichi issues a NEW G0.
