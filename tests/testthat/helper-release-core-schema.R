...
.release_core_attempt_status_levels <- c(
  "planned", "construction_error", "fit_error", "nonfinite",
  "optimizer_failed", "nonstationary", "non_pd_hessian", "boundary",
  "geometry_failed", "usable"
)

.release_core_attempt_columns <- c(
  ...
  "relative_covariance_error", "max_eigen_ratio", "family",
  "n_trials_min", "n_trials_max", "diag_B_skip", "diag_B_all_free",
  "error_class", "error_message", "elapsed_seconds"
)

.release_core_classify_attempt <- function(
  constructed = TRUE,
  fit_error = FALSE,
  ...
  if (length(flags) != 8L || anyNA(flags)) {
    stop(
      "Attempt-classification flags must be eight non-missing logical scalars.",
      call. = FALSE
    )
  }
...
.release_core_validate_attempt_table <- function(x) {
  if (!is.data.frame(x)) stop("Attempt ledger must be a data frame.", call. = FALSE)
  missing_columns <- setdiff(.release_core_attempt_columns, names(x))
  if (length(missing_columns)) {
    stop(
      "Attempt ledger is missing frozen columns: ",
      paste(missing_columns, collapse = ", "), call. = FALSE
    )
  }
  if (any(!x$status %in% .release_core_attempt_status_levels)) {
  ...
  if (anyNA(x$detector_flagged) || any(x$detector_flagged != expected_detector)) {
    stop(
      "detector_flagged must depend only on the observable terminal status.",
      call. = FALSE
    )
  }
  if (anyNA(x$catastrophic_truth_error) ||
    ...
    stop(
      "Truth-error labels and metrics must be complete for terminal attempts.",
      call. = FALSE
    )
  }
  binomial_usable <- x$family == "binomial" & !nzchar(x$error_class) &
    !nzchar(x$error_message)
  if (any(binomial_usable &
          (x$n_trials_min != 10L | x$n_trials_max != 10L |
           x$diag_B_skip != "0;0;0" | !x$diag_B_all_free))) {
    stop(
      paste(
        "Successful binomial attempts must prove n_trials=10 and all",
        "diag_B_skip entries free."
      ),
      call. = FALSE
    )
  }
