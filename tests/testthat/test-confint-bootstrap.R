...
## Build a tiny fit with a between-site/unit covariance structure only.
...
test_that("confint() keeps Sigma_B as a legacy alias", {
  ...
  fit <- make_tiny_B_fit()
  canonical <- suppressMessages(confint(
    fit,
    parm = "Sigma_unit",
    level = 0.95,
    method = "wald"
  ...
  expect_equal(
    sub("^Sigma_unit", "Sigma_B", canonical$parameter),
    legacy$parameter
  )
  expect_equal(canonical$estimate, legacy$estimate, tolerance = 0)
...
test_that("confint(fit, parm='Sigma_unit') errors when unit tier is absent", {
  ...
  ci_unit_obs <- suppressWarnings(suppressMessages(confint(
    fit_W_only,
    parm = "Sigma_unit_obs",
    method = "wald"
  )))
  expect_s3_class(ci_unit_obs, "data.frame")
  expect_true(all(grepl("^Sigma_unit_obs\\[", ci_unit_obs$parameter)))

  ## Requesting Sigma_unit when only unit_obs is present should error.
  expect_error(
    suppressMessages(confint(
      fit_W_only,
      parm = "Sigma_unit",
