testthat::test_that("current limits begins with reader decisions", {
  ...
  ## The first table ends before the specialist sections, which still retain
  ## compact availability labels alongside their narrower technical limits.
  ...
  testthat::expect_false(grepl("Experimental/partial", opening, fixed = TRUE))
})
