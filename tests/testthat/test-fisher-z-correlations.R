## Design 09 Stage 2: Wald CIs for cross-trait correlations.
...
## ---- 1. Compatibility "fisher-z" method is accepted ------------------------

test_that("extract_correlations accepts method = 'fisher-z' alias", {
  skip_on_cran()
...
## ---- 2. Default is "wald" (not profile or fisher-z) ------------------------

test_that("extract_correlations default method is 'wald'", {
  skip_on_cran()
  fit <- make_tiny_BW_fit_fz()
  cors_default <- gllvmTMB::extract_correlations(fit, tier = "unit")
  expect_true(all(cors_default$method == "wald"))
})

## ---- 3. "fisher-z" remains a backward-compat alias of "wald" --------------

test_that("method = 'fisher-z' returns identical numbers to 'wald'", {
  skip_on_cran()
  fit <- make_tiny_BW_fit_fz()
  cors_wald <- gllvmTMB::extract_correlations(
    fit,
    tier = "unit",
    method = "wald"
  )
  ## Suppress the one-shot alias message we emit for method='fisher-z'.
  cors_fz <- suppressMessages(
...
test_that("Wald correlation CIs are guaranteed inside [-1, 1] for any rho", {
  skip_on_cran()
  fit <- make_tiny_BW_fit_fz()
