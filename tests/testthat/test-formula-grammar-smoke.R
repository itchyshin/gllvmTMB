...
test_that("meta_V(V = V) single-V additive fit parses and converges (MET-01)", {
  set.seed(131)
  n_eff <- 50
  n_trait <- 3
  df <- expand.grid(
...
test_that("meta_V compatibility syntax and proportional type are handled explicitly", {
  f_old <- gllvmTMB:::desugar_brms_sugar(
    value ~ 0 + trait + meta_V(value, V = V)
  )
  f_new <- gllvmTMB:::desugar_brms_sugar(
    value ~ 0 + trait + meta_V(V = V)
  )
  f_positional <- gllvmTMB:::desugar_brms_sugar(
    value ~ 0 + trait + meta_V(V, type = "exact")
  )

  expect_identical(deparse(f_old), deparse(f_new))
  expect_identical(deparse(f_positional), deparse(f_new))
