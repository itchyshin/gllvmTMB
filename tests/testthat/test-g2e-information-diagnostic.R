...
test_that("G2e runner validates its no-fit contract", {
  ...
  result <- system2(file.path(R.home("bin"), "Rscript"), shQuote(c("--vanilla", script, "--mode=validate", paste0("--output=", out), paste0("--pkg=", pkg_root))), stdout = TRUE, stderr = TRUE)
...
test_that("G2e preflight retains a re-readable provenance root without fitting", {
  ...
  result <- system2(file.path(R.home("bin"), "Rscript"), shQuote(c("--vanilla", script, "--mode=preflight", paste0("--output=", out), paste0("--pkg=", pkg_root))), stdout = TRUE, stderr = TRUE)
  expect_true(is.null(attr(result, "status")) || identical(attr(result, "status"), 0L))
