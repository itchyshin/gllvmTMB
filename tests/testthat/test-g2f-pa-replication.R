...
test_that("G2f preflight retains a readable, self-contained no-fit contract", {
  ...
  result <- system2(file.path(R.home("bin"), "Rscript"), shQuote(c("--vanilla", script, "--mode=preflight", paste0("--output=", root), paste0("--pkg=", pkg))), stdout = TRUE, stderr = TRUE)
...
test_that("G2f runner validates without a fit", {
  ...
  result <- system2(file.path(R.home("bin"), "Rscript"), shQuote(c("--vanilla", script, "--mode=validate", paste0("--output=", out), paste0("--pkg=", pkg))), stdout = TRUE, stderr = TRUE)
  expect_true(is.null(attr(result, "status")) || identical(attr(result, "status"), 0L))
...
test_that("G2f reconciliation closes only an existing unclosed smoke root without fitting", {
  ...
  result <- system2(file.path(R.home("bin"), "Rscript"), shQuote(c("--vanilla", script, "--mode=reconcile", paste0("--output=", root), paste0("--pkg=", pkg))), stdout = TRUE, stderr = TRUE)
  expect_true(is.null(attr(result, "status")) || identical(attr(result, "status"), 0L))
