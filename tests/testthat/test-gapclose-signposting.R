...
##     augmented-LHS check redirect conditioned on the RHS of `|`: grouping
##     by the trait column points at the response-column slope grammar
##     (`slope()`/`phylo_slope()`/`animal_slope()`); grouping by an
##     ordinary column points at the group-axis grammar (`phylo_slope(x |
##     group, tree = tree)`). Previously BOTH branches wrongly named
##     `*_indep`/`*_latent`/`*_dep`, which also refuse a slope LHS.
...
## ---- (a) .assert_no_augmented_lhs: group-axis branch -------------------

test_that("indep(1 + x | species) redirects to the group-axis slope grammar", {
  expect_snapshot(
    error = TRUE,
...
test_that("phylo_slope(x | species, tree = tree) -- the named route -- fits", {
  fx <- .gapclose_signposting_fixture()
