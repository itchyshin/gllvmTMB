## Phase B-matrix C-beta (Design 59 Group C): `Beta()` (logit link) on the
...
## square, concentration phi = 5, response strictly in (0, 1), small SPDE
## mesh. The Gaussian latent surface (drawn either on the engine's own SPDE
## precision or via the package simulator's exponential kernel) is passed
## through invlogit() and then emitted as Beta(mu * phi, (1 - mu) * phi) with
## mu held mid-range (logit intercepts near 0) so the proportions never crowd
## the 0/1 boundaries where Beta degenerates. Per the B0 memo the SPDE +
...
skip_if_not_beta_spatial_deps <- function() {
...
beta_spatial_rho_ci_any_finite <- function(fit, n_traits) {
...
expect_beta_spatial_correlations_nondegenerate <- function(fit) {
...
## Paired (latent + unique) Beta spatial fixture. Mirrors the binary-probit
...
## The logit-scale eta is emitted as Beta(mu * phi, (1 - mu) * phi) with mu
## mid-range. The single shared field gives the cross-trait correlation that
## the paired (latent + unique) spec is meant to identify.
make_beta_spatial_paired_fixture <- function(n_sites = 100L, n_traits = 3L,
                                             range_true = 0.3, phi = 5,
                                             seed = 20260529L) {
  set.seed(seed)
  ...
  ## Modest loadings so the latent eta stays mid-range and Beta does not
  ## saturate at the 0/1 boundary: moderate same-sign on traits 1 and 2,
  ...
  mu      <- stats::plogis(eta)
  df$value <- stats::rbeta(length(mu), mu * phi, (1 - mu) * phi)
...
## Single-field Beta spatial fixture for scalar / indep / dep. Uses the
## package simulator to draw a Gaussian spatial residual surface (exponential
## Matern-ish kernel) with one species per site, then logit-transforms to a
## mid-range mu and emits Beta(mu * phi, (1 - mu) * phi). One species per site
## makes `site` the unit of replication for the spatial random field.
make_beta_spatial_fixture <- function(n_sites = 100L, n_traits = 3L,
                                      spatial_range = 0.35,
                                      sigma2_spa_true = 0.5, phi = 5,
                                      seed = 20260529L) {
  ...
  df  <- sim$data
  eta <- df$value                       # Gaussian latent logit-scale surface
  mu  <- stats::plogis(eta)
  df$value <- stats::rbeta(length(mu), mu * phi, (1 - mu) * phi)
...
test_that("Beta: spatial_latent(d=1) + spatial_unique paired fits; pd_hessian TRUE; CI smoke", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Beta spatial_latent + spatial_unique fit failed to construct: %s",
  ...
  expect_beta_spatial_fit_health(fit)
  ...
  if (!beta_spatial_rho_ci_any_finite(fit, fx$n_traits)) {
    skip("Profile CI for rho:spatial returned no finite bound on any pair (Beta spatial paired); honest skip rather than relax assertion")
  }
  expect_true(beta_spatial_rho_ci_any_finite(fit, fx$n_traits))
})
...
test_that("Beta: spatial_scalar(0 + trait | site) fits; tau tied; tau_spde profile CI finite", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Beta spatial_scalar fit failed to construct: %s",
  ...
  if (inherits(ci, "error")) {
    skip(sprintf(
      "confint(parm = 'tau_spde', method = 'profile') errored on Beta spatial_scalar: %s",
      conditionMessage(ci)
    ))
  }
  ...
  if (!any(is.finite(ci))) {
    skip("tau_spde profile CI returned no finite bound on Beta spatial_scalar; honest skip rather than relax assertion")
  }
...
test_that("Beta: spatial_indep(0 + trait | site) fits; pd_hessian TRUE", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Beta spatial_indep fit failed to construct: %s",
      if (inherits(fit, "error")) conditionMessage(fit) else "non-gllvmTMB return"
    ))
  }
  if (!isTRUE(fit$opt$convergence == 0L) ||
...
test_that("Beta: spatial_dep(0 + trait | site) fits; pd_hessian TRUE; CI smoke OR correlations non-degenerate", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Beta spatial_dep fit failed to construct: %s",
      if (inherits(fit, "error")) conditionMessage(fit) else "non-gllvmTMB return"
    ))
  }
  if (!isTRUE(fit$opt$convergence == 0L) ||
  ...
  ci_ok <- beta_spatial_rho_ci_any_finite(fit, fx$n_traits)
  cor_df <- tryCatch(
  ...
  if (!ci_ok && !cor_ok) {
    skip("Neither rho:spatial profile CI nor extract_correlations(tier='spatial') was non-degenerate (Beta spatial_dep); honest skip rather than relax assertion")
  }
