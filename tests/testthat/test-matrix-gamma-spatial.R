## Phase B-matrix C-gam (Design 59 Group C): `Gamma(link = "log")` on the
...
## for the gamma (log-link) branch, one structural keyword per `test_that`.
##
...
## structure" sense, not a narrow numeric band.
##
## Fixture: 3 traits, ~100 sites with random 2D coordinates in the unit
## square, gamma shape phi = 2 (=> CV = 1/sqrt(2) ~ 0.707), strictly-positive
## response, small SPDE mesh. The Gaussian latent surface (drawn either on
## the engine's own SPDE precision or via the package simulator's exponential
## kernel) is passed through exp() and emitted as
## Gamma(shape = phi, scale = mu / phi) so E(y) = mu, CV(y) = 1/sqrt(phi),
## with the log-scale trait intercepts held near 0 so E(y) = exp(eta) ~ 1 and
## the gamma mean stays identifiable at this fixture size. Per the B0 memo the
## SPDE + mean-dependent combination is genuinely noisy at small n -- and a
## CV of ~0.71 is high overdispersion -- so the paired (latent + unique) and
## dep cells are the ones most likely to honest-SKIP.
##
...
skip_if_not_gamma_spatial_deps <- function() {
...
gamma_spatial_rho_ci_any_finite <- function(fit, n_traits) {
...
expect_gamma_spatial_correlations_nondegenerate <- function(fit) {
...
## Paired (latent + unique) gamma spatial fixture. Mirrors the binary-probit
...
## The log-scale eta is emitted as Gamma(shape = phi, scale = mu / phi) with
## the trait intercepts near 0 so E(y) = exp(eta) ~ 1. The single shared field
## gives the cross-trait correlation that the paired (latent + unique) spec is
## meant to identify.
make_gamma_spatial_paired_fixture <- function(n_sites = 100L, n_traits = 3L,
  ...
  ## Modest loadings so the latent eta stays near 0 (E(y) ~ 1, no overflow):
  ## moderate same-sign on traits 1 and 2, opposite sign on trait 3 -- a
  ## non-trivial cross-trait correlation.
  ...
  mu      <- exp(eta)
  ## gamma shape = phi (so CV = 1/sqrt(phi)); E(y) = mu; scale = mu / shape.
  df$value <- stats::rgamma(length(mu), shape = phi, scale = mu / phi)
...
## Single-field gamma spatial fixture for scalar / indep / dep. Uses the
## package simulator to draw a Gaussian spatial residual surface (exponential
## Matern-ish kernel) with one species per site, then exp-transforms to a
## mean near 1 and emits Gamma(shape = phi, scale = mu / phi). One species per
## site makes `site` the unit of replication for the spatial random field.
make_gamma_spatial_fixture <- function(n_sites = 100L, n_traits = 3L,
                                       spatial_range = 0.35,
  ...
  df  <- sim$data
  eta <- df$value                       # Gaussian latent log-scale surface
  mu  <- exp(eta)
  df$value <- stats::rgamma(length(mu), shape = phi, scale = mu / phi)
...
test_that("Gamma: spatial_latent(d=1) + spatial_unique paired fits; pd_hessian TRUE; CI smoke", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Gamma spatial_latent + spatial_unique fit failed to construct: %s",
  ...
  expect_gamma_spatial_fit_health(fit)
  ...
  if (!gamma_spatial_rho_ci_any_finite(fit, fx$n_traits)) {
    skip("Profile CI for rho:spatial returned no finite bound on any pair (Gamma spatial paired); honest skip rather than relax assertion")
  }
  expect_true(gamma_spatial_rho_ci_any_finite(fit, fx$n_traits))
})
...
test_that("Gamma: spatial_scalar(0 + trait | site) fits; tau tied; tau_spde profile CI finite", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Gamma spatial_scalar fit failed to construct: %s",
  ...
  if (inherits(ci, "error")) {
    skip(sprintf(
      "confint(parm = 'tau_spde', method = 'profile') errored on Gamma spatial_scalar: %s",
      conditionMessage(ci)
    ))
  }
  ...
  if (!any(is.finite(ci))) {
    skip("tau_spde profile CI returned no finite bound on Gamma spatial_scalar; honest skip rather than relax assertion")
  }
...
test_that("Gamma: spatial_indep(0 + trait | site) fits; pd_hessian TRUE", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Gamma spatial_indep fit failed to construct: %s",
      if (inherits(fit, "error")) conditionMessage(fit) else "non-gllvmTMB return"
    ))
  }
  if (!.fit_stationary_for_recovery_test(fit)) {
...
test_that("Gamma: spatial_dep(0 + trait | site) fits; pd_hessian TRUE; CI smoke OR correlations non-degenerate", {
  ...
  if (inherits(fit, "error") || !inherits(fit, "gllvmTMB_multi")) {
    skip(sprintf(
      "Gamma spatial_dep fit failed to construct: %s",
      if (inherits(fit, "error")) conditionMessage(fit) else "non-gllvmTMB return"
    ))
  }
  if (!.fit_stationary_for_recovery_test(fit)) {
  ...
  ci_ok <- gamma_spatial_rho_ci_any_finite(fit, fx$n_traits)
  cor_df <- tryCatch(
  ...
  if (!ci_ok && !cor_ok) {
    skip("Neither rho:spatial profile CI nor extract_correlations(tier='spatial') was non-degenerate (Gamma spatial_dep); honest skip rather than relax assertion")
  }
