...
# ---- Shared-group independence pin (issue #336 gate 2) --------------------
# The covariate RE block b_x must stay independent of the response RE even when
# both share the same grouping factor. Without a response-side (1 | grp) the
# Phase 2b recovery cells above do not exercise this gate.

test_that("shared-group independence: response (1|grp) + covariate (1|grp) keeps b_x unbiased", {
  skip_if_not_heavy()
  set.seed(3362L)
  n_sites <- 180L
  n_group <- 18L
  ...
  expect_identical(fit$missing_data$predictors$x$version, "phase2b")
  expect_true(isTRUE(fit$missing_data$predictors$x$random$enabled))
  expect_identical(fit$missing_data$predictors$x$random$group, "grp")

  par <- fit$tmb_obj$env$parList(fit$opt$par)
  mu_col <- fit$missing_data$predictors$x$mu_col
  b_x_hat <- par$b_fix[mu_col]
  ## Independence pin: sharing the grouping factor must not bias the response
  ## slope on the imputed covariate (issue #336 gate 2).
  expect_equal(b_x_hat, b_x_true, tolerance = 0.35)
  expect_lt(max(abs(fit$tmb_obj$gr(fit$opt$par))), 1e-2)
})
