...
test_that("extract_correlations(link_residual = 'auto') adds per-family residuals", {
  ...
  ## Default output is point-only regardless of the link-residual choice.
  expect_true(all(R_auto$interval_status == "none"))
  expect_true(all(R_none$interval_status == "none"))
...
## ---- 5: extract_Sigma() with part = 'unique' on a no-Psi fit ----------
