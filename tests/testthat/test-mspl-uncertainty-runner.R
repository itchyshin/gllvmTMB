test_that("private MSPL uncertainty runner retains the Hessian-only campaign fence", {
  ...
  expect_match(runner, 'c\\("both", "hessian_only", "jackknife_only"\\)')
  expect_match(runner, "gllvmTMB_mspl_jackknife_feasibility")
  expect_match(runner, "jackknife_status")
  expect_match(runner, "jackknife_coverage_unconditional")
...
test_that("jackknife runner summary retains failed candidates and rejects stale provenance", {
  runner_path <- test_path("..", "..", "inst", "sim", "lane-b-uncertainty",
                           "run-mspl-uncertainty.R")
  root <- tempfile("mspl-jackknife-runner-")
  ...
  manifest <- data.frame(
    ...
    procedure = "jackknife_only", campaign_id = "campaign-a", source_sha = "source-a"
  )
  ...
  receipt <- data.frame(
    ...
    fit_status = "ok", hessian_status = "not_run",
    jackknife_status = "delete_site_refit_failure",
  ...
  expect_true(all(summary$jackknife_available == 0))
  expect_true(all(summary$jackknife_coverage_unconditional == 0))
