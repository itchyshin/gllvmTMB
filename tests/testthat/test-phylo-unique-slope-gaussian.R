...
test_that("phylo_unique augmented fit aborts when n_lhs_cols is forced to 1", {
  ...
  ## Robust default fixture (n_sp = 60): a tiny n_sp = 10 fit is knife-edge under
  ## any Ainv reimplementation (Gaussian is less fragile than binomial, but same
  ## pattern). This guard only needs a converged fit to corrupt, so fixture size
  ## is incidental. (phylo_unique(slope) is slated for deprecation.)
  fx <- make_phylo_unique_slope_fixture()
  fit <- fit_phylo_unique_slope_pair(fx)$long
