...
test_that("PVT-02 target is the total unit-tier diagonal", {
  spec <- pvt02_target_spec(
    ...
    theta_psi = log(c(2, 3, 4)),
    n_traits = 3,
    d = 2,
    trait = 2
  )
  ## Engine order: diagonal first (L11, L22), then strict lower entries
  ## column-by-column (L21, L31, L32).
  expect_equal(spec$lambda, rbind(c(1, 0), c(3, 2), c(4, 5)))
  expect_equal(spec$psi_sq, c(4, 9, 16))
  expect_equal(spec$V, 2^2 + 3^2 + 3^2)
  expect_equal(spec$log_V, log(22))
})

test_that("analytic log-V gradients match central finite differences for both targets", {
  ...
  for (trait in 1:2) {
    ...
    analytic <- pvt02_target_spec(theta_lambda, theta_psi, 3, 2, trait)$gradient
...
test_that("one-df LR roots require a real bracket and invert symmetrically", {
  ...
  expect_equal(
    pvt02_profile_root(score, -4, 0)$root,
    -sqrt(crit),
    tolerance = 1e-7
  )
  expect_equal(
    pvt02_profile_root(score, 0, 4)$root,
    sqrt(crit),
    tolerance = 1e-7
  )
...
test_that("promotion fails closed on wrong cell, insufficient attempts, and weak lower band", {
  ...
  expect_false(
    pvt02_promotion_verdict(
      cell,
      modifyList(good, list(n_attempted = 4999L)),
      TRUE
    )$promote
  )
  expect_false(
    pvt02_promotion_verdict(
      cell,
      modifyList(good, list(lower_band = 0.939)),
      TRUE
    )$promote
  )
  expect_false(
    pvt02_promotion_verdict(
      modifyList(cell, list(n_units = 150L)),
      good,
      TRUE
    )$promote
  )
  expect_false(
    pvt02_promotion_verdict(
      modifyList(cell, list(unique = FALSE)),
      good,
      TRUE
    )$promote
  )
