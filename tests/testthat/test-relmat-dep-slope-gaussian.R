## Design 55 §A5 + Design 56 §9.5f-dep — relmat_dep(1 + x | id) Gaussian recovery
##
...
## (PR #282); customises for the (relmat × dep) cell per Design 55 §5.
##
## Σ_b shape per Design 56 §5.3: full 2T × 2T unstructured with user-supplied A.
...
skip_until_stage3 <- function() {
  testthat::skip(
    "Stage 3 engine work in progress; see docs/design/56-augmented-lhs-engine-stage3.md §9.1-§9.5f-dep"
  )
}
...
test_that(
  ...
  ## structural matrix (relmat) and the keyword (dep) accordingly.
  ## Phase 56.5f-dep fills in the body.
