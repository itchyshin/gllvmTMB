...
test_that("slope_sd_ci() recovers a known-truth slope SD with a finite interval", {
  ...
  ci_var <- slope_sd_ci(fit, scale = "variance")
  ...
  ## print() surfaces the fence and the rr_B_slope caveat (this fixture
  ## has both theta_diag_B_slope and theta_rr_B_slope).
...
test_that("slope_sd_ci() guard: boundary collapse (se_theta > 10) -> NA interval + warning (guard proven to fire)", {
  ...
  ## Without the boundary gate, this is exactly the "plausible-looking
  ## garbage" failure mode: a FINITE numeric interval that spans an
  ## astronomically wide range and carries no information.
  z <- stats::qnorm(0.975)
  ...
  expect_identical(ci$status, c("boundary", "ok"))
  expect_true(is.na(ci$lower[1]) && is.na(ci$upper[1]))
  expect_false(is.na(ci$lower[2]) || is.na(ci$upper[2]))
  expect_true(is.finite(ci$estimate[1]))
})
