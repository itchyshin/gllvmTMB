## Design 55 §A4 + Design 56 §9.5e-latent — spatial_latent(1 + x | id) Gaussian recovery
##
...
## (PR #282); customises for the (spatial × latent) cell per Design 55 §5.
##
## Σ_b shape per Design 56 §5.3: block-diagonal per LHS column (per-column Lambda factor-analytic) × SPDE precision.
...
skip_until_stage3 <- function() {
  testthat::skip(
    "Stage 3 engine work in progress; see docs/design/56-augmented-lhs-engine-stage3.md §9.1-§9.5e-latent"
  )
}
...
test_that(
  ...
  ## structural matrix (spatial) and the keyword (latent) accordingly.
  ## Phase 56.5e-latent fills in the body.
