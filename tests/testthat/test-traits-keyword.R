...
## dispatches to the long-format engine. The trait-stacked RHS (`0 +
## trait`, `(0 + trait):x`, `latent(0 + trait | g)`) is supplied verbatim
## by the user — no semantic rewriting on the RHS.
##
## Verifications:
...
## ---- Test 2: byte-equivalent objective vs hand-rolled long format ---------

test_that("traits() fit matches hand-rolled long-format pivot byte-for-byte", {
  skip_if_not_installed("tidyr")
  wide <- make_wide_df()
  trait_cols <- c("trait_1", "trait_2", "trait_3", "trait_4")
  ...
  fit_long <- suppressMessages(suppressWarnings(gllvmTMB::gllvmTMB(
    .y_wide_ ~ 0 + trait + (0 + trait):env_temp +
      latent(0 + trait | individual, d = 1) +
      unique(0 + trait | individual),
    data   = long,
    unit   = "individual",
  ...
  expect_equal(fit_wide$opt$objective, fit_long$opt$objective, tolerance = 1e-10)
})
...
test_that("traits() supports tidyselect verbs (all_of, starts_with, matches, bare names)", {
  ...
  fit_bare <- suppressMessages(suppressWarnings(gllvmTMB::gllvmTMB(
    traits(trait_1, trait_2, trait_3, trait_4) ~ 0 + trait + (0 + trait):env_temp +
      latent(0 + trait | individual, d = 1) +
      unique(0 + trait | individual),
    data   = wide,
    unit   = "individual",
  ...
  fit_starts <- suppressMessages(suppressWarnings(gllvmTMB::gllvmTMB(
    traits(tidyselect::starts_with("trait_")) ~ 0 + trait + (0 + trait):env_temp +
      latent(0 + trait | individual, d = 1) +
      unique(0 + trait | individual),
    data   = wide,
    unit   = "individual",
  ...
  fit_match <- suppressMessages(suppressWarnings(gllvmTMB::gllvmTMB(
    traits(tidyselect::matches("^trait_[0-9]+$")) ~ 0 + trait + (0 + trait):env_temp +
      latent(0 + trait | individual, d = 1) +
      unique(0 + trait | individual),
    data   = wide,
    unit   = "individual",
  ...
  expect_equal(fit_starts$opt$objective, fit_all$opt$objective, tolerance = 1e-10)
  expect_equal(fit_match$opt$objective,  fit_all$opt$objective, tolerance = 1e-10)
})

## ---- Test 4: NA handling drops cells, reports count -----------------------

test_that("traits() drops NA cells with values_drop_na = TRUE and reports count", {
  skip("0.2.0: fix-effects-only fit hits the removed sdmTMB() fallback. Migrate to covstruct + drop-NA in 0.2.x.")
  skip_if_not_installed("tidyr")
  wide <- make_wide_df()
  trait_cols <- c("trait_1", "trait_2", "trait_3", "trait_4")
  ...
  withCallingHandlers(
    fit <- suppressWarnings(gllvmTMB::gllvmTMB(
      traits(tidyselect::all_of(trait_cols)) ~ 0 + trait +
        (0 + trait):env_temp,
      data   = wide,
      unit   = "individual",
      family = gaussian()
  ...
  expect_true(any(grepl("\\b3\\b", msgs) & grepl("(?i)dropp", msgs, perl = TRUE)),
...
test_that("traits() does not intercept family = list(...) (mixed-family pass-through)", {
  ...
  fit <- tryCatch(
    suppressMessages(suppressWarnings(gllvmTMB::gllvmTMB(
      traits(tidyselect::all_of(c("trait_1", "trait_2"))) ~ 0 + trait +
        (0 + trait):env_temp,
      data    = wide,
      unit    = "individual",
  ...
  if (inherits(fit, "error")) {
    expect_false(grepl("traits", conditionMessage(fit), fixed = TRUE),
...
test_that("long-format calls without traits() are not intercepted", {
  set.seed(42)
  long <- gllvmTMB::simulate_site_trait(
    n_sites = 20, n_species = 4, n_traits = 4,
...
test_that("traits() rejects a weights matrix with a pointer to gllvmTMB_wide()", {
  ...
  wide <- make_wide_df()
  trait_cols <- c("trait_1", "trait_2", "trait_3", "trait_4")
  W_mat <- matrix(runif(nrow(wide) * length(trait_cols), 0.5, 1.5),
                  nrow = nrow(wide), ncol = length(trait_cols))
  expect_error(
    suppressMessages(suppressWarnings(gllvmTMB::gllvmTMB(
      traits(tidyselect::all_of(trait_cols)) ~ 0 + trait + (0 + trait):env_temp,
      data    = wide,
