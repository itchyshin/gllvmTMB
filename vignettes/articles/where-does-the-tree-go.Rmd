...
These examples separate tree-structured covariance from ordinary residual
covariance. They illustrate one synthetic realization; they do not establish
parameter recovery or interval coverage.
...
```{r axis-map, fig.width=8, fig.height=3.5, fig.alt="Two response matrices place the tree on species rows for morphology and species coefficient columns for communities. Both retain an ordinary covariance source."}
axes <- data.frame(
...
We simulate 80 species and six synthetic traits on standardized scales: leaf area, specific leaf
...
no within-species variance. These are simulated scale units, not
empirical measurements subsequently centred and divided by their sample SD.

\[
Y=XB+U_{\mathrm{phy}}+U_{\mathrm{non}}.
\]
\[
...
=A\otimes\Sigma_{\mathrm{phy}}+I\otimes\Sigma_{\mathrm{non}}.
\]
...
These equations describe the simulated components. The fitted Gaussian
covariance also includes a fixed $\sigma_\epsilon^2 I$ stabilizer, defined
below; the simulation itself has no additional observation-error draw.
...
```{r morphology-figure, fig.width=8, fig.height=6, fig.alt="Phylogenetic and ordinary trait-correlation heatmaps above four-component variance bars. The fitted phylogenetic diagonal is near zero; ordinary diagonal variation remains visible."}
...
knitr::kable(truth_estimate[, 1:5], digits = 3, row.names = FALSE,
  ...
  caption = "Total variance within each source. Fixed observation stabilizer excluded; one realization, not recovery evidence.")
knitr::kable(truth_estimate[, c(1, 6:9)], digits = 3, row.names = FALSE,
  col.names = table_names,
  caption = "Communality: the shared fraction of variance within each source.")
```
...
\[
y_{is}=\alpha_{C_s}+\beta_{C_s}z_i+b_{0s}+b_{1s}z_i+e_{is},
\]
\[
\operatorname{Cov}(e_i)=\Lambda_{\mathrm{site}}\Lambda_{\mathrm{site}}^{\mathsf T}+\Psi_{\mathrm{site}}.
\]

For the phylogenetic coefficient model,
\[
\operatorname{Cov}(b_s,b_t)=(K_\rho)_{st}\Sigma_b,
\]
\[
...
As in Example 1, the fitted likelihood adds the fixed observation stabilizer
to the simulated site covariance shown above.
...
The code below uses planted species
...
```{r community-pathway-coefficients, fig.width=8, fig.height=4, fig.cap="Thin lines are planted species curves; dashed lines are fitted IID pathway means. Species curves are not fitted random effects. The right panel compares intercept and latitude-slope deviations, not species pairs."}
...
The fitted tree-mixture parameter is `r sprintf("%.3g", coefphy$rho)`, compared
with the planted value 0.60. It describes dependence **across species**.
The two-by-two matrix instead describes the correlation between species'
intercept and latitude-slope deviations. These are different questions.
A fitted mixture near zero supplies little evidence for tree-structured
coefficient variation in this realization, even though it was planted;
numerical convergence alone does not demonstrate recovery.

```{r community-residual-ordination, fig.width=8, fig.height=3.6, fig.alt="A heatmap shows residual associations among all 50 species; a scatterplot shows the 150 sites on two ordinary latent axes."}
...
All three models passed a bounded check using three deterministic starts,
with matching long- and wide-format results. This checks numerical stability
in this realization; neither example makes a recovery, interval, or causal
interaction claim.
